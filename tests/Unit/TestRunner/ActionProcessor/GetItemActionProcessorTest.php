...
class GetItemActionProcessorTest extends KernelTestCase
{
    ...
    private const EXPECTED_ACTION_NAME = 'getItem';

    private GetItemActionProcessor $subject;
    private SignedUrlGeneratorRegistry $signedUrlGeneratorRegistry;
    private GetItemService $getItemServiceMock;
    private Logger $auditDeliveryExecutionLogger;
    ...
    public function setUp(): void
    {
        ...
        $this->getItemServiceMock = $this->createMock(GetItemService::class);

        $this->auditDeliveryExecutionLogger = static::getContainer()->get('monolog.logger.audit_delivery_execution');

        $this->getItemServiceMock->method('getItem')
            ->willReturn([
                'success' => true,
                'itemIdentifier' => 'Item-Q02',
                'data' => 'mocked item data',
                'assets' => ['img' => ['logo.png' => '/url/to/logo.png']],
                'type' => 'mockedType',
            ]);

        $this->subject = new GetItemActionProcessor($this->getItemServiceMock, $this->auditDeliveryExecutionLogger);
    }

    public function testGetName(): void
    {
        $this->assertEquals(GetItemActionProcessor::ACTION_NAME, $this->subject->getActionName());
    }

    public function testProcess(): void
    {
        $deliveryExecution = $this->createTestDeliveryExecution(
            id: 'userId#BasicAssets#resultId#tenantId',
        ...
        $this->assertEquals('getItem', $response['name']);
        $this->assertEquals('getItem_1', $response['id']);

        $this->assertArrayHasKey('values', $response);
        $this->assertEquals('Item-Q02', $response['values']['itemIdentifier']);

        $this->assertHasLogRecordWithMessage(
            '[userId#BasicAssets#resultId#tenantId] - test taker has requested the following Item: Item-Q02',
            Logger::INFO,
