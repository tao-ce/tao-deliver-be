...
class SetAnnotationCommentActionTest extends WebTestCase
{
    use DomainTestingTrait;
    use DocumentTestingTrait;
    use OAuth2SecurityTestingTrait;

    private const BASE_URL = '/api/v1/delivery-executions/%s/scoring/annotation-comment';
    ...
    public function testAddNewAnnotations(): void
    {
    ...
    public function testRemoveAnnotationsBySendEmpty(): void
    {
        $deliveryExecution = $this->getDeliveryExecution();
    ...
    public function testSendNotReviewDelivery(): void
    {
        $deliveryExecution = $this->getDeliveryExecution();

        $this->saveDocument($deliveryExecution);

        $this->doRequest($deliveryExecution->getId(), 'test', keyPrefix: '');
    ...
    private function doRequest(
        ...
        $this->client->request(
            ...
            uri: sprintf(self::BASE_URL, rawurlencode($keyPrefix . $deliveryExecutionId)),
            server: [
