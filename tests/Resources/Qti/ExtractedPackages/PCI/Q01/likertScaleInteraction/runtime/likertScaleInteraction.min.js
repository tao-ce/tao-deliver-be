...
var markup=$container.html();markup=xml.removeNamespace(markup),$container.html(markup),math.render($container),tooltip.render($container)}}}),define("likertScaleInteraction/runtime/js/renderer",["taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/OAT/util/html"],function($,html){"use strict";function renderChoices(id,$container,config){var $li,level=parseInt(config.level)||5,$ul=$container.find("ul.likert");$ul.empty();for(var i=1;i<=level;i++)$li=$("<li>",{class:"likert"}),$li.append($("<input>",{type:"radio",name:id,value:i})),$ul.append($li)}function renderLabels(id,$container,config,assetManager){var $ul=$container.find("ul.likert"),$labelMin=$("<span>",{
class:"likert-label likert-label-min"}).html(config["label-min"]),$labelMax=$("<span>",{class:"likert-label likert-label-max"}).html(config["label-max"]);$labelMin.append($("<img>",{src:assetManager.resolve("likertScaleInteraction/runtime/assets/ThumbDown.png")}).css({top:6,marginLeft:12})),$labelMax.prepend($("<img>",{src:assetManager.resolve("likertScaleInteraction/runtime/assets/ThumbUp.png")}).css({top:2,marginRight:12})),$ul.before($labelMin),$ul.after($labelMax)}return{render:function(id,container,config,assetManager){var $container=$(container);renderChoices(id,$container,config),renderLabels(id,$container,config,assetManager),html.render($container.find(".prompt"));
},renderChoices:function(id,container,config){renderChoices(id,$(container),config)}}}),define("likertScaleInteraction/runtime/js/likertScaleInteraction",["qtiCustomInteractionContext","taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/OAT/util/event","likertScaleInteraction/runtime/js/renderer"],function(qtiCustomInteractionContext,$,event,renderer){"use strict";var likertScaleInteraction={id:-1,getTypeIdentifier:function(){return"likertScaleInteraction"},initialize:function(id,dom,config,assetManager){var self=this;this.id=id,this.dom=dom,this.config=config||{},event.addEventMgr(this),renderer.render(this.id,this.dom,this.config,assetManager),qtiCustomInteractionContext.notifyReady(this),
this.on("levelchange",function(level){self.config.level=level,renderer.renderChoices(self.id,self.dom,self.config)})},setResponse:function(response){var $container=$(this.dom),value=response&&response.base?parseInt(response.base.integer):-1;$container.find('input[value="'+value+'"]').prop("checked",!0)},getResponse:function(){var $container=$(this.dom),value=parseInt($container.find("input:checked").val())||0;return{base:{integer:value}}},resetResponse:function(){var $container=$(this.dom);$container.find("input").prop("checked",!1)},destroy:function(){var $container=$(this.dom);$container.off().empty()},setSerializedState:function(state){state&&state.response&&this.setResponse(state.response);
},getSerializedState:function(){return{response:this.getResponse()}}};qtiCustomInteractionContext.register(likertScaleInteraction)}),define(["likertScaleInteraction/runtime/js/likertScaleInteraction"],function(PCI){return PCI});
//# sourceMappingURL=likertScaleInteraction.min.js.map