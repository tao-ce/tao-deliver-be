...
}}),define("likertCompactInteraction/likert/runtime/js/renderer",["taoQtiItem/portableLib/jquery_2_1_1","likertCompactInteraction/likert/runtime/js/assets"],function($,assets){"use strict";function renderChoices(id,$container,config){var i,$li,level=parseInt(config.level)||5,$ul=$container.find("ul.likert");for($ul.empty(),i=1;i<=level;i++)$li=$("<li>",{class:"likert"}),$li.append($("<input>",{type:"radio",name:id,value:i})),$ul.append($li)}function renderLabels($container,config){var $ul=$container.find("ul.likert"),$labelMin=$("<span>",{class:"likert-label likert-label-min"}).html(config["label-min"]),$labelMax=$("<span>",{class:"likert-label likert-label-max"
}).html(config["label-max"]);$ul.before($labelMin).before(assets.thumbDown),$ul.after($labelMax).after(assets.thumbUp)}return{render:function(id,container,config,assetManager){var $container=$(container);renderChoices(id,$container,config),renderLabels($container,config,assetManager)},renderChoices:function(id,container,config){renderChoices(id,$(container),config)}}}),function(){function n(n,t,e){e=(e||0)-1;for(var r=n?n.length:0;++e<r;)if(n[e]===t)return e;return-1}function t(t,e){var r=typeof e;if(t=t.l,"boolean"==r||null==e)return t[e]?0:-1;"number"!=r&&"string"!=r&&(r="object");var u="number"==r?e:b+e;return t=(t=t[r])&&t[u],"object"==r?t&&-1<n(t,e)?0:-1:t?0:-1;
...
e.callback.apply({type:event,ns:[]},data)})}}}),define("taoQtiItem/portableLib/OAT/util/event",["taoQtiItem/portableLib/OAT/util/EventMgr"],function(EventMgr){return{addEventMgr:function(instance){var eventMgr=new EventMgr;instance.on=function(event,callback){eventMgr.on(event,callback)},instance.off=function(event){eventMgr.off(event)},instance.trigger=function(event,data){eventMgr.trigger(event,data)}}}}),define("likertCompactInteraction/likert/runtime/js/likertInteraction",["qtiCustomInteractionContext","taoQtiItem/portableLib/jquery_2_1_1","likertCompactInteraction/likert/runtime/js/renderer","taoQtiItem/portableLib/OAT/util/event"],function(qtiCustomInteractionContext,$,renderer,event){
"use strict";var _typeIdentifier="likertCompactInteraction",likertInteraction={typeIdentifier:_typeIdentifier,getInstance:function(dom,config,state){var response=config.boundTo;this.initialize(Object.getOwnPropertyNames(response).pop(),dom,config.properties,config.assetManager),this.setSerializedState(state),"function"==typeof config.onready&&config.onready(this,this.getState())},getState:function(){return this.getSerializedState()},oncompleted:function(){this.destroy()},getResponse:function(){var $container=$(this.dom),value=parseInt($container.find("input:checked").val())||0;return{base:{integer:value}}},destroy:function(){var $container=$(this.dom);$container.off().empty();
},getTypeIdentifier:function(){return _typeIdentifier},initialize:function(id,dom,config,assetManager){var self=this;event.addEventMgr(this),this.dom=dom,this.config=config||{},renderer.render(id,this.dom,this.config,assetManager),this.on("levelchange",function(level){self.config.level=level,renderer.renderChoices(id,self.dom,self.config)})},setResponse:function(response){var $container=$(this.dom),value=response&&response.base?parseInt(response.base.integer):-1;$container.find('input[value="'+value+'"]').prop("checked",!0)},resetResponse:function(){var $container=$(this.dom);$container.find("input").prop("checked",!1)},setSerializedState:function(state){state&&state.response&&this.setResponse(state.response);
},getSerializedState:function(){return{response:this.getResponse()}}};return qtiCustomInteractionContext.register(likertInteraction),likertInteraction}),define(["likertCompactInteraction/likert/runtime/js/likertInteraction"],function(IMSPCI){return IMSPCI});
//# sourceMappingURL=likertInteraction.min.js.map