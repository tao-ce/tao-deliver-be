...
}),define("parccTei/pciCreator/ims/graphZoomNumberLineInteraction/runtime/libs/axisZoom",["taoQtiItem/portableLib/lodash"],function(_){"use strict";function createZoomable(axis){function addRectangle(position,coord){var rect=paper.rect(position,top,subDivisionSize,2*config.divisionWidth);rect.attr({stroke:"#1a6596",fill:"#e6eef4"}),rect.hide(),clickableSet.push(rect),clickables.push({position:position,rect:rect,coord:coord,active:!1})}for(var clickables=[],paper=axis.getCanvas(),config=axis.getConfig(),clickableSet=paper.set(),subDivisionSize=config.unitSize/config.unitSubDivision,subDivisionIncrement=1/config.unitSubDivision,left=config.left,top=config.top-config.divisionWidth,i=config.min;i<config.max;i++){
if(addRectangle(left,i),i<config.max)for(var left_=left,i_=i,j=2;j<=config.unitSubDivision;j++)left_+=subDivisionSize,i_+=subDivisionIncrement,addRectangle(left_,i_);left+=config.unitSize}return clickableSet.insertBefore(axis.getSet()),clickables}function drawPath(paper,points){var pathStr="";return _.each(points,function(point){pathStr+=pathStr?"L":"M",pathStr+=point.toString()}),pathStr+="Z",paper.path(pathStr).attr({stroke:"#1a6596",fill:"#e6eef4"})}function drawZoomEffect(paper,from,to){var source=from.getBBox(),target=to.getBBox(),points=[[source.x,source.y+source.height],[source.x+source.width,source.y+source.height],[target.x+target.width,target.y],[target.x,target.y]],zoomEffect=drawPath(paper,points);
return zoomEffect&&(to.zoomEffect&&to.zoomEffect.remove(),to.zoomEffect=zoomEffect,zoomEffect.insertBefore(to)),zoomEffect}return{createZoomable:createZoomable,drawZoomEffect:drawZoomEffect}}),define("normalize",[],function(){function convertURIBase(uri,fromBase,toBase){if(uri.match(absUrlRegEx)||uri.match(protocolRegEx))return uri;uri=removeDoubleSlashes(uri);var toBaseProtocol=toBase.match(protocolRegEx),fromBaseProtocol=fromBase.match(protocolRegEx);return!fromBaseProtocol||toBaseProtocol&&toBaseProtocol[1]==fromBaseProtocol[1]&&toBaseProtocol[2]==fromBaseProtocol[2]?relativeURI(absoluteURI(uri,fromBase),toBase):absoluteURI(uri,fromBase)}function absoluteURI(uri,base){
...
if(sheet.href==link.href)return clearInterval(loadInterval),callback()}},10);link.href=url,head.appendChild(link)};return cssAPI.normalize=function(name,normalize){return".css"==name.substr(name.length-4,4)&&(name=name.substr(0,name.length-4)),normalize(name)},cssAPI.load=function(cssId,req,load,config){(useImportLoad?importLoad:linkLoad)(req.toUrl(cssId+".css"),load)},cssAPI}),define("css!parccTei/pciCreator/ims/graphZoomNumberLineInteraction/runtime/css/graphZoomNumberLineInteraction",[],function(){}),define("graphZoomNumberLineInteraction/runtime/graphZoomNumberLineInteraction",["taoQtiItem/portableLib/jquery_2_1_1","qtiCustomInteractionContext","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/OAT/util/event","taoQtiItem/portableLib/OAT/scale.raphael","parccTei/portableLib/pointFactory","parccTei/portableLib/axisFactory","parccTei/pciCreator/ims/graphZoomNumberLineInteraction/runtime/libs/axisZoom","css!parccTei/pciCreator/ims/graphZoomNumberLineInteraction/runtime/css/graphZoomNumberLineInteraction"],function($,qtiCustomInteractionContext,_,event,scaleRaphael,pointFactory,axisFactory,axisZoom){
"use strict";function createCanvas($container,config){var padding=2,width=2*padding+config.unitSize*(2+config.max-config.min),paper=scaleRaphael($(".shape-container",$container)[0],width,260);return paper}function buildAxisConfig(rawConfig){var _default={top:60,left:50,unitSize:50,min:0,max:10,unitSubDivision:2,arrows:!0,point:{color:"#266d9c"}};return _.merge(_default,{min:void 0===rawConfig.min?void 0:parseInt(rawConfig.min),max:void 0===rawConfig.max?void 0:parseInt(rawConfig.max),unitSubDivision:void 0===rawConfig.unitSubDivision?void 0:parseInt(rawConfig.unitSubDivision),point:{color:rawConfig.graphColor}})}function _format(num){var str=num+"";return str.length>5&&(str.match(/99999\d$/)?str=Math.round(1e4*num)/1e4:(str=Math.floor(1e3*num)/1e3,
str=str.toString(),str.match(/^\d+\.\d{3,}$/)&&(str+="..."))),str}var _typeIdentifier="graphZoomNumberLineInteraction",graphZoomNumberLineInteraction={getInstance:function(dom,config,state){var response=config.boundTo;this.initialize(Object.getOwnPropertyNames(response).pop(),dom,config.properties,config.assetManager),this.setSerializedState(state),"function"==typeof config.onready&&config.onready(this,this.getState())},getState:function(){return this.getSerializedState()},oncompleted:function(){this.destroy()},getResponse:function(){var value=this.getRawResponse();return value?{base:{float:value}}:{base:null}},destroy:function(){var $container=$(this.dom);$container.off().empty();
},getTypeIdentifier:function(){return _typeIdentifier},initialize:function(id,dom,config){function findRect(allRects,position){var ret,positions=_.pluck(allRects,"position"),stepWidth=positions[1]-positions[0];return _.each(allRects,function(rect){var pos=parseInt(rect.position);if(pos<position&&position<pos+stepWidth)return ret=rect,!1}),ret}function zoomOnRect(x){var rect=findRect(rects,x);if(rect){selectedRect&&selectedRect.rect.hide(),zoomEffect&&zoomEffect.remove(),selectedRect=rect,zoomAxis.addCssClass(selectedRect.rect,"scene scene-selected-rectangle"),selectedRect.rect.show(),zoomAxis.setConfig("labels",[_format(selectedRect.coord),_format(selectedRect.coord+subDivisionIncrement)]),
zoomAxis.render();var containerBox=zoomAxis.buildContainerBox({shadow:!0});zoomAxis.addCssClass(containerBox,"scene scene-zoom-popup");var left=getSelectedPointPositionLeft(selectedCoord);void 0!==left&&drawZoomAxisPoint(left),zoomAxis.clickable().click(function(e){drawZoomAxisPoint(e.layerX),updateAxisPoint()}),zoomEffect=axisZoom.drawZoomEffect(paper,selectedRect.rect,containerBox),zoomAxis.addCssClass(zoomEffect,"scene scene-zoom-effect")}}function drawAxisPoint(coord){axisPoint&&axisPoint.remove();var top=axis.getOriginPosition().top,pointConfig={radius:6,removable:!1,glow:!1};return pointConfig=_.defaults(pointConfig,_this.axisConfig.point),axisPoint=pointFactory(paper,axis,pointConfig),
axisPoint.setCartesianCoord(coord,top,pointConfig),axisPoint.render(),axisPoint.children.attr({cursor:"default"}),axis.getSet().push(axisPoint.children),axisPoint}function drawZoomAxisPoint(left){zoomPoint&&zoomPoint.remove();var zoomAxisConfig=zoomAxis.getConfig(),pointConfig={x:left,axis:"x",xMin:zoomAxisConfig.left,xMax:zoomAxisConfig.left+zoomAxisConfig.unitSize,removable:!1,on:{dragStop:updateAxisPoint}};return pointConfig=_.defaults(pointConfig,_this.axisConfig.point),zoomPoint=pointFactory(paper,zoomAxis,pointConfig),zoomPoint.render(),zoomPoint.drag(),zoomAxis.getSet().push(zoomPoint.children),zoomPoint}function getZoomPointCoordinate(){return selectedRect.coord+zoomPoint.getCartesianCoord().x/_this.axisConfig.unitSubDivision;
}function getSelectedPointPositionLeft(){if(selectedCoord){var zoomAxisConfig=zoomAxis.getConfig(),offset=zoomAxisConfig.unitSize*_this.axisConfig.unitSubDivision/(10*_this.axisConfig.unitSubDivision),left=(selectedCoord-selectedRect.coord)*zoomAxisConfig.unitSize*_this.axisConfig.unitSubDivision+offset;if(left=parseInt(_format(left)),zoomAxisConfig.left<=left+offset&&left-offset<=zoomAxisConfig.left+zoomAxisConfig.unitSize)return left}}function updateAxisPoint(){selectedCoord=getZoomPointCoordinate(),drawAxisPoint(selectedCoord),_.defer(function(){_this.trigger("responseChange")})}function initAxis($canvasContainer,axisConfig){var axisSizePx,zoomAxisConfig;subDivisionIncrement=1/axisConfig.unitSubDivision,
paper=createCanvas($canvasContainer,axisConfig),axis=new axisFactory(paper,axisConfig),axisSizePx=(axisConfig.max-axisConfig.min)*axisConfig.unitSize,zoomAxisConfig=_.defaults({min:0,max:1,top:axisConfig.top+150,unitSize:axisSizePx,unitSubDivision:10,arrows:!1,labels:["A","B"]},axisConfig),zoomAxis=new axisFactory(paper,zoomAxisConfig),zoomAxis.getSet().hide(),rects=axisZoom.createZoomable(axis),axis.clickable().click(function(e){var fx=e.layerX;zoomOnRect(fx)})}function reset(){selectedRect&&(selectedRect.rect.hide(),selectedRect=void 0),zoomEffect&&(zoomEffect.remove(),zoomEffect=void 0),zoomAxis&&zoomAxis.getSet().remove().clear(),axisPoint&&(axisPoint.remove(),
axisPoint=void 0),selectedCoord=void 0}function setAxis(axisConfig){_this.axisConfig=buildAxisConfig(_.merge(_this.config,_this.axisConfig,axisConfig)),initAxis($container,_this.axisConfig),reset()}event.addEventMgr(this),this.id=id,this.dom=dom,this.config=config||{};var paper,axis,zoomAxis,zoomEffect,selectedRect,selectedCoord,subDivisionIncrement,axisPoint,zoomPoint,rects,$container=$(dom),_this=this;this.reset=function(){reset()},this.getRawResponse=function(){return selectedCoord},this.setRawResponse=function(selected){var axisSelectedX,zoomAxisSelectedX;selectedCoord=selected,axisSelectedX=axis.coordinateToPosition(selected).left+1,zoomOnRect(axisSelectedX),
zoomAxisSelectedX=getSelectedPointPositionLeft(),drawZoomAxisPoint(zoomAxisSelectedX),updateAxisPoint()},this.axisConfig=buildAxisConfig(this.config),initAxis($container,this.axisConfig),this.on("axischange",setAxis)},setResponse:function(response){response&&response.base&&response.base.float&&this.setRawResponse(response.base.float)},resetResponse:function(){this.reset()},setSerializedState:function(state){this.setResponse(state)},getSerializedState:function(){return this.getResponse()}};qtiCustomInteractionContext.register(graphZoomNumberLineInteraction)}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",
d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))}("/*add some style, need to scope (using the typeIdentifier as class prefix) every style declaration to prevent collision */\n\n.graphZoomNumberLineInteraction {\n    width:100%;\n    padding:30px 30px 50px;\n    margin:20px 0;\n    padding:20px;\n    border : 6px solid #f5f3f2;\n}\n\n.graphZoomNumberLineInteraction .shape-controls .clearfix {\n    overflow: auto;\n}\n\n.graphZoomNumberLineInteraction .shape-controls .intervals-available {float:left; position : relative; border : 6px solid #f5f3f2; padding:4px; margin-right : 10px; margin-bottom:10px;}\n.graphZoomNumberLineInteraction .shape-controls .intervals-available .intervals-overlay {position:absolute; width:100%; height : 100%; top:0; left:0; background: #ddd; opacity : 0.6; display:none;}\n.graphZoomNumberLineInteraction .shape-controls .intervals-available .interval-available {display:none;}\n.graphZoomNumberLineInteraction .shape-controls .intervals-available .interval-available.activated {display:inline-block;}\n\n.graphZoomNumberLineInteraction .shape-controls .intervals-selected {float:left;}\n.graphZoomNumberLineInteraction .shape-controls .intervals-selected .interval {float:left; position : relative; width:92px; border : 6px solid #f5f3f2; padding:4px; margin-right:10px;}\n.graphZoomNumberLineInteraction .shape-controls .intervals-selected .interval .deleter {cursor: pointer; }\n.graphZoomNumberLineInteraction .shape-controls .intervals-selected .interval .deleter:before {font-size:1.25rem; margin-right: 3px;}\n\n.graphZoomNumberLineInteraction .shape-controls .intervals-selected .interval .btn {margin-bottom:10px;}\n\n.graphZoomNumberLineInteraction .shape-controls button {padding: 0 6px 8px; height:36px; width:72px; margin-right:2px;}\n.graphZoomNumberLineInteraction .shape-controls button img {height:20px;width:50px;}\n\n.graphZoomNumberLineInteraction .shape-controls .intervals-template {display:none;}\n\n.graphZoomNumberLineInteraction .shape-controls .intervals-row {margin-bottom: 2px;}\n"),
define(["graphZoomNumberLineInteraction/runtime/graphZoomNumberLineInteraction"],function(IMSPCI){return IMSPCI});
//# sourceMappingURL=graphZoomNumberLineInteraction.min.js.map