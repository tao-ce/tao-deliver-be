...
var item,items,k,i,ii,j,jj,nes,names=name.split(separator),e=events,es=[e],out=[];for(i=0,ii=names.length;i<ii;i++){for(nes=[],j=0,jj=es.length;j<jj;j++)for(e=es[j].n,items=[e[names[i]],e[wildcard]],k=2;k--;)item=items[k],item&&(nes.push(item),out=out.concat(item.f||[]));es=nes}return out},eve.on=function(name,f){if(name=String(name),"function"!=typeof f)return function(){};for(var names=name.split(separator),e=events,i=0,ii=names.length;i<ii;i++)e=e.n,e=e.hasOwnProperty(names[i])&&e[names[i]]||(e[names[i]]={n:{}});for(e.f=e.f||[],i=0,ii=e.f.length;i<ii;i++)if(e.f[i]==f)return fun;return e.f.push(f),function(zIndex){+zIndex==+zIndex&&(f.zIndex=+zIndex)}},eve.f=function(event){
...
x:px,y:py}}}}function interHelper(bez1,bez2,justCount){var bbox1=R.bezierBBox(bez1),bbox2=R.bezierBBox(bez2);if(!R.isBBoxIntersect(bbox1,bbox2))return justCount?0:[];for(var l1=bezlen.apply(0,bez1),l2=bezlen.apply(0,bez2),n1=mmax(~~(l1/5),1),n2=mmax(~~(l2/5),1),dots1=[],dots2=[],xy={},res=justCount?0:[],i=0;i<n1+1;i++){var p=R.findDotsAtSegment.apply(R,bez1.concat(i/n1));dots1.push({x:p.x,y:p.y,t:i/n1})}for(i=0;i<n2+1;i++)p=R.findDotsAtSegment.apply(R,bez2.concat(i/n2)),dots2.push({x:p.x,y:p.y,t:i/n2});for(i=0;i<n1;i++)for(var j=0;j<n2;j++){var di=dots1[i],di1=dots1[i+1],dj=dots2[j],dj1=dots2[j+1],ci=abs(di1.x-di.x)<.001?"y":"x",cj=abs(dj1.x-dj.x)<.001?"y":"x",is=intersect(di.x,di.y,di1.x,di1.y,dj.x,dj.y,dj1.x,dj1.y);
if(is){if(xy[is.x.toFixed(4)]==is.y.toFixed(4))continue;xy[is.x.toFixed(4)]=is.y.toFixed(4);var t1=di.t+abs((is[ci]-di[ci])/(di1[ci]-di[ci]))*(di1.t-di.t),t2=dj.t+abs((is[cj]-dj[cj])/(dj1[cj]-dj[cj]))*(dj1.t-dj.t);t1>=0&&t1<=1.001&&t2>=0&&t2<=1.001&&(justCount?res++:res.push({x:is.x,y:is.y,t1:mmin(t1,1),t2:mmin(t2,1)}))}}return res}function interPathHelper(path1,path2,justCount){path1=R._path2curve(path1),path2=R._path2curve(path2);for(var x1,y1,x2,y2,x1m,y1m,x2m,y2m,bez1,bez2,res=justCount?0:[],i=0,ii=path1.length;i<ii;i++){var pi=path1[i];if("M"==pi[0])x1=x1m=pi[1],y1=y1m=pi[2];else{"C"==pi[0]?(bez1=[x1,y1].concat(pi.slice(1)),x1=bez1[6],y1=bez1[7]):(bez1=[x1,y1,x1,y1,x1m,y1m,x1m,y1m],
x1=x1m,y1=y1m);for(var j=0,jj=path2.length;j<jj;j++){var pj=path2[j];if("M"==pj[0])x2=x2m=pj[1],y2=y2m=pj[2];else{"C"==pj[0]?(bez2=[x2,y2].concat(pj.slice(1)),x2=bez2[6],y2=bez2[7]):(bez2=[x2,y2,x2,y2,x2m,y2m,x2m,y2m],x2=x2m,y2=y2m);var intr=interHelper(bez1,bez2,justCount);if(justCount)res+=intr;else{for(var k=0,kk=intr.length;k<kk;k++)intr[k].segment1=i,intr[k].segment2=j,intr[k].bez1=bez1,intr[k].bez2=bez2;res=res.concat(intr)}}}}}return res}function Matrix(a,b,c,d,e,f){null!=a?(this.a=+a,this.b=+b,this.c=+c,this.d=+d,this.e=+e,this.f=+f):(this.a=1,this.b=0,this.c=0,this.d=1,this.e=0,this.f=0)}function x_y_w_h(){return this.x+S+this.y+S+this.width+" × "+this.height;
...
var percents=[],newAnim={};if(this.ms=ms,this.times=1,anim){for(var attr in anim)anim[has](attr)&&(newAnim[toFloat(attr)]=anim[attr],percents.push(toFloat(attr)));percents.sort(sortByNumber)}this.anim=newAnim,this.top=percents[percents.length-1],this.percents=percents}function runAnimation(anim,element,percent,status,totalOrigin,times){percent=toFloat(percent);var params,isInAnim,isInAnimSet,next,prev,timestamp,ms=anim.ms,from={},to={},diff={};if(status)for(i=0,ii=animationElements.length;i<ii;i++){var e=animationElements[i];if(e.el.id==element.id&&e.anim==anim){e.percent!=percent?(animationElements.splice(i,1),isInAnimSet=1):isInAnim=e,element.attr(e.totalOrigin);
break}}else status=+to;for(var i=0,ii=anim.percents.length;i<ii;i++){if(anim.percents[i]==percent||anim.percents[i]>status*anim.top){percent=anim.percents[i],prev=anim.percents[i-1]||0,ms=ms/anim.top*(percent-prev),next=anim.percents[i+1],params=anim.anim[percent];break}status&&element.attr(anim.anim[anim.percents[i]])}if(params){if(isInAnim)isInAnim.initstatus=status,isInAnim.start=new Date-isInAnim.ms*status;else{for(var attr in params)if(params[has](attr)&&(availableAnimAttrs[has](attr)||element.paper.customAttributes[has](attr)))switch(from[attr]=element.attr(attr),null==from[attr]&&(from[attr]=availableAttrs[attr]),to[attr]=params[attr],availableAnimAttrs[attr]){
...
return ellipsePath(a.cx,a.cy,a.rx,a.ry)},rect:function(el){var a=el.attrs;return rectPath(a.x,a.y,a.width,a.height,a.r)},image:function(el){var a=el.attrs;return rectPath(a.x,a.y,a.width,a.height)},text:function(el){var bbox=el._getBBox();return rectPath(bbox.x,bbox.y,bbox.width,bbox.height)},set:function(el){var bbox=el._getBBox();return rectPath(bbox.x,bbox.y,bbox.width,bbox.height)}},mapPath=R.mapPath=function(path,matrix){if(!matrix)return path;var x,y,i,j,ii,jj,pathi;for(path=path2curve(path),i=0,ii=path.length;i<ii;i++)for(pathi=path[i],j=1,jj=pathi.length;j<jj;j+=2)x=matrix.x(pathi[j],pathi[j+1]),y=matrix.y(pathi[j],pathi[j+1]),pathi[j]=x,pathi[j+1]=y;return path;
...
rgb=R.rgb2hsb(clr),clr.v=rgb.b):(clr={hex:"none"},clr.r=clr.g=clr.b=clr.h=clr.s=clr.v=clr.l=-1)),clr.toString=rgbtoString,clr},R.hsb2rgb=function(h,s,v,o){this.is(h,"object")&&"h"in h&&"s"in h&&"b"in h&&(v=h.b,s=h.s,h=h.h,o=h.o),h*=360;var R,G,B,X,C;return h=h%360/60,C=v*s,X=C*(1-abs(h%2-1)),R=G=B=v-C,h=~~h,R+=[C,X,0,0,X,C][h],G+=[X,C,C,X,0,0][h],B+=[0,0,X,C,C,X][h],packageRGB(R,G,B,o)},R.hsl2rgb=function(h,s,l,o){this.is(h,"object")&&"h"in h&&"s"in h&&"l"in h&&(l=h.l,s=h.s,h=h.h),(h>1||s>1||l>1)&&(h/=360,s/=100,l/=100),h*=360;var R,G,B,X,C;return h=h%360/60,C=2*s*(l<.5?l:1-l),X=C*(1-abs(h%2-1)),R=G=B=l-C/2,h=~~h,R+=[C,X,0,0,X,C][h],G+=[X,C,C,X,0,0][h],B+=[0,0,X,C,C,X][h],
packageRGB(R,G,B,o)},R.rgb2hsb=function(r,g,b){b=prepareRGB(r,g,b),r=b[0],g=b[1],b=b[2];var H,S,V,C;return V=mmax(r,g,b),C=V-mmin(r,g,b),H=0==C?null:V==r?(g-b)/C:V==g?(b-r)/C+2:(r-g)/C+4,H=(H+360)%6*60/360,S=0==C?0:C/V,{h:H,s:S,b:V,toString:hsbtoString}},R.rgb2hsl=function(r,g,b){b=prepareRGB(r,g,b),r=b[0],g=b[1],b=b[2];var H,S,L,M,m,C;return M=mmax(r,g,b),m=mmin(r,g,b),C=M-m,H=0==C?null:M==r?(g-b)/C:M==g?(b-r)/C+2:(r-g)/C+4,H=(H+360)%6*60/360,L=(M+m)/2,S=0==C?0:L<.5?C/(2*L):C/(2-2*L),{h:H,s:S,l:L,toString:hsltoString}},R._path2string=function(){return this.join(",").replace(p2s,"$1")};R._preload=function(src,f){var img=g.doc.createElement("img");img.style.cssText="position:absolute;left:-9999em;top:-9999em",
...
f1=x1<cx?PI-f1:f1,f2=x2<cx?PI-f2:f2,f1<0&&(f1=2*PI+f1),f2<0&&(f2=2*PI+f2),sweep_flag&&f1>f2&&(f1-=2*PI),!sweep_flag&&f2>f1&&(f2-=2*PI)}var df=f2-f1;if(abs(df)>_120){var f2old=f2,x2old=x2,y2old=y2;f2=f1+_120*(sweep_flag&&f2>f1?1:-1),x2=cx+rx*math.cos(f2),y2=cy+ry*math.sin(f2),res=a2c(x2,y2,rx,ry,angle,0,sweep_flag,x2old,y2old,[f2,f2old,cx,cy])}df=f2-f1;var c1=math.cos(f1),s1=math.sin(f1),c2=math.cos(f2),s2=math.sin(f2),t=math.tan(df/4),hx=4/3*rx*t,hy=4/3*ry*t,m1=[x1,y1],m2=[x1+hx*s1,y1-hy*c1],m3=[x2+hx*s2,y2-hy*c2],m4=[x2,y2];if(m2[0]=2*m1[0]-m2[0],m2[1]=2*m1[1]-m2[1],recursive)return[m2,m3,m4][concat](res);res=[m2,m3,m4][concat](res).join()[split](",");for(var newres=[],i=0,ii=res.length;i<ii;i++)newres[i]=i%2?rotate(res[i-1],res[i],rad).y:rotate(res[i],res[i+1],rad).x;
return newres},findDotAtSegment=function(p1x,p1y,c1x,c1y,c2x,c2y,p2x,p2y,t){var t1=1-t;return{x:pow(t1,3)*p1x+3*pow(t1,2)*t*c1x+3*t1*t*t*c2x+pow(t,3)*p2x,y:pow(t1,3)*p1y+3*pow(t1,2)*t*c1y+3*t1*t*t*c2y+pow(t,3)*p2y}},curveDim=cacher(function(p1x,p1y,c1x,c1y,c2x,c2y,p2x,p2y){var dot,a=c2x-2*c1x+p1x-(p2x-2*c2x+c1x),b=2*(c1x-p1x)-2*(c2x-c1x),c=p1x-c1x,t1=(-b+math.sqrt(b*b-4*a*c))/2/a,t2=(-b-math.sqrt(b*b-4*a*c))/2/a,y=[p1y,p2y],x=[p1x,p2x];return abs(t1)>"1e12"&&(t1=.5),abs(t2)>"1e12"&&(t2=.5),t1>0&&t1<1&&(dot=findDotAtSegment(p1x,p1y,c1x,c1y,c2x,c2y,p2x,p2y,t1),x.push(dot.x),y.push(dot.y)),t2>0&&t2<1&&(dot=findDotAtSegment(p1x,p1y,c1x,c1y,c2x,c2y,p2x,p2y,t2),x.push(dot.x),
y.push(dot.y)),a=c2y-2*c1y+p1y-(p2y-2*c2y+c1y),b=2*(c1y-p1y)-2*(c2y-c1y),c=p1y-c1y,t1=(-b+math.sqrt(b*b-4*a*c))/2/a,t2=(-b-math.sqrt(b*b-4*a*c))/2/a,abs(t1)>"1e12"&&(t1=.5),abs(t2)>"1e12"&&(t2=.5),t1>0&&t1<1&&(dot=findDotAtSegment(p1x,p1y,c1x,c1y,c2x,c2y,p2x,p2y,t1),x.push(dot.x),y.push(dot.y)),t2>0&&t2<1&&(dot=findDotAtSegment(p1x,p1y,c1x,c1y,c2x,c2y,p2x,p2y,t2),x.push(dot.x),y.push(dot.y)),{min:{x:mmin[apply](0,x),y:mmin[apply](0,y)},max:{x:mmax[apply](0,x),y:mmax[apply](0,y)}}}),path2curve=R._path2curve=cacher(function(path,path2){var pth=!path2&&paths(path);if(!path2&&pth.curve)return pathClone(pth.curve);for(var p=pathToAbsolute(path),p2=path2&&pathToAbsolute(path2),attrs={
...
},getBBox:function(){return bb}};return extractTransform(el,transform),el.matrix}),extractTransform=(R.transformPath=function(path,transform){return mapPath(path,toMatrix(path,transform))},R._extractTransform=function(el,tstr){if(null==tstr)return el._.transform;tstr=Str(tstr).replace(/\.{3}|\u2026/g,el._.transform||E);var tdata=R.parseTransformString(tstr),deg=0,dx=0,dy=0,sx=1,sy=1,_=el._,m=new Matrix;if(_.transform=tdata||[],tdata)for(var i=0,ii=tdata.length;i<ii;i++){var x1,y1,x2,y2,bb,t=tdata[i],tlen=t.length,command=Str(t[0]).toLowerCase(),absolute=t[0]!=command,inver=absolute?m.invert():0;"t"==command&&3==tlen?absolute?(x1=inver.x(0,0),y1=inver.y(0,0),x2=inver.x(t[1],t[2]),
...
var pos=getEventPosition(e);return fn.call(element,e,pos.x,pos.y)};if(obj.addEventListener(type,f,!1),supportsTouch&&touchMap[type]){var _f=function(e){for(var pos=getEventPosition(e),olde=e,i=0,ii=e.targetTouches&&e.targetTouches.length;i<ii;i++)if(e.targetTouches[i].target==obj){e=e.targetTouches[i],e.originalEvent=olde,e.preventDefault=preventTouch,e.stopPropagation=stopTouch;break}return fn.call(element,e,pos.x,pos.y)};obj.addEventListener(touchMap[type],_f,!1)}return function(){return obj.removeEventListener(type,f,!1),supportsTouch&&touchMap[type]&&obj.removeEventListener(touchMap[type],f,!1),!0}}:g.doc.attachEvent?function(obj,type,fn,element){var f=function(e){
...
glow=glow||{};var s={width:(glow.width||10)+(+this.attr("stroke-width")||1),fill:glow.fill||!1,opacity:glow.opacity||.5,offsetx:glow.offsetx||0,offsety:glow.offsety||0,color:glow.color||"#000"},c=s.width/2,r=this.paper,out=r.set(),path=this.realPath||getPath[this.type](this);path=this.matrix?mapPath(path,this.matrix):path;for(var i=1;i<c+1;i++)out.push(r.path(path).attr({stroke:s.color,fill:s.fill?s.color:"none","stroke-linejoin":"round","stroke-linecap":"round","stroke-width":+(s.width/c*i).toFixed(3),opacity:+(s.opacity/c).toFixed(3)}));return out.insertBefore(this).translate(s.offsetx,s.offsety)};var getPointAtSegmentLength=function(p1x,p1y,c1x,c1y,c2x,c2y,p2x,p2y,length){
...
e.status=e.initstatus,delete e.initstatus,e.stop&&animationElements.splice(l--,1)):e.status=(e.prev+(e.percent-e.prev)*(time/ms))/e.anim.top,!(time<0))if(time<ms){var pos=easing(time/ms);for(var attr in from)if(from[has](attr)){switch(availableAnimAttrs[attr]){case nu:now=+from[attr]+pos*ms*diff[attr];break;case"colour":now="rgb("+[upto255(round(from[attr].r+pos*ms*diff[attr].r)),upto255(round(from[attr].g+pos*ms*diff[attr].g)),upto255(round(from[attr].b+pos*ms*diff[attr].b))].join(",")+")";break;case"path":now=[];for(var i=0,ii=from[attr].length;i<ii;i++){now[i]=[from[attr][i][0]];for(var j=1,jj=from[attr][i].length;j<jj;j++)now[i][j]=+from[attr][i][j]+pos*ms*diff[attr][i][j];
now[i]=now[i].join(S)}now=now.join(S);break;case"transform":if(diff[attr].real)for(now=[],i=0,ii=from[attr].length;i<ii;i++)for(now[i]=[from[attr][i][0]],j=1,jj=from[attr][i].length;j<jj;j++)now[i][j]=from[attr][i][j]+pos*ms*diff[attr][i][j];else{var get=function(i){return+from[attr][i]+pos*ms*diff[attr][i]};now=[["m",get(0),get(1),get(2),get(3),get(4),get(5)]]}break;case"csv":if("clip-rect"==attr)for(now=[],i=4;i--;)now[i]=+from[attr][i]+pos*ms*diff[attr][i];break;default:var from2=[][concat](from[attr]);for(now=[],i=that.paper.customAttributes[attr].length;i--;)now[i]=+from2[i]+pos*ms*diff[attr][i]}set[attr]=now}that.attr(set),function(id,that,anim){setTimeout(function(){
...
return color>255?255:color<0?0:color};elproto.animateWith=function(el,anim,params,ms,easing,callback){var element=this;if(element.removed)return callback&&callback.call(element),element;var a=params instanceof Animation?params:R.animation(params,ms,easing,callback);runAnimation(a,element,a.percents[0],null,element.attr());for(var i=0,ii=animationElements.length;i<ii;i++)if(animationElements[i].anim==anim&&animationElements[i].el==el){animationElements[ii-1].start=animationElements[i].start;break}return element},elproto.onAnimation=function(f){return f?eve.on("raphael.anim.frame."+this.id,f):eve.unbind("raphael.anim.frame."+this.id),this},Animation.prototype.delay=function(delay){
...
},elproto.animate=function(params,ms,easing,callback){var element=this;if(element.removed)return callback&&callback.call(element),element;var anim=params instanceof Animation?params:R.animation(params,ms,easing,callback);return runAnimation(anim,element,anim.percents[0],null,element.attr()),element},elproto.setTime=function(anim,value){return anim&&null!=value&&this.status(anim,mmin(value,anim.ms)/anim.ms),this},elproto.status=function(anim,value){var len,e,out=[],i=0;if(null!=value)return runAnimation(anim,this,-1,mmin(value,1)),this;for(len=animationElements.length;i<len;i++)if(e=animationElements[i],e.el.id==this.id&&(!anim||e.anim==anim)){if(anim)return e.status;
...
return this},eve.on("raphael.remove",stopAnimation),eve.on("raphael.clear",stopAnimation),elproto.toString=function(){return"Raphaël’s object"};var Set=function(items){if(this.items=[],this.length=0,this.type="set",items)for(var i=0,ii=items.length;i<ii;i++)!items[i]||items[i].constructor!=elproto.constructor&&items[i].constructor!=Set||(this[this.items.length]=this.items[this.items.length]=items[i],this.length++)},setproto=Set.prototype;setproto.push=function(){for(var item,len,i=0,ii=arguments.length;i<ii;i++)item=arguments[i],!item||item.constructor!=elproto.constructor&&item.constructor!=Set||(len=this.items.length,this[len]=this.items[len]=item,this.length++);
return this},setproto.pop=function(){return this.length&&delete this[this.length--],this.items.pop()},setproto.forEach=function(callback,thisArg){for(var i=0,ii=this.items.length;i<ii;i++)if(callback.call(thisArg,this.items[i],i)===!1)return this;return this};for(var method in elproto)elproto[has](method)&&(setproto[method]=function(methodname){return function(){var arg=arguments;return this.forEach(function(el){el[methodname][apply](el,arg)})}}(method));return setproto.attr=function(name,value){if(name&&R.is(name,array)&&R.is(name[0],"object"))for(var j=0,jj=name.length;j<jj;j++)this.items[j].attr(name[j]);else for(var i=0,ii=this.items.length;i<ii;i++)this.items[i].attr(name,value);
return this},setproto.clear=function(){for(;this.length;)this.pop()},setproto.splice=function(index,count,insertion){index=index<0?mmax(this.length+index,0):index,count=mmax(0,mmin(this.length-index,count));var i,tail=[],todel=[],args=[];for(i=2;i<arguments.length;i++)args.push(arguments[i]);for(i=0;i<count;i++)todel.push(this[index+i]);for(;i<this.length-index;i++)tail.push(this[index+i]);var arglen=args.length;for(i=0;i<arglen+tail.length;i++)this.items[index+i]=this[index+i]=i<arglen?args[i]:tail[i-arglen];for(i=this.items.length=this.length-=count-arglen;this[i];)delete this[i++];return new Set(todel)},setproto.exclude=function(el){for(var i=0,ii=this.length;i<ii;i++)if(this[i]==el)return this.splice(i,1),
...
vector[2]<0&&(vector[0]=-vector[2],vector[2]=0),vector[3]<0&&(vector[1]=-vector[3],vector[3]=0)}var dots=R._parseDots(gradient);if(!dots)return null;if(id=id.replace(/[\(\)\s,\xb0#]/g,"_"),element.gradient&&id!=element.gradient.id&&(SVG.defs.removeChild(element.gradient),delete element.gradient),!element.gradient){el=$(type+"Gradient",{id:id}),element.gradient=el,$(el,"radial"==type?{fx:fx,fy:fy}:{x1:vector[0],y1:vector[1],x2:vector[2],y2:vector[3],gradientTransform:element.matrix.invert()}),SVG.defs.appendChild(el);for(var i=0,ii=dots.length;i<ii;i++)el.appendChild($("stop",{offset:dots[i].offset?dots[i].offset:i?"100%":"0%","stop-color":dots[i].color||"#fff"
...
if(params[has]("text")){for(a.text=params.text;node.firstChild;)node.removeChild(node.firstChild);for(var tspan,texts=Str(params.text).split("\n"),tspans=[],i=0,ii=texts.length;i<ii;i++)tspan=$("tspan"),i&&$(tspan,{dy:fontSize*leading,x:a.x}),tspan.appendChild(R._g.doc.createTextNode(texts[i])),node.appendChild(tspan),tspans[i]=tspan}else for(tspans=node.getElementsByTagName("tspan"),i=0,ii=tspans.length;i<ii;i++)i?$(tspans[i],{dy:fontSize*leading,x:a.x}):$(tspans[0],{dy:0});$(node,{x:a.x,y:a.y}),el._.dirty=1;var bb=el._getBBox(),dif=a.y-(bb.y+bb.height/2);dif&&R.is(dif,"finite")&&$(tspans[0],{dy:dif})}},Element=function(node,svg){this[0]=this.node=node,node.raphael=!0,
...
var H,W,width=this.width,height=this.height,size=1/mmax(w/width,h/height);return fit&&(H=height/h,W=width/w,w*H<width&&(x-=(width-w*H)/2/H),h*W<height&&(y-=(height-h*W)/2/W)),this._viewBox=[x,y,w,h,!!fit],this._viewBoxShift={dx:-x,dy:-y,scale:size},this.forEach(function(el){el.transform("...")}),this};var createNode;R._engine.initWin=function(win){var doc=win.document;doc.createStyleSheet().addRule(".rvml","behavior:url(#default#VML)");try{!doc.namespaces.rvml&&doc.namespaces.add("rvml","urn:schemas-microsoft-com:vml"),createNode=function(tagName){return doc.createElement("<rvml:"+tagName+' class="rvml">')}}catch(e){createNode=function(tagName){return doc.createElement("<"+tagName+' xmlns="urn:schemas-microsoft.com:vml" class="rvml">');
...
},R.prototype.remove=function(){R.eve("raphael.remove",this),this.canvas.parentNode.removeChild(this.canvas);for(var i in this)this[i]="function"==typeof this[i]?R._removedFactory(i):null;return!0};var setproto=R.st;for(var method in elproto)elproto[has](method)&&!setproto[has](method)&&(setproto[method]=function(methodname){return function(){var arg=arguments;return this.forEach(function(el){el[methodname].apply(el,arg)})}}(method))}}(),"object"==typeof exports&&(module.exports=R),R}),define("taoQtiItem/portableLib/OAT/scale.raphael",["taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/raphael"],function($,Raphael){var scaleRaphael=function(container,width,height){
var wrapper=container.style?container:document.getElementById(container);wrapper.style.position||(wrapper.style.position="relative"),wrapper.style.width=width+"px",wrapper.style.height=height+"px";var nestedWrapper;"VML"===Raphael.type?(wrapper.innerHTML="<rvml:group style='position : absolute; width: 1000px; height: 1000px; top: 0px; left: 0px' coordsize='1000,1000' class='rvml'></rvml:group>",nestedWrapper=$(".rvml",$(wrapper))[0]):(wrapper.innerHTML="<div class='svggroup'></div>",nestedWrapper=$(".svggroup",$(wrapper))[0]);var vmlDiv,paper=new Raphael(nestedWrapper,width,height);return"SVG"===Raphael.type?paper.canvas.setAttribute("viewBox","0 0 "+width+" "+height):vmlDiv=wrapper.getElementsByTagName("div")[0],
paper.changeSize=function(w,h,center,clipping){clipping=!clipping;var ratioW=w/width,ratioH=h/height,scale=ratioW<ratioH?ratioW:ratioH,newHeight=parseInt(height*scale),newWidth=parseInt(width*scale);if("VML"===Raphael.type){var txt=document.getElementsByTagName("textpath");for(var i in txt){var curr=txt[i];if(curr.style){if(!curr._fontSize){var mod=curr.style.font.split("px");curr._fontSize=parseInt(mod[0]),curr._font=mod[1]}curr.style.font=curr._fontSize*scale+"px"+curr._font}}var newSize;newSize=newWidth<newHeight?1e3*newWidth/width:1e3*newHeight/height,newSize=parseInt(newSize),nestedWrapper.style.width=newSize+"px",nestedWrapper.style.height=newSize+"px",
clipping&&(nestedWrapper.style.left=parseInt((w-newWidth)/2)+"px",nestedWrapper.style.top=parseInt((h-newHeight)/2)+"px"),vmlDiv.style.overflow="visible"}clipping&&(newWidth=w,newHeight=h),wrapper.style.width=newWidth+"px",wrapper.style.height=newHeight+"px",paper.setSize(newWidth,newHeight),center&&(wrapper.style.position="absolute",wrapper.style.left=parseInt((w-newWidth)/2)+"px",wrapper.style.top=parseInt((h-newHeight)/2)+"px")},paper.scaleAll=function(amount){paper.changeSize(width*amount,height*amount)},paper.changeSize(width,height),paper.w=width,paper.h=height,paper};return scaleRaphael}),define("parccTei/portableLib/gridFactory",["taoQtiItem/portableLib/lodash"],function(_){
"use strict";function gridFactory(paper,rawOptions){function _getAxisTitleCoords(){var axisTitleCoords={abs:{},ord:{}};return axisTitleCoords.abs.x=_width/2,axisTitleCoords.abs.angle=0,"bottom"===_axisTitlePositions.abs?axisTitleCoords.abs.y=_height+options.axisTitlePadding:"top"===_axisTitlePositions.abs&&(axisTitleCoords.abs.y=-options.axisTitlePadding),axisTitleCoords.ord.y=_height/2,axisTitleCoords.ord.angle=-90,"left"===_axisTitlePositions.ord?axisTitleCoords.ord.x=-options.axisTitlePadding:"right"===_axisTitlePositions.ord&&(axisTitleCoords.ord.x=_width+options.padding),axisTitleCoords}function _getLabelsCoords(){var labelCoords={abs:{},ord:{}};return labelCoords.abs.x=_width+options.labelPadding,
labelCoords.abs.angle=0,_y.start<0&&_y.end>0?labelCoords.abs.y=-1*_y.start*_y.unit:labelCoords.abs.y=_y.start>=0?0:_height,labelCoords.ord.y=-options.labelPadding,labelCoords.ord.angle=0,_x.start<0&&_x.end>0?labelCoords.ord.x=-1*_x.start*_x.unit:labelCoords.ord.x=_x.start>=0?0:_width,labelCoords}function _getSnapToValues(){var snapValue,i,j,snapToValues={x:[],y:[]},xStepSize=_width/(_xRange/_x.step),yStepSize=_height/(_yRange/_y.step);for(i=0;i<=_width;i+=xStepSize)for(j=0;j<xStepSize;j+=_xSubStepSize)snapValue=i+j,snapValue<=_width&&snapToValues.x.push(snapValue+_padding.left);for(i=0;i<=_height;i+=yStepSize)for(j=0;j<yStepSize;j+=_ySubStepSize)snapValue=i+j,
snapValue<=_height&&snapToValues.y.push(snapValue+_padding.top);return snapToValues}function _drawGraphTitle(){var x=_padding.left+_width/2,y=options.padding,style={"font-size":options.graphTitleSize,"font-weight":"bold"};options.graphTitle&&options.graphTitleRequired===!0&&_drawTitle(options.graphTitle,style,x,y)}function _drawAxis(){function drawXaxis(top,config){config=config||{};var textTop,text,i,line=_drawLine([0,top],[_width,top],config.style),readabilityOffset=config.multiQuadrant?5:0,position=readabilityOffset,fontSize=10;for(textTop=config.unitsOnTop?top+_padding.top-fontSize:top+_padding.top+fontSize,i=_x.start;i<=_x.end;i+=_x.step)text=paper.text(_padding.left+position,textTop,i).attr({
"font-size":fontSize}),_addCssClass(text,"scene scene-text"),position+=_x.unit*_x.step;return line}function drawYaxis(left,config){config=config||{};var textLeft,text,i,line=_drawLine([left,_height],[left,0],config.style),readabilityOffset=config.multiQuadrant?-5:0,position=readabilityOffset,fontSize=10;for(textLeft=config.unitsOnRight?left+_padding.left+fontSize/2+2:left+_padding.left-fontSize-2,i=_y.start;i<=_y.end;i+=_y.step)text=paper.text(textLeft,_padding.top+position,-i).attr({"font-size":fontSize}),_addCssClass(text,"scene scene-text"),position+=_y.unit*_y.step;return line}var xStyle={stroke:_x.color,"stroke-width":_x.weight},yStyle={stroke:_y.color,"stroke-width":_y.weight
},axisTitleStyle={"font-size":options.axisTitleSize,"font-weight":"bold"},labelStyle={"font-size":options.labelSize};_y.start<0&&_y.end<=0?drawXaxis(_height,{style:xStyle}):_y.start>=0&&_y.end>0?drawXaxis(0,{style:xStyle,unitsOnTop:!0}):drawXaxis(Math.abs(_y.start)*_y.unit,{style:xStyle,multiQuadrant:!0}),_x.start<0&&_x.end<=0?drawYaxis(_width,{style:yStyle,unitsOnRight:!0}):_x.start>=0&&_x.end>0?drawYaxis(0,{style:yStyle}):drawYaxis(Math.abs(_x.start)*_x.unit,{style:yStyle,multiQuadrant:!0}),_x.label&&_drawTitle(_x.label,labelStyle,_padding.left+_labelCoords.abs.x,_padding.top+_labelCoords.abs.y,_labelCoords.abs.angle),_y.label&&_drawTitle(_y.label,labelStyle,_padding.left+_labelCoords.ord.x,_padding.top+_labelCoords.ord.y,_labelCoords.ord.angle),
_x.title&&_drawTitle(_x.title,axisTitleStyle,_padding.left+_axisTitleCoords.abs.x,_padding.top+_axisTitleCoords.abs.y,_axisTitleCoords.abs.angle),_y.title&&_drawTitle(_y.title,axisTitleStyle,_padding.left+_axisTitleCoords.ord.x,_padding.top+_axisTitleCoords.ord.y,_axisTitleCoords.ord.angle)}function _drawGrid(){var x,y,style={stroke:_color,"stroke-width":_weight};for(y=0;y<=_height;y+=_y.step*_y.unit)_drawLine([0,y],[_width,y],style);for(Math.abs(_y.end-_y.start)%_y.step&&_drawLine([0,_height],[_width,_height],style),x=0;x<=_width;x+=_x.step*_x.unit)_drawLine([x,0],[x,_height],style);Math.abs(_x.end-_x.start)%_x.step&&_drawLine([_width,0],[_width,_height],style);
}function _drawLine(start,end,style){var path=paper.path("M"+(_padding.left+start[0])+" "+(_padding.top+start[1])+"L"+(_padding.left+end[0])+" "+(_padding.top+end[1])).attr(style);return _addCssClass(path,"scene scene-grid"),path}function _drawTitle(text,style,x,y,angle){var textElement=paper.text(x,y,text).attr(style);angle&&textElement.rotate(angle,x,y)}function _addCssClass(raphaelObj,newClass){var pattern=new RegExp("\\b"+newClass+"\\b"),oldClass=raphaelObj.node.getAttribute("class")||"";raphaelObj.node.setAttribute("class",pattern.test(oldClass)?oldClass:oldClass+" "+newClass)}function _calculateBBox(){var x=_padding.left,y=_padding.top;_borderBox={x:x,y:y,
width:_width,height:_height,x2:x+_width,y2:y+_height}}if("object"!=typeof rawOptions.x&&"object"!=typeof rawOptions.y)throw new Error("I need x and y axis");if(rawOptions.x.start>=rawOptions.x.end||rawOptions.y.start>=rawOptions.y.end)throw new Error("end must be greater than start");var clickableArea,options=_buildOptions(rawOptions),_x=options.x,_y=options.y,_xRange=Math.abs(_x.end-_x.start),_yRange=Math.abs(_y.end-_y.start),_width=_xRange*_x.unit,_height=_yRange*_y.unit,_xSubStepSize=_width/(_xRange/_x.step)/_x.subStep,_ySubStepSize=_height/(_yRange/_y.step)/_y.subStep,_color=options.color,_weight=options.weight,_axisTitlePositions=_getAxisTitlePositions(options),_padding=_getPadding(options,_axisTitlePositions),_axisTitleCoords=_getAxisTitleCoords(),_labelCoords=_getLabelsCoords(),_snapToValues=_getSnapToValues(),set=paper.set(),_borderBox={},obj={
children:set,snapping:options.snapping||!1,setColor:function(color){_color=String(color),set.remove().clear(),this.render()},setWeight:function(value){_weight=parseInt(value,10),set.remove().clear(),this.render()},getX:function(){return _borderBox.x},getY:function(){return _borderBox.y},getBBox:function(){return _.clone(_borderBox)},getWidth:function(){return _borderBox.width},getHeight:function(){return _borderBox.height},getUnits:function(){return{x:_x.unit,y:_y.unit}},getUnitSizes:function(){return{x:_borderBox.width/_x.unit,y:_borderBox.height/_y.unit}},getSubStepSizes:function(){return{x:_xSubStepSize,y:_ySubStepSize}},getCanvas:function(){return paper},
getOriginPosition:function(){return{left:_padding.left-_x.start*_x.unit,top:_padding.top-_y.start*_y.unit}},getPostionFromCartesian:function(x,y){var origin=this.getOriginPosition(),unitSizes=this.getUnits();return{left:origin.left+unitSizes.x*x,top:origin.top-unitSizes.y*y}},getGridBounds:function(){return{x:{start:_x.start,end:_x.end},y:{start:_y.start,end:_y.end}}},render:function(){_drawGrid(),_drawAxis(),_drawGraphTitle()},snap:function(x,y){return x=paper.raphael.snapTo(_snapToValues.x,x,_xSubStepSize/2),y=paper.raphael.snapTo(_snapToValues.y,y,_ySubStepSize/2),[x,y]},clickable:function(){clickableArea&&clickableArea.remove(),clickableArea=paper.rect(_borderBox.x,_borderBox.y,_borderBox.width,_borderBox.height),
clickableArea.attr({fill:"rgba(0,0,0,0)",stroke:0}),set.push(clickableArea)},toBack:function(shape){clickableArea&&shape.insertBefore(clickableArea)},toFront:function(shape){shape.toFront()}};return _calculateBBox(),obj.render(),obj}function _buildOptions(rawOptions){var axisColor="#222",gridColor="#222",options=_.merge({},{graphTitle:null,graphTitleRequired:!1,graphTitleSize:20,graphTitlePadding:40,color:gridColor,weight:1,axisTitleSize:14,axisTitlePadding:36,labelSize:12,labelPadding:24,padding:30,height:null,width:null,x:{start:-10,end:10,label:null,title:null,step:1,subStep:1,unit:10,color:axisColor,weight:3},y:{start:-10,end:10,label:null,title:null,step:1,
subStep:1,unit:10,color:axisColor,weight:3}},rawOptions);return options.width&&(options.x.unit=(options.width/Math.abs(options.x.end-options.x.start)).toPrecision(2)),options.height&&(options.y.unit=(options.height/Math.abs(options.y.end-options.y.start)).toPrecision(2)),options}function _getAxisTitlePositions(options){var axisTitlePositions={};return options.x.title&&(options.y.start<0?axisTitlePositions.abs="bottom":axisTitlePositions.abs="top"),options.y.title&&(options.x.start<0&&options.x.end<=0?axisTitlePositions.ord="right":axisTitlePositions.ord="left"),axisTitlePositions}function _getPadding(options,_axisTitlePositions){var padding={top:options.padding,
right:options.padding,bottom:options.padding,left:options.padding};return options.graphTitle&&options.graphTitleRequired===!0&&(padding.top+=options.graphTitlePadding),"top"===_axisTitlePositions.abs&&options.x.title?padding.top+=options.axisTitlePadding:options.y.label&&(padding.top+=options.labelPadding),"bottom"===_axisTitlePositions.abs&&options.x.title&&(padding.bottom+=options.axisTitlePadding),"right"===_axisTitlePositions.ord&&options.y.title?padding.right+=options.axisTitlePadding:options.x.label&&(padding.right+=options.labelPadding),"left"===_axisTitlePositions.ord&&options.y.title&&(padding.left+=options.axisTitlePadding),padding}return gridFactory.getPaperSize=function(rawOptions){
var options=_buildOptions(rawOptions),width=Math.abs(options.x.end-options.x.start)*options.x.unit,height=Math.abs(options.y.end-options.y.start)*options.y.unit,axisTitlePositions=_getAxisTitlePositions(options),padding=_getPadding(options,axisTitlePositions);return{width:padding.left+width+padding.right,height:padding.top+height+padding.bottom}},gridFactory}),define("parccTei/portableLib/pointFactory",["taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/lodash"],function($,_){"use strict";function pointFactory(paper,grid,options){options=_.defaults(options||{},_defaults);var _color=options.color,_r=parseInt(options.radius),_x=0,_y=1,_label="",_rGlow=parseInt(options.glowRadius)||3*_r,_events=options.on||{},_dragEnabled=!1,obj={
children:paper.set(),uid:_.uniqueId(),setColor:function(color){_color=String(color)},setX:function(val){_x=parseInt(val)},getX:function(){return _x},getLabel:function(){return _label},getY:function(){return _y},setY:function(val){_y=parseInt(val)},setLabel:function(val){_label=val},setCartesianCoord:function(x,y){_x=grid.getOriginPosition().left+grid.getUnits().x*x,_y=grid.getOriginPosition().top-grid.getUnits().y*y},getCartesianCoord:function(){var cartesianX=(_x-grid.getOriginPosition().left)/grid.getUnits().x,cartesianY=-(_y-grid.getOriginPosition().top)/grid.getUnits().y;return{x:Number(cartesianX.toFixed(2)),y:Number(cartesianY.toFixed(2))}},setCoord:function(x,y){
var coord=grid.snap(parseInt(x),parseInt(y));_x=coord[0],_y=coord[1]},setR:function(val){_r=parseInt(val)},setRGlow:function(val){_rGlow=parseInt(val)},render:function(){this.remove();var circle=paper.circle(_x,_y,_r);if(options.fill?circle.attr({fill:_color,stroke:"#000",cursor:"move"}):circle.attr({stroke:_color,"stroke-width":3,cursor:"move"}),this.children.push(circle),options.glow&&this.showGlow(),_label){var text=paper.text(_x,_y,_label);text.attr({fill:options.labelColor||"#000",stroke:options.labelColor||"#000","stroke-width":options.labelWeight||0,"font-size":options.labelSize||10}),this.children.push(text)}},remove:function(){this.children.length>0&&this.children.remove().clear();
},removeOnClic:function(){var self=this;this.children.click(function(){options.removable&&_dragEnabled===!1&&(self.remove(),$(paper.canvas).trigger("removed.point",self))})},drag:function(){var bb,self=this,moved=!1;_dragEnabled=!0,this.children.drag(function(dx,dy){moved=!0,bb=self.children.getBBox();var newX=self.oBB.x-bb.x+dx,newY=self.oBB.y-bb.y+dy;"x"===options.axis?newY=self.getY()-(bb.y+bb.width/2):"y"===options.axis&&(newX=self.getX()-(bb.x+bb.width/2));var absoluteX=bb.x+bb.width/2,absoluteY=bb.y+bb.height/2;options.xMin&&absoluteX+newX<options.xMin?newX=options.xMin-absoluteX:options.xMax&&absoluteX+newX>options.xMax&&(newX=options.xMax-absoluteX),options.yMin&&absoluteY+newY<options.yMin?newY=options.yMin-absoluteY:options.yMax&&absoluteY+newY>options.yMax&&(newY=options.yMax-absoluteY),
"function"==typeof _events.drag&&_events.drag.call(self,newX,newY),self.children.translate(newX,newY)},function(){moved=!1,"function"==typeof _events.dragStart&&_events.dragStart.call(self),self.oBB=self.children.getBBox()},function(){if(moved){var newX=bb.x+bb.width/2,newY=bb.y+bb.height/2;self.setCoord(newX,newY),self.children.translate(parseInt(self.getX()-newX),parseInt(self.getY()-newY)),"function"==typeof _events.dragStop&&_events.dragStop.call(self,self.getX(),self.getY()),$(paper.canvas).trigger("moved.point",self)}else options.removable&&(self.remove(),$(paper.canvas).trigger("removed.point",self))})},unDrag:function(){this.children.undrag(),_dragEnabled=!1;
},showGlow:function(){var rgb=paper.raphael.color(_color),glow=paper.circle(_x,_y,_rGlow).attr({fill:"rgba("+rgb.r+","+rgb.g+","+rgb.b+", "+options.glowOpacity+" )",stroke:"none",cursor:"move"});this.children.length>1&&this.children.pop().remove(),this.children.push(glow),this.children.attr("cursor","move")},hideGlow:function(){this.children.length>1&&this.children.pop().remove(),this.children.attr("cursor","default")},setOption:function(key,value){options[key]=value}};return options.cartesian?obj.setCartesianCoord(options.x,options.y):obj.setCoord(options.x,options.y),obj.setLabel(options.label),obj}var _defaults={x:0,y:0,color:"#f00",radius:10,glow:!0,glowRadius:0,
glowOpacity:.3,fill:!0,removable:!0,cartesian:!1};return pointFactory}),define("parccTei/pciCreator/ims/graphLineAndPointInteraction/runtime/wrappers/setOfPoints",["taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/lodash","parccTei/portableLib/pointFactory"],function($,_,pointFactory){"use strict";function initialize(grid,config){function setConfig(cfg){config=_.defaults(cfg,_defaults)}function unbindEvents(){$canvas.off("."+uid)}function bindEvents(){$canvas.on("click_grid."+uid,function(event,coord){if(points.length<config.maximumPoints)addPoint(coord.x,coord.y);else{var oldPoint=points.pop();oldPoint.setCoord(coord.x,coord.y),oldPoint.render(),
oldPoint.drag(),points.push(oldPoint),$canvas.trigger("moved.pointSet",[oldPoint])}}).on("removed.point."+uid,function(event,removedPoint){if(points){var pointToDelete=_.findIndex(points,{uid:removedPoint.uid});pointToDelete>-1&&points.splice(pointToDelete,1)}})}function addPoint(x,y,cartesian){var gridBBox=grid.getBBox(),newPoint=pointFactory(paper,grid,{x:x,y:y,xMin:gridBBox.x,xMax:gridBBox.x2,yMin:gridBBox.y,yMax:gridBBox.y2,cartesian:!!cartesian,radius:config.pointRadius,color:config.pointColor});return newPoint.render(),newPoint.drag(),points.push(newPoint),$canvas.trigger("added.pointSet",[newPoint]),newPoint}var points=[],active=!1,uid=(config.max||1,config.uid),paper=grid.getCanvas(),$canvas=$(paper.canvas);
setConfig(config);var pointsWrapper={type:"pointSet",getId:function(){return uid},isActive:function(){return active},activate:function(){_.forEach(points,function(point){point.showGlow(),point.drag()}),bindEvents(),active=!0},disactivate:function(){_.forEach(points,function(point){point.hideGlow(),point.unDrag()}),unbindEvents(),active=!1},destroy:function(){void 0!==points&&points!==[]&&(_.forEach(points,function(point){point.children.remove().clear()}),points=[])},highlightOn:function(){_.forEach(points,function(point){point.showGlow()})},highlightOff:function(){_.forEach(points,function(point){point.hideGlow()})},getState:function(){var pts=[];return _.each(points,function(pt){
pts.push(pt.getCartesianCoord())}),{points:pts,config:_.cloneDeep(config)}},setState:function(state){if(state.config&&setConfig(state.config),_.each(points,function(point){point.remove()}),points=[],state.points){var i=0,maxPoints=config.maximumPoints;_.each(state.points,function(point){return i<maxPoints&&(addPoint(point.x,point.y,!0),void i++)})}pointsWrapper.disactivate()}};return pointsWrapper}var _defaults={pointColor:"#bb1a2a",pointRadius:10,maximumPoints:1};return{initialize:initialize}}),define("parccTei/pciCreator/ims/graphLineAndPointInteraction/runtime/wrappers/points",["parccTei/pciCreator/ims/graphLineAndPointInteraction/runtime/wrappers/setOfPoints"],function(setOfPoints){
function initialize(grid,config){config.max=1;var point=setOfPoints.initialize(grid,config);return point.type="point",point}return{initialize:initialize}}),define("parccTei/portableLib/graphFunction",["taoQtiItem/portableLib/lodash"],function(_){"use strict";function _round(number,decimal){var m=Math.pow(10,parseInt(decimal));return Math.round(number*m)/m}function _isPoint(point){return void 0!==point.x&&void 0!==point.y}function checkPairOfPoints(point1,point2){return _isPoint(point1)&&_isPoint(point2)&&point1.x!==point2.x}function plotCurvedEquation(canvas,equation,config,calc){function jump(){x+=config.precision,pathMove=!0}function getBoundedTop(top){return top<topMin?topMin:top>topMax?topMax:top;
}function appendPath(newPosition){prefix="L",pathMove&&(pathMove=!1,currentPosition?path+="M"+currentPosition.left+" "+getBoundedTop(currentPosition.top):prefix="M"),path+=prefix+newPosition.left+" "+getBoundedTop(newPosition.top)}for(var y,currentPosition,newPosition,x=config.start,margin=100,topMin=-margin,topMax=canvas.height+margin,path="",pathMove=!0,prefix="";x<config.end;)y=calc(equation,x),y!==!1?(newPosition={left:_round(x*config.unitSize.x+config.origin.left,3),top:_round(-y*config.unitSize.y+config.origin.top,3)},newPosition.top>canvas.height+margin||newPosition.top<-margin?(appendPath(newPosition),currentPosition=!1,jump()):(appendPath(newPosition),
currentPosition=newPosition,x+=config.precision)):jump();return canvas.path(path)}var quadratic={get:function(vertex,point){if(checkPairOfPoints(vertex,point)){var a=(point.y-vertex.y)/Math.pow(point.x-vertex.x,2),b=-2*a*vertex.x,c=vertex.y+a*Math.pow(vertex.x,2);return[a,b,c]}},plot:function(canvas,equation,config){return plotCurvedEquation(canvas,equation,config,function(equation,x){var a=equation[0],b=equation[1],c=equation[2];return a*Math.pow(x,2)+b*x+c})}},exponential={get:function(point1,point2){if(checkPairOfPoints(point1,point2)&&point1.y*point2.y>0){var b=Math.log(point2.y/point1.y)/(point2.x-point1.x),a=point1.y/Math.exp(b*point1.x);return[a,b]}},plot:function(canvas,equation,config){
return plotCurvedEquation(canvas,equation,config,function(equation,x){var a=equation[0],b=equation[1];return a*Math.exp(b*x)})}},cosine={get:function(inflection,start){if(checkPairOfPoints(start,inflection)){var d=inflection.y,a=start.y-inflection.y,b=Math.PI/(2*(inflection.x-start.x)),c=-b*start.x;return[a,b,c,d]}},plot:function(canvas,equation,config){return plotCurvedEquation(canvas,equation,config,function(equation,x){var a=equation[0],b=equation[1],c=equation[2],d=equation[3];return a*Math.cos(b*x+c)+d})}},logarithmic={get:function(point1,point2){if(checkPairOfPoints(point1,point2)&&point1.y!==point2.y&&point2.x*point1.x>0){var a=(point2.y-point1.y)/Math.log(point2.x/point1.x),b=Math.exp(point1.y/a)/point1.x;
return[a,b]}return!1},plot:function(canvas,equation,config){return plotCurvedEquation(canvas,equation,config,function(equation,x){var a=equation[0],b=equation[1];return b*x>0&&a*Math.log(b*x)})}},tangent={get:function(start,inflection){if(checkPairOfPoints(start,inflection)){var d=start.y,b=Math.PI/(4*(inflection.x-start.x)),c=-b*start.x,a=(inflection.y-d)/Math.tan(b*inflection.x+c);return[a,b,c,d]}},plot:function(canvas,equation,config){return plotCurvedEquation(canvas,equation,config,function(equation,x){var a=equation[0],b=equation[1],c=equation[2],d=equation[3];return a*Math.tan(b*x+c)+d})}},linear={get:function(point1,point2){if(checkPairOfPoints(point1,point2)){
var a=(point2.y-point1.y)/(point2.x-point1.x),b=point1.y-a*point1.x;return[a,b]}},plot:function(canvas,equation,config){function calc(equation,x){var a=equation[0],b=equation[1];return a*x+b}var startPosition={left:_round(config.start*config.unitSize.x+config.origin.left,3),top:_round(-calc(equation,config.start)*config.unitSize.y+config.origin.top,3)},endPosition={left:_round(config.end*config.unitSize.x+config.origin.left,3),top:_round(-calc(equation,config.end)*config.unitSize.y+config.origin.top,3)},path="M"+startPosition.left+" "+startPosition.top+"L"+endPosition.left+" "+endPosition.top;return canvas.path(path)}},absolute={get:function(start,point2){if(checkPairOfPoints(start,point2)){
var a=(point2.y-start.y)/Math.abs(point2.x-start.x),b=-start.x,c=start.y;return[a,b,c]}},plot:function(canvas,equation,config){function calc(equation,x){var a=equation[0],b=equation[1],c=equation[2];return a*Math.abs(x+b)+c}var startPosition={left:_round(config.start*config.unitSize.x+config.origin.left,3),top:_round(-calc(equation,config.start)*config.unitSize.y+config.origin.top,3)},middlePosition={left:_round(-equation[1]*config.unitSize.x+config.origin.left,3),top:_round(-calc(equation,-equation[1])*config.unitSize.y+config.origin.top,3)},endPosition={left:_round(config.end*config.unitSize.x+config.origin.left,3),top:_round(-calc(equation,config.end)*config.unitSize.y+config.origin.top,3)
},path="M"+startPosition.left+" "+startPosition.top+"L"+middlePosition.left+" "+middlePosition.top+"L"+endPosition.left+" "+endPosition.top;return canvas.path(path)}},vertical={get:function(point){if(void 0!==point.x)return[point.x]},plot:function(canvas,equation,config){var multiplier;multiplier=config.segment===!0?-1:1;var x=equation[0]*config.unitSize.x+config.origin.left,yStart=multiplier*config.start_y*config.unitSize.y+config.origin.top,yEnd=multiplier*config.end_y*config.unitSize.y+config.origin.top,path="M"+x+" "+yStart+"L"+x+" "+yEnd;return canvas.path(path)}};return{checkPairOfPoints:checkPairOfPoints,linear:linear,absolute:absolute,quadratic:quadratic,
exponential:exponential,logarithmic:logarithmic,cosine:cosine,tangent:tangent,vertical:vertical}}),define("parccTei/portableLib/plotFactory",["taoQtiItem/portableLib/lodash","parccTei/portableLib/graphFunction"],function(_,graphFunction){"use strict";function _applyStyle(path,config){path.attr({stroke:config.color?config.color:_defaults.color,"stroke-width":config.thickness?config.thickness:_defaults.thickness,opacity:config.opacity?config.opacity:_defaults.opacity})}function PlotFactory(grid,config0){function _translateCoordinate(point){return{x:(point.getX()-config.origin.left)/config.unitSize.x,y:-(point.getY()-config.origin.top)/config.unitSize.y}}function _plot(fnName,p1,p2,conf){
var equation,plot,point1=_translateCoordinate(p1),point2=_translateCoordinate(p2);conf=_.defaults(conf||{},config);try{if(equation=graphFunction[fnName].get(point1,point2))return conf.segment&&(conf.start=Math.min(point1.x,point2.x),conf.end=Math.max(point1.x,point2.x),conf.start_y=point1.y,conf.end_y=point2.y),equation.type=fnName,plot=graphFunction[fnName].plot(canvas,equation,conf),plot.equation=equation,_applyStyle(plot,conf),plot}catch(e){console.log(e)}return!1}var _this=this,canvas=grid.getCanvas(),config=_.clone(config0||{});config=_.defaults(config,_defaults);var bounds=grid.getGridBounds();config.unitSize=grid.getUnits(),config.origin=grid.getOriginPosition(),
config.start=bounds.x.start,config.end=bounds.x.end,config.start_y=bounds.y.start,config.end_y=bounds.y.end;var availableFunctions=["vertical","linear","absolute","cosine","tangent","exponential","logarithmic","quadratic"];_.each(availableFunctions,function(fnName){_this[PlotFactory.getPlotName(fnName)]=function(point1,point2,conf){return _plot(fnName,point1,point2,conf)}})}var _defaults={start:-10,end:10,precision:.01,opacity:.8,color:"#bb1a2a",thickness:3};return PlotFactory.getPlotName=function(mathFunctionName){return"plot"+mathFunctionName.charAt(0).toUpperCase()+mathFunctionName.substr(1)},PlotFactory}),define("parccTei/pciCreator/ims/graphLineAndPointInteraction/runtime/wrappers/lines",["taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/lodash","parccTei/portableLib/pointFactory","parccTei/portableLib/plotFactory"],function($,_,pointFactory,PlotFactory){
"use strict";function initialize(grid,config){function setConfig(cfg){config=_.defaults(cfg,_defaults)}function unbindEvents(){$paperCanvas.off("."+uid)}function plot(){var point1=points[0],point2=points[1],plotConf={color:config.lineColor,segment:segment,thickness:config.lineWeight,opacity:.8};point1&&point2&&(clearPlot(),line=plotFactory.plotLinear(point1,point2,plotConf),point1.getX()===point2.getX()&&(line=plotFactory.plotVertical(point1,point2,plotConf)),line.uid=uid,config.lineStyle&&line.attr({"stroke-dasharray":config.lineStyle}),$paperCanvas.trigger("drawn.lines",[line]))}function clearPlot(){line&&(line.remove(),line=null,$paperCanvas.trigger("removed.lines",[line]));
}function addPoint(x,y,cartesian){var gridBBox=grid.getBBox(),newPoint=pointFactory(paper,grid,{x:x,y:y,xMin:gridBBox.x,xMax:gridBBox.x2,yMin:gridBBox.y,yMax:gridBBox.y2,cartesian:!!cartesian,radius:config.pointRadius,color:config.pointColor,on:{dragStart:clearPlot}});return newPoint.render(),newPoint.drag(),points.push(newPoint),2===points.length&&(plot(),$paperCanvas.on("moved.point",plot)),newPoint}function bindEvents(){$paperCanvas.on("click_grid."+uid,function(event,coord){if(points.length<2)addPoint(coord.x,coord.y);else{var oldPoint=points.pop();oldPoint.setCoord(coord.x,coord.y),oldPoint.render(),oldPoint.drag(),points.push(oldPoint),plot()}}).on("removed.point."+uid,function(event,removedPoint){
if(points){var pointToDelete=_.findIndex(points,{uid:removedPoint.uid});pointToDelete>-1&&(points.splice(pointToDelete,1),clearPlot())}}).trigger("activated.lines",[uid])}var line,points=[],active=!1,uid=config.uid,segment=config.segment||!1,paper=grid.getCanvas(),$paperCanvas=$(paper.canvas),plotFactory=new PlotFactory(grid);setConfig(config);var linesWrapper={type:"line",getId:function(){return uid},getLine:function(){return line},isActive:function(){return active},activate:function(){_.forEach(points,function(point){point.showGlow(),point.drag()}),bindEvents(),active=!0},disactivate:function(){_.forEach(points,function(point){point.hideGlow(),point.unDrag();
}),unbindEvents(),active=!1},destroy:function(){void 0!==line&&null!==line&&(line.remove(),line=null),void 0!==points&&points!==[]&&(_.forEach(points,function(point){point.children.remove().clear()}),points=[])},setLineStyle:function(style){config.lineStyle=style||"",plot()},highlightOn:function(){_.forEach(points,function(point){point.showGlow()})},highlightOff:function(){_.forEach(points,function(point){point.hideGlow()})},getState:function(){var pts=[];return _.each(points,function(pt){pts.push(pt.getCartesianCoord())}),{points:pts,config:_.cloneDeep(config)}},setState:function(state){state.config&&setConfig(state.config),clearPlot(),_.each(points,function(point){
point.remove()}),points=[],state.points&&_.each(state.points,function(point){addPoint(point.x,point.y,!0)}),linesWrapper.disactivate()}};return linesWrapper}var _defaults={pointColor:"#bb1a2a",lineColor:"#bb1a2a",lineStyle:"",lineWeight:3,pointRadius:10};return{initialize:initialize}}),define("parccTei/pciCreator/ims/graphLineAndPointInteraction/runtime/wrappers/segments",["parccTei/pciCreator/ims/graphLineAndPointInteraction/runtime/wrappers/lines"],function(lines){function initialize(grid,config){config.segment=!0;var segment=lines.initialize(grid,config);return segment.type="segment",segment}return{initialize:initialize}}),define("parccTei/pciCreator/ims/graphLineAndPointInteraction/runtime/wrappers/solutionSet",["taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/lodash"],function($,_){
"use strict";function getHorizontalEquation(y){y=-y;var equation=[0,y];return equation.type="linear",{equation:equation,uid:"horizontal_"+y}}function getVerticalEquation(x){var equation=[x];return equation.type="vertical",{equation:equation,uid:"vertical_"+x}}function _round(num){return Math.round(100*num)/100}function intersect(grid,line1,line2){var x,y,bounds=grid.getGridBounds(),eq1=line1.equation,eq2=line2.equation;if(eq1.type!==eq2.type||"vertical"!==eq1.type&&eq1[0]!==eq2[0]){if(eq1.type===eq2.type)x=(eq1[1]-eq2[1])/(eq2[0]-eq1[0]),y=eq1[0]*x+eq1[1];else{var vertical,linear;"vertical"===eq1.type?(vertical=eq1,linear=eq2):(vertical=eq2,linear=eq1),x=vertical[0],
y=linear[0]*vertical[0]+linear[1]}var intersection={uid:_.uniqueId("intersection_"),x:_round(x),y:_round(y),lines:[line1,line2],initCircling:function(){var nextDirection,siblings,next;if("vertical"===line1.type)nextDirection=line2;else if("vertical"===line2.type)nextDirection=line1;else if(line1.equation[0]>line2.equation[0])nextDirection=line1;else{if(!(line2.equation[0]>line1.equation[0]))throw"unexpected situation";nextDirection=line2}return(y>-bounds.y.end||y===-bounds.y.end&&nextDirection.equation[0]>0)&&(siblings=nextDirection.getSiblings(intersection),next=siblings.next),{point:next,direction:nextDirection,ascendingX:!0}},next:function next(previousDirection,ascendingX){
ascendingX=!!ascendingX;var nextDirection,next;previousDirection&&(nextDirection=line1.uid===previousDirection.uid?line2:line1);var siblings=nextDirection.getSiblings(intersection);return"linear"===nextDirection.type?("linear"===previousDirection.type&&nextDirection.equation[0]>previousDirection.equation[0]&&(ascendingX=!ascendingX),next=ascendingX?siblings.next:siblings.previous):"vertical"===nextDirection.type&&(ascendingX=!ascendingX,next=ascendingX?siblings.next:siblings.previous),{point:next,direction:nextDirection,ascendingX:ascendingX}}};return intersection}}function withinGrid(grid,intersection){var bounds=grid.getGridBounds();return bounds.x.start<=intersection.x&&intersection.x<=bounds.x.end&&-bounds.y.end<=intersection.y&&intersection.y<=-bounds.y.start;
}function getClosedPath(intersection){var path=[],current=intersection.initCircling(),i=0;if(current.point)for(path.push([intersection.x,intersection.y]);current.point.x!==intersection.x||current.point.y!==intersection.y;){path.push([current.point.x,current.point.y]);var next=current.point.next(current.direction,current.ascendingX),current=next;if(i++,i>20)break}return path}function createIntersectingLine(drawnLine){var intersections=[],sorted=!1,intersectingLine={equation:drawnLine.equation,type:drawnLine.equation.type,uid:drawnLine.uid};return intersectingLine.addIntersection=function(intersection){intersections.push(intersection),sorted=!1},intersectingLine.getSiblings=function(intersection){
var previous,next,index;return sorted||(intersections="vertical"===drawnLine.equation.type?_.sortBy(intersections,"y"):_.sortBy(intersections,"x")),index=_.findIndex(intersections,function(i){return i.uid===intersection.uid}),0===index?next=intersections[1]:index===intersections.length-1?previous=intersections[intersections.length-2]:index>0&&(previous=intersections[index-1],next=intersections[index+1]),{previous:previous,next:next}},intersectingLine}function zipPath(pathArray){var pathStr="";return _.forIn(pathArray,function(coords,i){pathStr+=0==i?"M":"L",pathStr+=coords[0]+","+coords[1]}),pathStr+="z"}function convertPath(grid,pathArray){var positionArray=[];
return _.each(pathArray,function(coord){var pos=grid.getPostionFromCartesian(coord[0],coord[1]);positionArray.push([pos.left,pos.top])}),positionArray}function drawSolutionSet(grid,lines,config){var paper=grid.getCanvas(),intersectingLines=[],intersections=[],areas=[],bounds=grid.getGridBounds();for(_.each(lines,function(line){var drawnLine=line.getLine();drawnLine&&intersectingLines.push(createIntersectingLine(drawnLine))}),intersectingLines.push(createIntersectingLine(getHorizontalEquation(bounds.y.start))),intersectingLines.push(createIntersectingLine(getHorizontalEquation(bounds.y.end))),intersectingLines.push(createIntersectingLine(getVerticalEquation(bounds.x.start))),
intersectingLines.push(createIntersectingLine(getVerticalEquation(bounds.x.end)));intersectingLines.length;){var line1=intersectingLines.pop();_.each(intersectingLines,function(line2){var intersection=intersect(grid,line1,line2);intersection&&withinGrid(grid,intersection)&&(line1.addIntersection(intersection),line2.addIntersection(intersection),intersections.push(intersection))})}return _.each(intersections,function(intersection){var convertedPath,closedPath=getClosedPath(intersection);if(closedPath.length){convertedPath=convertPath(grid,closedPath),convertedPath=zipPath(convertedPath);var $area,area=paper.path(convertedPath).attr({fill:config.color,opacity:_style.opacityDefault
});area.selected=!1,area.closedPath=_.map(closedPath,function(point){return{x:point[0],y:point[1]}}),_debug&&area.attr({stroke:"#222"}),$area=$(area[0]).on("mouseenter",function(){area.attr({opacity:_style.opacityHover})}).on("mouseleave",function(){area.attr({opacity:area.selected?_style.opacitySelected:_style.opacityDefault})}).on("click",function(){area.selected?unselectArea(area):selectArea(area)}),areas.push(area)}}),areas}function selectArea(area){area.selected=!0,area.attr({opacity:_style.opacitySelected}),$(area[0]).trigger("selected.solutionSet",[area])}function unselectArea(area){area.selected=!1,area.attr({opacity:_style.opacityDefault}),$(area[0]).trigger("unselected.solutionSet",[area]);
}function initialize(grid,config){function setConfig(cfg){config=_.defaults(cfg,_defaults)}function createSolutionSet(elements){areas=drawSolutionSet(grid,_.where(elements,{type:"line"}),config),_.each(areas,function(area){set.push(area)})}function clearSolutionSet(){set.remove().clear()}var line,areas=[],active=!1,uid=config.uid,paper=grid.getCanvas(),$paperCanvas=$(paper.canvas),set=paper.set();setConfig(config),$paperCanvas.on("drawn.lines removed.lines activated.lines",clearSolutionSet);var solutionSetWrapper={type:"solutionSet",getId:function(){return uid},getLine:function(){return line},isActive:function(){return active},activate:function(elements){set.length||createSolutionSet(elements),
grid.toFront(set),active=!0},disactivate:function(){grid.toBack(set),active=!1},destroy:function(){},highlightOn:function(){},highlightOff:function(){},getState:function(){var selections=[];return _.each(areas,function(area){area.selected&&selections.push(area.closedPath)}),{selections:selections,config:_.cloneDeep(config)}},setState:function(state){state.config&&setConfig(state.config),state.selections&&_.isArray(state.selections)&&_.each(state.selections,function(selection){_.each(areas,function(area){if(_.isEqual(selection,area.closedPath))return selectArea(area),!1})})},createSolutionSet:function(elements){clearSolutionSet(),createSolutionSet(elements)}};return solutionSetWrapper;
}var _debug=!1,_defaults={color:"#326399"},_style={opacityDefault:_debug?.1:0,opacityHover:.3,opacitySelected:.6};return{initialize:initialize}}),define("normalize",[],function(){function convertURIBase(uri,fromBase,toBase){if(uri.match(absUrlRegEx)||uri.match(protocolRegEx))return uri;uri=removeDoubleSlashes(uri);var toBaseProtocol=toBase.match(protocolRegEx),fromBaseProtocol=fromBase.match(protocolRegEx);return!fromBaseProtocol||toBaseProtocol&&toBaseProtocol[1]==fromBaseProtocol[1]&&toBaseProtocol[2]==fromBaseProtocol[2]?relativeURI(absoluteURI(uri,fromBase),toBase):absoluteURI(uri,fromBase)}function absoluteURI(uri,base){if("./"==uri.substr(0,2)&&(uri=uri.substr(2)),
...
if(sheet.href==link.href)return clearInterval(loadInterval),callback()}},10);link.href=url,head.appendChild(link)};return cssAPI.normalize=function(name,normalize){return".css"==name.substr(name.length-4,4)&&(name=name.substr(0,name.length-4)),normalize(name)},cssAPI.load=function(cssId,req,load,config){(useImportLoad?importLoad:linkLoad)(req.toUrl(cssId+".css"),load)},cssAPI}),define("css!parccTei/pciCreator/ims/graphLineAndPointInteraction/runtime/css/graphLineAndPointInteraction",[],function(){}),define("graphLineAndPointInteraction/runtime/graphLineAndPointInteraction",["taoQtiItem/portableLib/jquery_2_1_1","qtiCustomInteractionContext","taoQtiItem/portableLib/OAT/util/event","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/OAT/scale.raphael","taoQtiItem/portableLib/raphael","parccTei/portableLib/gridFactory","parccTei/pciCreator/ims/graphLineAndPointInteraction/runtime/wrappers/setOfPoints","parccTei/pciCreator/ims/graphLineAndPointInteraction/runtime/wrappers/points","parccTei/pciCreator/ims/graphLineAndPointInteraction/runtime/wrappers/lines","parccTei/pciCreator/ims/graphLineAndPointInteraction/runtime/wrappers/segments","parccTei/pciCreator/ims/graphLineAndPointInteraction/runtime/wrappers/solutionSet","css!parccTei/pciCreator/ims/graphLineAndPointInteraction/runtime/css/graphLineAndPointInteraction"],function($,qtiCustomInteractionContext,event,_,scaleRaphael,Raphael,gridFactory,setPointsWrapper,pointsWrapper,linesWrapper,segmentsWrapper,solutionSetWrapper){
"use strict";function buildGridConfig(rawConfig){return{x:{start:_.isUndefined(rawConfig.xMin)?-10:parseInt(rawConfig.xMin),end:_.isUndefined(rawConfig.xMax)?10:parseInt(rawConfig.xMax),unit:20},y:{start:_.isUndefined(rawConfig.yMax)?10:-1*parseInt(rawConfig.yMax),end:_.isUndefined(rawConfig.yMin)?-10:-1*parseInt(rawConfig.yMin),unit:20},graphs:_.isUndefined(rawConfig.graphs)?{}:rawConfig.graphs,padding:20}}function createCanvas($container,config){var padding=2*config.padding,paper=scaleRaphael($(".shape-container",$container)[0],(config.x.end-config.x.start)*config.x.unit+padding,(config.y.end-config.y.start)*config.y.unit+padding);return paper}function getWrapper(type){
return _wrappers[type]}function drawLineStyle(dom,config){var w=57,h=20,lineStylePaper=new Raphael(dom,w,h),line=lineStylePaper.path("M0 "+h/2+"L"+w+" "+h/2);line.attr({stroke:config.lineColor||"#000","stroke-width":config.lineWeight||3,"stroke-dasharray":config.lineStyle||"",opacity:config.opacity||1})}function isValidRecordEntry(entry){return _.isPlainObject(entry)&&_.isString(entry.name)&&entry.base&&entry.base.list&&_.isArray(entry.base.list.point)}function formatResponseElement(name,points){if(_.isString(name),_.isArray(points))return points=_.map(points,function(point){return[point.x,point.y]}),{name:name,base:{list:{point:points}}};throw new Error("invalid arguments");
}var graphLineAndPointInteraction,_wrappers={setPoints:setPointsWrapper,points:pointsWrapper,lines:linesWrapper,segments:segmentsWrapper,solutionSet:solutionSetWrapper},responseChangeEvents=["drawn.lines","moved.point","removed.point","added.pointSet","moved.pointSet","unselected.solutionSet","selected.solutionSet"],_typeIdentifier="graphLineAndPointInteraction";graphLineAndPointInteraction={typeIdentifier:_typeIdentifier,getInstance:function(dom,config,state){var response=config.boundTo;try{config.properties.graphs=JSON.parse(config.properties.graphs)}catch(e){}this.initialize(Object.getOwnPropertyNames(response).pop(),dom,config.properties),this.setSerializedState(state),
"function"==typeof config.onready&&config.onready(this,this.getState())},getState:function(){return this.getSerializedState()},oncompleted:function(){this.destroy()},getResponse:function(){var rawResponse=this.getRawResponse(),response={record:[]};return _.each(rawResponse,function(element){"solutionSet"===element.type?_.each(element.selections,function(selection){response.record.push(formatResponseElement(element.id,selection))}):response.record.push(formatResponseElement(element.id,element.points))}),response},destroy:function(){var $container=$(this.dom);$container.off().empty()},getTypeIdentifier:function(){return _typeIdentifier},initialize:function(id,dom,config){
function initGrid($container,gridConfig){var grid,paper=createCanvas($container,gridConfig),$canvas=$(paper.canvas);return elements={},_.isObject(gridConfig.x)&&_.isObject(gridConfig.y)&&gridConfig.x.start<gridConfig.x.end&&gridConfig.y.start<gridConfig.y.end&&(grid=gridFactory(paper,gridConfig),grid.clickable(),grid.children.click(function(event){var bnds=event.target.getBoundingClientRect(),wfactor=paper.w/paper.width,fx=grid.getX()+Math.round((event.clientX-bnds.left)/bnds.width*grid.getWidth()*wfactor),fy=grid.getY()+Math.round((event.clientY-bnds.top)/bnds.height*grid.getHeight()*wfactor);$canvas.trigger("click_grid",{x:fx,y:fy})})),grid}function initInteraction(grid,$cont,options){
var $templates=$cont.find(".templates"),$controlArea=$cont.find(".shape-controls");if($controlArea.empty(),_.each(options.graphs,function(graphType,typeName){var $template=$templates.find(".template-"+typeName);_.each(graphType.elements,function(elementConfig){var $buttonContainer=$template.children().first().clone(),$button=$buttonContainer.children(".btn"),$arrow=$buttonContainer.children(".triangle"),color=elementConfig.lineColor||elementConfig.pointColor;$buttonContainer.attr("data-uid",elementConfig.uid),$buttonContainer.data("config",elementConfig),$button.text(elementConfig.label),$button.css({backgroundColor:color}),$arrow.css({borderColor:"transparent transparent transparent "+color
}),elementConfig.lineStyleToggle&&"false"!==elementConfig.lineStyleToggle&&$buttonContainer.find(".line-styles").show().find("input[name=line-style]").attr("name","line-style-"+elementConfig.uid).change(function(){elementConfig.lineStyle=$(this).val(),$buttonContainer.data("element").setLineStyle(elementConfig.lineStyle),$button.click()}).each(function(){var $input=$(this),$lineStyle=$input.siblings(".line-style"),style=$input.val();drawLineStyle($lineStyle[0],{lineStyle:style,lineColor:elementConfig.lineColor,lineWeight:elementConfig.lineWeight}),elementConfig.lineStyle===style&&$input.prop("checked",!0)}),$controlArea.append($buttonContainer);var wrapper=getWrapper(typeName);
if(grid){var element=wrapper.initialize(grid,elementConfig);$buttonContainer.data("element",element),element.$buttonContainer=$buttonContainer,elements[elementConfig.uid]=element}})}),$controlArea.on("click",".button-container:not(.deactivated)",function(){activate($(this).data("element"))}).on("mouseenter",".button-container:not(.deactivated)",function(){var element=$(this).data("element");element&&element.highlightOn()}).on("mouseleave",".button-container:not(.deactivated)",function(){var element=$(this).data("element");element&&!element.isActive()&&element.highlightOff()}),$container.on(responseChangeEvents.join(" "),_.debounce(function(){self.trigger("responseChange",[self.getResponse()]);
},100)),grid){var $solutionSet=$controlArea.find(".graph-solutionSet");$(grid.getCanvas().canvas).on("drawn.lines removed.lines",function(){var drawnLineExists=!1;_.each(_.where(elements,{type:"line"}),function(line){var drawnLine=line.getLine();if(drawnLine)return drawnLineExists=!0,!1}),drawnLineExists?$solutionSet.removeClass("deactivated"):$solutionSet.addClass("deactivated")})}}function activate(element){graph&&graph.disactivate(),element.$buttonContainer.addClass("activated"),element.$buttonContainer.siblings(".button-container").removeClass("activated"),element.activate(elements),graph=element}function getElement(uid){return elements[uid]}function getState(){
var state={activeGraph:graph?graph.getId():"",elements:{}};return _.each(elements,function(element){state.elements[element.getId()]=element.getState()}),state}function setState(state,ignoreConfig){if(_.forIn(state.elements,function(state,uid){var element=getElement(uid);element&&(ignoreConfig&&delete state.config,element.setState(state))}),state.activeGraph){var currentActive=getElement(state.activeGraph);currentActive&&activate(currentActive)}}var grid,graph,elements,$container=$(dom),self=this;this.id=id,this.dom=dom,this.config=buildGridConfig(config||{}),event.addEventMgr(this),this.getRawResponse=function(){var response=[],states=[];return _.each(elements,function(element,id){
var res={id:id,type:element.type};"solutionSet"===element.type?res.selections=element.getState().selections:res.points=element.getState().points,response.push(res),states.push(element.getState())}),response},this.setRawResponse=function(response){_.each(response,function(res){var state={},element=elements[res.id];element&&("solutionSet"===res.type?(element.createSolutionSet(elements),state.selections=res.selections):state.points=res.points,element.setState(state))})},grid=initGrid($container,this.config),initInteraction(grid,$container,this.config),this.on("configchange",function(newConfig){var state=getState();self.config=newConfig?buildGridConfig(newConfig):self.config,
grid=initGrid($container,self.config),initInteraction(grid,$container,self.config),setState(state,!0)}),this.on("gridchange",function(newConfig){self.config=newConfig?buildGridConfig(newConfig):self.config,grid=initGrid($container,self.config),initInteraction(grid,$container,self.config)})},setResponse:function(response){var rawResponse=[],solutionSetSelections=[],solutionSetId="";response&&_.isArray(response.record)&&(_.each(response.record,function(entry){var points,id;isValidRecordEntry(entry)&&(id=entry.name,points=_.map(entry.base.list.point,function(point){return{x:point[0],y:point[1]}}),id.match(/^solutionSet/)?(solutionSetId=id,solutionSetSelections.push(points)):rawResponse.push({
id:id,points:points}))}),solutionSetId&&solutionSetSelections.length&&rawResponse.push({id:solutionSetId,type:"solutionSet",selections:solutionSetSelections}),this.setRawResponse(rawResponse))},resetResponse:function(){},setSerializedState:function(state){this.setResponse(state)},getSerializedState:function(){return this.getResponse()}},qtiCustomInteractionContext.register(graphLineAndPointInteraction)}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))}("/*add some style, need to scope (using the typeIdentifier as class prefix) every style declaration to prevent collision */\n\n.pointAndLineFunctionInteraction {\n    width: 100%;\n    margin: 20px 0;\n    padding: 20px;\n    border : 6px solid #f5f3f2;\n}\n\n.pointAndLineFunctionInteraction .shape-controls .clearfix {\n    overflow: auto;\n}\n\n.pointAndLineFunctionInteraction .shape-panel {position:relative; display: inline-block; width: 100%;}\n\n.pointAndLineFunctionInteraction .shape-panel .templates {display:none;}\n\n.pointAndLineFunctionInteraction .shape-panel .shape-container {float:left;}\n\n.pointAndLineFunctionInteraction .shape-panel .shape-controls {float:left; margin-bottom:20px;}\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container {width:200px; margin:3px 0; display:block; position:relative; padding: 3px;}\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container .btn {width:150px; float:left;}\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container .triangle{\n    width: 0;\n    height: 0;\n    border-style: solid;\n    border-width: 16px 0 16px 27.7px;\n    border-color: transparent transparent transparent #266d9c;\n    float:left;\n    top:2px;\n    position: relative;\n    visibility: hidden;\n}\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container.activated .triangle{\n    visibility: visible;\n}\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container .btn:hover + .triangle {opacity:.85;}\n\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container .line-styles {display:none; margin-top:30px; padding:10px;width:150px;border: solid 3px #EFEFEF;}\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container .line-style {display:inline-block; margin-left:3px;}\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container svg {top:5px;}\n\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container.deactivated .btn {background : #AFB4BB !important; text-shadow:none;}\n.pointAndLineFunctionInteraction .shape-panel .shape-controls .button-container.deactivated .btn:hover {opacity:1; cursor:default;}\n"),
define(["graphLineAndPointInteraction/runtime/graphLineAndPointInteraction"],function(IMSPCI){return IMSPCI});
//# sourceMappingURL=graphLineAndPointInteraction.min.js.map