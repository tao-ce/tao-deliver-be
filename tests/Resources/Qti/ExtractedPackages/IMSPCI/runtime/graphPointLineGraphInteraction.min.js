...
if(sheet.href==link.href)return clearInterval(loadInterval),callback()}},10);link.href=url,head.appendChild(link)};return cssAPI.normalize=function(name,normalize){return".css"==name.substr(name.length-4,4)&&(name=name.substr(0,name.length-4)),normalize(name)},cssAPI.load=function(cssId,req,load,config){(useImportLoad?importLoad:linkLoad)(req.toUrl(cssId+".css"),load)},cssAPI}),define("css!parccTei/pciCreator/ims/graphPointLineGraphInteraction/runtime/css/graphPointLineGraphInteraction",[],function(){}),define("graphPointLineGraphInteraction/runtime/graphPointLineGraphInteraction",["taoQtiItem/portableLib/jquery_2_1_1","qtiCustomInteractionContext","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/OAT/util/event","taoQtiItem/portableLib/OAT/scale.raphael","parccTei/portableLib/gridFactory","parccTei/portableLib/pointFactory","parccTei/portableLib/plotFactory","css!parccTei/pciCreator/ims/graphPointLineGraphInteraction/runtime/css/graphPointLineGraphInteraction"],function($,qtiCustomInteractionContext,_,event,scaleRaphael,gridFactory,pointFactory,PlotFactory){
"use strict";var _typeIdentifier="graphPointLineGraphInteraction",buildGridConfig=function(rawConfig){var getBoolean=function(value,defaultValue){return"undefined"==typeof value?defaultValue:value===!0||"true"===value},radix=10,xWeight=parseInt(rawConfig.xWeight,radix),xBorderWeight=parseInt(rawConfig.xBorderWeight,radix),yWeight=parseInt(rawConfig.yWeight,radix),yBorderWeight=parseInt(rawConfig.yBorderWeight,radix),gridConfig={draggable:getBoolean(rawConfig.draggable,!0),graphType:rawConfig.graphType,maxPoints:parseInt(rawConfig.maxPoints,radix),segment:getBoolean(rawConfig.segment,!0),graphTitle:rawConfig.graphTitle,graphTitleRequired:getBoolean(rawConfig.graphTitleRequired,!1),
weight:parseInt(rawConfig.weight,radix),width:parseInt(rawConfig.width,radix),height:parseInt(rawConfig.height,radix),x:{start:parseInt(rawConfig.xStart,radix),end:parseInt(rawConfig.xEnd,radix),label:rawConfig.xLabel,title:rawConfig.xTitle,step:parseInt(rawConfig.xStep,radix),subStep:parseInt(rawConfig.xSubStep,radix),weight:xWeight>0?xWeight:xBorderWeight,allowOuter:getBoolean(rawConfig.xAllowOuter,!0)},y:{start:-1*parseInt(rawConfig.yEnd,radix),end:-1*parseInt(rawConfig.yStart,radix),label:rawConfig.yLabel,title:rawConfig.yTitle,step:parseInt(rawConfig.yStep,radix),subStep:parseInt(rawConfig.ySubStep,radix),weight:yWeight>0?yWeight:yBorderWeight,allowOuter:getBoolean(rawConfig.yAllowOuter,!0)
},plot:{color:rawConfig.plotColor,thickness:parseInt(rawConfig.plotThickness,radix)},point:{color:rawConfig.pointColor,glow:getBoolean(rawConfig.pointGlow,!0),radius:parseInt(rawConfig.pointRadius,radix)},staticPlot:{color:rawConfig.staticLineColor,thickness:parseInt(rawConfig.staticLineThickness,radix)},staticPoint:{color:rawConfig.staticPointColor,labelColor:rawConfig.staticPointLabelColor,labelSize:rawConfig.staticPointLabelSize,labelWeight:rawConfig.staticPointLabelWeight,glow:getBoolean(rawConfig.staticPointGlow,!1),radius:parseInt(rawConfig.staticPointRadius,radix)},showStaticLines:getBoolean(rawConfig.staticShowLine,!0),showStaticPoints:getBoolean(rawConfig.staticDisplayPoints,!0),
staticPoints:rawConfig.staticPoints};try{gridConfig.staticPoints=JSON.parse(gridConfig.staticPoints)}catch(e){}return gridConfig.x.step<1&&(gridConfig.x.step=1),gridConfig.y.step<1&&(gridConfig.y.step=1),gridConfig.x.subStep<1&&(gridConfig.x.subStep=1),gridConfig.y.subStep<1&&(gridConfig.y.subStep=1),gridConfig.x.weight>0==!1&&(gridConfig.x.weight=3),gridConfig.y.weight>0==!1&&(gridConfig.y.weight=3),gridConfig},createCanvas=function($container,config){var paperSize=gridFactory.getPaperSize(config),paper=scaleRaphael($(".shape-container",$container)[0],paperSize.width,paperSize.height);return paper},graphPointLineGraphInteraction={typeIdentifier:_typeIdentifier,
getInstance:function(dom,config,state){var response=config.boundTo;this.initialize(Object.getOwnPropertyNames(response).pop(),dom,config.properties,config.assetManager),this.setSerializedState(state),"function"==typeof config.onready&&config.onready(this,this.getState())},getState:function(){return this.getSerializedState()},oncompleted:function(){this.destroy()},getResponse:function(){var raw=this.getRawResponse();return raw?{list:{string:raw}}:{base:null}},destroy:function(){var $container=$(this.dom);$container.off().empty()},getTypeIdentifier:function(){return _typeIdentifier},initialize:function(id,dom,config){function initGrid($gridContainer,gridConfig){
return clearPlot(),clearPoint(),clearStaticPlot(),clearStaticPoint(),paper=createCanvas($gridContainer,gridConfig),_.isObject(gridConfig.x)&&_.isObject(gridConfig.y)&&gridConfig.x.start<gridConfig.x.end&&gridConfig.y.start<gridConfig.y.end&&(grid=gridFactory(paper,gridConfig),grid.clickable(),grid.children.click(function(clickEvent){var oldPoint,bnds=clickEvent.target.getBoundingClientRect(),wfactor=paper.w/paper.width,fx=grid.getX()+Math.round((clickEvent.clientX-bnds.left)/bnds.width*grid.getWidth()*wfactor),fy=grid.getY()+Math.round((clickEvent.clientY-bnds.top)/bnds.height*grid.getHeight()*wfactor);areCoordsValid(fx,fy)&&(points.length<self.gridConfig.maxPoints?(addPoint(fx,fy),
plot()):(oldPoint=points.pop(),oldPoint.setCoord(fx,fy),points.push(oldPoint),plot()))}),plotFactory=new PlotFactory(grid,gridConfig.plot),staticPlotFactory=new PlotFactory(grid,gridConfig.staticPlot),_.forEach(gridConfig.staticPoints,function(point){addStaticPoint(point.x,point.y,point.label,!0)}),staticPlot(),$(paper.canvas).off("removed.point").on("removed.point",function(_event,removedPoint){var pointToDelete=_.findIndex(points,{uid:removedPoint.uid});pointToDelete>-1&&(points.splice(pointToDelete,1),plot())})),grid}function removeAll(list){list.forEach(function(element){element.remove()})}function clearPlot(){removeAll(paths),paths=[]}function clearPoint(){
removeAll(points),points=[]}function clearStaticPlot(){removeAll(staticPaths),staticPaths=[]}function clearStaticPoint(){removeAll(staticPoints),staticPoints=[]}function plotLines(points,paths,plotFactory){var sortedPoints=points.sort(function(pointA,pointB){var ax=pointA.getX(),ay=pointA.getY(),bx=pointB.getX(),by=pointB.getY();return ax!==bx?ax-bx:ay-by}),plotConfig={segment:self.gridConfig.segment};sortedPoints.length>=2&&sortedPoints.reduce(function(pointA,pointB){return pointA.getX()===pointB.getX()?paths.push(plotFactory.plotVertical(pointA,pointB,plotConfig)):paths.push(plotFactory.plotLinear(pointA,pointB,plotConfig)),pointB})}function plot(){clearPlot(),
"line"===self.gridConfig.graphType&&plotLines(points,paths,plotFactory),points.forEach(function(point){point.render(),self.gridConfig.draggable&&point.drag(),point.removeOnClic()}),self.trigger("responseChange",[self.getResponse()])}function staticPlot(){clearStaticPlot(),self.gridConfig.showStaticLines&&plotLines(staticPoints,staticPaths,staticPlotFactory),self.gridConfig.showStaticPoints&&staticPoints.forEach(function(point){point.render()})}function areCoordsValid(x,y,cartesian){var gridBBox,snappedPoint,xOnOuter,yOnOuter;return cartesian?(xOnOuter=x===self.gridConfig.x.start||x===self.gridConfig.x.end,yOnOuter=y===self.gridConfig.y.start||y===self.gridConfig.y.end):(gridBBox=grid.getBBox(),
snappedPoint=grid.snap(x,y),xOnOuter=snappedPoint[0]===gridBBox.x||snappedPoint[0]===gridBBox.x2,yOnOuter=snappedPoint[1]===gridBBox.y||snappedPoint[1]===gridBBox.y2),!(self.gridConfig.x.allowOuter===!1&&xOnOuter||self.gridConfig.y.allowOuter===!1&&yOnOuter)}function addPoint(fx,fy,cartesian){var gridBBox,newPoint,pointConfig,draggableArea,subStepSizes;return grid&&areCoordsValid(fx,fy,cartesian)&&(gridBBox=grid.getBBox(),subStepSizes=grid.getSubStepSizes(),draggableArea={xMin:self.gridConfig.x.allowOuter?gridBBox.x:gridBBox.x+subStepSizes.x,xMax:self.gridConfig.x.allowOuter?gridBBox.x2:gridBBox.x2-subStepSizes.x,yMin:self.gridConfig.y.allowOuter?gridBBox.y:gridBBox.y+subStepSizes.y,
yMax:self.gridConfig.y.allowOuter?gridBBox.y2:gridBBox.y2-subStepSizes.y},pointConfig=_.defaults({x:fx,y:fy,xMin:draggableArea.xMin,xMax:draggableArea.xMax,yMin:draggableArea.yMin,yMax:draggableArea.yMax,on:{dragStart:clearPlot,dragStop:plot}},self.gridConfig.point),newPoint=pointFactory(paper,grid,pointConfig),cartesian&&newPoint.setCartesianCoord(fx,fy,pointConfig),points.push(newPoint)),newPoint}function addStaticPoint(fx,fy,label,cartesian){var newPoint,pointConfig;return grid&&areCoordsValid(fx,fy,cartesian)&&(pointConfig=_.defaults({x:fx,y:fy,label:label},self.gridConfig.staticPoint),newPoint=pointFactory(paper,grid,pointConfig),cartesian&&newPoint.setCartesianCoord(fx,fy,pointConfig),
staticPoints.push(newPoint)),newPoint}var self,paper,grid,plotFactory,staticPlotFactory,$container=$(dom),points=[],paths=[],staticPoints=[],staticPaths=[];event.addEventMgr(this),this.id=id,this.dom=dom,this.config=config||{},self=this,this.getRawResponse=function(){var serializedPoints;return 0===points.length?null:serializedPoints=points.map(function(point){var pointData=point.getCartesianCoord(1);return pointData.x+" "+pointData.y})},this.setRawResponse=function(serializedPoints){clearPoint(),clearPlot(),serializedPoints&&serializedPoints.forEach(function(pointData){var pointCoord=pointData.split(" ");addPoint(pointCoord[0],pointCoord[1],!0)}),plot()},this.gridConfig=buildGridConfig(this.config),
initGrid($container,this.gridConfig),self.on("configChange",function(newRawConfig){var state=self.getRawResponse();self.config=newRawConfig,self.gridConfig=buildGridConfig(newRawConfig),initGrid($container,self.gridConfig),self.setRawResponse(state)}),self.on("gridChange",function(newRawConfig){self.config=newRawConfig,self.gridConfig=buildGridConfig(newRawConfig),initGrid($container,self.gridConfig)})},setResponse:function(response){if(response&&response.list)if(response.list.string&&_.isArray(response.list.string))this.setRawResponse(response.list.string);else{if(!response.list.point||!_.isArray(response.list.point))throw new Error("invalid response baseType");
this.setRawResponse(response.list.point)}else response&&null===response.base&&this.setRawResponse([])},resetResponse:function(){},setSerializedState:function(state){this.setResponse(state)},getSerializedState:function(){return this.getResponse()}};qtiCustomInteractionContext.register(graphPointLineGraphInteraction)}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))}("/*add some style, need to scope (using the typeIdentifier as class prefix) every style declaration to prevent collision */\n\n.graphPointLineGraphInteraction {\n    width:100%;\n    padding:30px 30px 50px;\n    margin:20px 0;\n    padding:20px;\n    border : 6px solid #f5f3f2;\n}\n\n.graphPointLineGraphInteraction .shape-panel {position:relative; display: inline-block; width: 100%;}\n.graphPointLineGraphInteraction .shape-panel .shape-container {float:left; margin-left:30px;}\n"),
define(["graphPointLineGraphInteraction/runtime/graphPointLineGraphInteraction"],function(IMSPCI){return IMSPCI});
//# sourceMappingURL=graphPointLineGraphInteraction.min.js.map