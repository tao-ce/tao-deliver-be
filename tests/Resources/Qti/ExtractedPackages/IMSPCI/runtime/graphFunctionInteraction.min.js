...
if(sheet.href==link.href)return clearInterval(loadInterval),callback()}},10);link.href=url,head.appendChild(link)};return cssAPI.normalize=function(name,normalize){return".css"==name.substr(name.length-4,4)&&(name=name.substr(0,name.length-4)),normalize(name)},cssAPI.load=function(cssId,req,load,config){(useImportLoad?importLoad:linkLoad)(req.toUrl(cssId+".css"),load)},cssAPI}),define("css!parccTei/pciCreator/ims/graphFunctionInteraction/runtime/css/graphFunctionInteraction",[],function(){}),define("graphFunctionInteraction/runtime/graphFunctionInteraction",["taoQtiItem/portableLib/jquery_2_1_1","qtiCustomInteractionContext","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/OAT/util/event","taoQtiItem/portableLib/OAT/scale.raphael","parccTei/portableLib/gridFactory","parccTei/portableLib/pointFactory","parccTei/portableLib/plotFactory","css!parccTei/pciCreator/ims/graphFunctionInteraction/runtime/css/graphFunctionInteraction"],function($,qtiCustomInteractionContext,_,event,scaleRaphael,gridFactory,pointFactory,PlotFactory){
"use strict";function extractPointsFromString(stringPoints){var points=[];return _.each(stringPoints.split(/,/),function(strPoint){var pt=strPoint.trim().split(/\s+/);return 2===pt.length&&void points.push({x:pt[0],y:pt[1]})}),points}var graphFunctionInteraction,_typeIdentifier="graphFunctionInteraction",buildGridConfig=function(rawConfig){var getBoolean=function(value,defaultValue){return"undefined"==typeof value?defaultValue:value===!0||"true"===value},radix=10,xWeight=parseInt(rawConfig.xWeight,radix),xBorderWeight=parseInt(rawConfig.xBorderWeight,radix),yWeight=parseInt(rawConfig.yWeight,radix),yBorderWeight=parseInt(rawConfig.yBorderWeight,radix),gridConfig={
graphType:rawConfig.graphType,graphTitle:rawConfig.graphTitle,graphTitleRequired:getBoolean(rawConfig.graphTitleRequired,!1),weight:parseInt(rawConfig.weight,radix),width:parseInt(rawConfig.width,radix),height:parseInt(rawConfig.height,radix),x:{start:parseInt(rawConfig.xStart,radix),end:parseInt(rawConfig.xEnd,radix),label:rawConfig.xLabel,title:rawConfig.xTitle,step:parseInt(rawConfig.xStep,radix),subStep:parseInt(rawConfig.xSubStep,radix),weight:xWeight>0?xWeight:xBorderWeight,allowOuter:getBoolean(rawConfig.xAllowOuter,!0)},y:{start:-1*parseInt(rawConfig.yEnd,radix),end:-1*parseInt(rawConfig.yStart,radix),label:rawConfig.yLabel,title:rawConfig.yTitle,step:parseInt(rawConfig.yStep,radix),
subStep:parseInt(rawConfig.ySubStep,radix),weight:yWeight>0?yWeight:yBorderWeight,allowOuter:getBoolean(rawConfig.yAllowOuter,!0)},plot:{color:rawConfig.plotColor,thickness:parseInt(rawConfig.plotThickness,radix)},point:{color:rawConfig.pointColor,glow:getBoolean(rawConfig.pointGlow,!0),radius:parseInt(rawConfig.pointRadius,radix)}};return gridConfig.x.step<1&&(gridConfig.x.step=1),gridConfig.y.step<1&&(gridConfig.y.step=1),gridConfig.x.subStep<1&&(gridConfig.x.subStep=1),gridConfig.y.subStep<1&&(gridConfig.y.subStep=1),gridConfig.x.weight>0==!1&&(gridConfig.x.weight=3),gridConfig.y.weight>0==!1&&(gridConfig.y.weight=3),gridConfig},createCanvas=function($container,config){
var paperSize=gridFactory.getPaperSize(config),paper=scaleRaphael($(".shape-container",$container)[0],paperSize.width,paperSize.height);return paper};graphFunctionInteraction={typeIdentifier:_typeIdentifier,getInstance:function(dom,config,state){var response=config.boundTo;this.initialize(Object.getOwnPropertyNames(response).pop(),dom,config.properties),this.setSerializedState(state),"function"==typeof config.onready&&config.onready(this,this.getState())},getState:function(){return this.getSerializedState()},oncompleted:function(){this.destroy()},getResponse:function(){var raw=this.getRawResponse();return raw?{record:[{name:"functionGraphType",base:{string:raw.mathFunction
}},{name:"points",base:{string:raw.point1.x+" "+raw.point1.y+","+raw.point2.x+" "+raw.point2.y}}]}:{base:null}},destroy:function(){var $container=$(this.dom);$container.off().empty()},getTypeIdentifier:function(){return"graphFunctionInteraction"},initialize:function(id,dom,config){function initGrid($container,gridConfig){return clearPlot(),clearPoint(),paper=createCanvas($container,gridConfig),_.isObject(gridConfig.x)&&_.isObject(gridConfig.y)&&gridConfig.x.start<gridConfig.x.end&&gridConfig.y.start<gridConfig.y.end&&(grid=gridFactory(paper,gridConfig),grid.clickable(),grid.children.click(function(event){var oldPoint,bnds=event.target.getBoundingClientRect(),wfactor=paper.w/paper.width,fx=grid.getX()+Math.round((event.clientX-bnds.left)/bnds.width*grid.getWidth()*wfactor),fy=grid.getY()+Math.round((event.clientY-bnds.top)/bnds.height*grid.getHeight()*wfactor);
areCoordsValid(fx,fy)&&(points.length<2?(addPoint(fx,fy),2===points.length&&plot()):(oldPoint=points.pop(),oldPoint.setCoord(fx,fy),oldPoint.render(),oldPoint.drag(),points.push(oldPoint),plot()))}),plotFactory=new PlotFactory(grid,gridConfig.plot),$(paper.canvas).off("removed.point").on("removed.point",function(event,removedPoint){var pointToDelete=_.findIndex(points,{uid:removedPoint.uid});pointToDelete>-1&&(points.splice(pointToDelete,1),clearPlot())})),grid}function showControl(graphs){$shapeControls.children("button").removeClass("available"),_.each(graphs,function(graph){$shapeControls.find("[name="+graph+"]").addClass("available")}),activateButton($shapeControls.children(".available:first"));
}function activateButton($button,preventDefaultPlot){if("string"==typeof $button&&($button=$shapeControls.find("[name="+$button+"]")),$button.length){var fnName=$button.val();mathFunction=PlotFactory.getPlotName(fnName),$button.removeClass("btn-info").addClass("btn-success"),$button.siblings("button").removeClass("btn-success").addClass("btn-info"),preventDefaultPlot||plotDefault()}}function clearPlot(){path&&path.remove()}function clearPoint(){_.each(points,function(point){point.remove()}),points=[]}function plot(){var point1=points[0],point2=points[1];point1&&point2&&mathFunction&&plotFactory[mathFunction]&&(clearPlot(),path=plotFactory[mathFunction](point1,point2),
self.trigger("responseChange",[self.getResponse()]))}function areCoordsValid(x,y,cartesian){var gridBBox,snappedPoint,xOnOuter,yOnOuter;return cartesian?(xOnOuter=x===self.gridConfig.x.start||x===self.gridConfig.x.end,yOnOuter=y===self.gridConfig.y.start||y===self.gridConfig.y.end):(gridBBox=grid.getBBox(),snappedPoint=grid.snap(x,y),xOnOuter=snappedPoint[0]===gridBBox.x||snappedPoint[0]===gridBBox.x2,yOnOuter=snappedPoint[1]===gridBBox.y||snappedPoint[1]===gridBBox.y2),!(self.gridConfig.x.allowOuter===!1&&xOnOuter||self.gridConfig.y.allowOuter===!1&&yOnOuter)}function addPoint(fx,fy,cartesian){var gridBBox,newPoint,pointConfig,draggableArea,subStepSizes;return grid&&areCoordsValid(fx,fy,cartesian)&&(gridBBox=grid.getBBox(),
subStepSizes=grid.getSubStepSizes(),draggableArea={xMin:self.gridConfig.x.allowOuter?gridBBox.x:gridBBox.x+subStepSizes.x,xMax:self.gridConfig.x.allowOuter?gridBBox.x2:gridBBox.x2-subStepSizes.x,yMin:self.gridConfig.y.allowOuter?gridBBox.y:gridBBox.y+subStepSizes.y,yMax:self.gridConfig.y.allowOuter?gridBBox.y2:gridBBox.y2-subStepSizes.y},pointConfig=_.defaults({x:fx,y:fy,xMin:draggableArea.xMin,xMax:draggableArea.xMax,yMin:draggableArea.yMin,yMax:draggableArea.yMax,on:{dragStart:clearPlot,dragStop:plot}},self.gridConfig.point),newPoint=pointFactory(paper,grid,pointConfig),cartesian&&newPoint.setCartesianCoord(fx,fy,pointConfig),newPoint.render(),newPoint.drag(),
points.push(newPoint)),newPoint}function getGraphOrigin(){var x0,y0,_y=self.gridConfig.y,_x=self.gridConfig.x;return y0=_y.start<0&&_y.end<=0?_y.start:_y.start>=0&&_y.end>0?_y.start:0,x0=_x.start<0&&_x.end<=0?_x.start:_x.start>=0&&_x.end>0?_x.start:0,{x:x0,y:y0}}function plotDefault(){clearPoint(),clearPlot();var origin=getGraphOrigin();"plotExponential"===mathFunction||"plotLogarithmic"===mathFunction?(addPoint(origin.x+1,origin.y+1,!0),addPoint(origin.x+2,origin.y+2,!0)):mathFunction&&(addPoint(origin.x+0,origin.y+0,!0),addPoint(origin.x+1,origin.y+1,!0)),mathFunction&&plot()}event.addEventMgr(this),this.id=id,this.dom=dom,this.config=config||{};var paper,grid,plotFactory,path,mathFunction,$container=$(dom),mathFunctions=config.graphs.split(","),$shapeControls=$container.find(".shape-controls"),self=this,points=[];
this.getRawResponse=function(){var point1=points[0],point2=points[1];if(point1&&point2&&mathFunction)return{point1:point1.getCartesianCoord(1),point2:point2.getCartesianCoord(1),mathFunction:mathFunction.replace("plot","").toLowerCase()}},this.setRawResponse=function(mathFn,point1,point2){clearPoint(),clearPlot(),mathFunction=mathFn||mathFunctions[0],activateButton(mathFunction,!0),point1&&point1.x&&point1.y&&addPoint(point1.x,point1.y,!0),point2&&point2.x&&point2.y&&addPoint(point2.x,point2.y,!0),plot()},this.gridConfig=buildGridConfig(this.config),initGrid($container,this.gridConfig),showControl(mathFunctions),plotDefault(),$shapeControls.on("click","button",function(){
activateButton($(this))}),self.on("configChange",function(newRawConfig){var state=self.getRawResponse();self.config=newRawConfig,self.gridConfig=buildGridConfig(newRawConfig),initGrid($container,self.gridConfig),self.setRawResponse(state)}),self.on("gridChange",function(newRawConfig){self.config=newRawConfig,self.gridConfig=buildGridConfig(newRawConfig),initGrid($container,self.gridConfig),plotDefault()}),self.on("functionsChange",function(graphs){mathFunction=null,mathFunctions=graphs,showControl(mathFunctions),plotDefault()})},setResponse:function(response){var points;response&&_.isArray(response.record)&&response.record[0]&&response.record[1]&&"functionGraphType"===response.record[0].name&&response.record[0].base&&response.record[0].base.string&&"points"===response.record[1].name&&response.record[1].base&&response.record[1].base.string&&(points=extractPointsFromString(response.record[1].base.string),
this.setRawResponse(response.record[0].base.string,points.shift(),points.shift()))},resetResponse:function(){this.setRawResponse()},setSerializedState:function(state){this.setResponse(state)},getSerializedState:function(){return this.getResponse()}},qtiCustomInteractionContext.register(graphFunctionInteraction)}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))}("/*add some style, need to scope (using the typeIdentifier as class prefix) every style declaration to prevent collision */\n\n.graphFunctionInteraction {\n    width:100%;\n    padding:30px 30px 50px;\n    margin:20px 0;\n    padding:20px;\n    border : 6px solid #f5f3f2;\n}\n\n.graphFunctionInteraction .prompt {margin-bottom:20px;}\n.graphFunctionInteraction .shape-panel {position:relative; display: inline-block; width: 100%;}\n.graphFunctionInteraction .shape-panel .shape-container {float:left; margin-left:30px;}\n.graphFunctionInteraction .shape-panel .shape-controls {float:left; margin-bottom:20px;}\n.graphFunctionInteraction .shape-panel .shape-controls button {width:150px; margin:3px 0; display:none;}\n.graphFunctionInteraction .shape-panel .shape-controls button.available {display:block;}"),
define(["graphFunctionInteraction/runtime/graphFunctionInteraction"],function(IMSPCI){return IMSPCI});
//# sourceMappingURL=graphFunctionInteraction.min.js.map