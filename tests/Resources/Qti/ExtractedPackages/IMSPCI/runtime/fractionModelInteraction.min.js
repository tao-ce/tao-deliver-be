...
clipping&&(nestedWrapper.style.left=parseInt((w-newWidth)/2)+"px",nestedWrapper.style.top=parseInt((h-newHeight)/2)+"px"),vmlDiv.style.overflow="visible"}clipping&&(newWidth=w,newHeight=h),wrapper.style.width=newWidth+"px",wrapper.style.height=newHeight+"px",paper.setSize(newWidth,newHeight),center&&(wrapper.style.position="absolute",wrapper.style.left=parseInt((w-newWidth)/2)+"px",wrapper.style.top=parseInt((h-newHeight)/2)+"px")},paper.scaleAll=function(amount){paper.changeSize(width*amount,height*amount)},paper.changeSize(width,height),paper.w=width,paper.h=height,paper};return scaleRaphael}),define("parccTei/pciCreator/ims/fractionModelInteraction/runtime/libs/pieChart",["taoQtiItem/portableLib/raphael","taoQtiItem/portableLib/lodash"],function(Raphael,_){
"use strict";function getTotalSelected(selectedPartitions){return _.compact(_.values(selectedPartitions)).length}Raphael.fn.pieChart=function(selectedPartitions,config,$container){function sector(cx,cy,r,startAngle,endAngle,params){var x1=cx+r*Math.cos(-startAngle*rad),x2=cx+r*Math.cos(-endAngle*rad),y1=cy+r*Math.sin(-startAngle*rad),y2=cy+r*Math.sin(-endAngle*rad);return paper.path(["M",cx,cy,"L",x1,y1,"A",r,r,0,+(endAngle-startAngle>180),0,x2,y2,"z"]).attr(params)}function drawSlice(j){var p,selected=selectedPartitions[j];selectedPartitions[j]=selected,p=sector(cx,cy,r,angle,angle+angleplus,getStyle(selected)),angle+=angleplus,setHoverStyle(p),chart.push(p),
p.click(getClickCallback(j))}function drawCircle(){var c=paper.circle(cx,cy,r).attr(getStyle(!1));setHoverStyle(c),chart.push(c),c.click(getClickCallback(0))}function getClickCallback(j){return function(){String(this.attrs.fill)===config.partitionColor?(this.attr(getStyle(!0)),selectedPartitions[j]=!0,$container.trigger("select_slice.pieChart",[selectedPartitions,getTotalSelected(selectedPartitions)])):(this.attr(getStyle(!1)),selectedPartitions[j]=!1,$container.trigger("unselect_slice.pieChart",[selectedPartitions,getTotalSelected(selectedPartitions)]))}}var paper=this,chart=this.set(),padding=config.padding||2,r=parseInt(config.radius),cx=r+padding,cy=r+padding,rad=Math.PI/180,getStyle=function(selected){
return{fill:selected?config.selectedPartitionsColor:config.partitionColor,cursor:"pointer",stroke:config.outlineColor,"stroke-width":config.outlineThickness}},setHoverStyle=function(raphaelObject){raphaelObject.hover(function(){this.animate({"fill-opacity":.7})},function(){this.stop().attr({"fill-opacity":1})})};selectedPartitions=_.clone(selectedPartitions);var angle=0,angleplus=360/selectedPartitions.length;if(selectedPartitions.length>1)for(var i=0;i<selectedPartitions.length;i++)drawSlice(i);else drawCircle();$container.trigger("drawn.pieChart")}}),define("normalize",[],function(){function convertURIBase(uri,fromBase,toBase){if(uri.match(absUrlRegEx)||uri.match(protocolRegEx))return uri;
uri=removeDoubleSlashes(uri);var toBaseProtocol=toBase.match(protocolRegEx),fromBaseProtocol=fromBase.match(protocolRegEx);return!fromBaseProtocol||toBaseProtocol&&toBaseProtocol[1]==fromBaseProtocol[1]&&toBaseProtocol[2]==fromBaseProtocol[2]?relativeURI(absoluteURI(uri,fromBase),toBase):absoluteURI(uri,fromBase)}function absoluteURI(uri,base){if("./"==uri.substr(0,2)&&(uri=uri.substr(2)),uri.match(absUrlRegEx)||uri.match(protocolRegEx))return uri;var baseParts=base.split("/"),uriParts=uri.split("/");for(baseParts.pop();curPart=uriParts.shift();)".."==curPart?baseParts.pop():baseParts.push(curPart);return baseParts.join("/")}function relativeURI(uri,base){var baseParts=base.split("/");
for(baseParts.pop(),base=baseParts.join("/")+"/",i=0;base.substr(i,1)==uri.substr(i,1);)i++;for(;"/"!=base.substr(i,1);)i--;base=base.substr(i+1),uri=uri.substr(i+1),baseParts=base.split("/");var uriParts=uri.split("/");for(out="";baseParts.shift();)out+="../";for(;curPart=uriParts.shift();)out+=curPart+"/";return out.substr(0,out.length-1)}var slashes=/([^:])\/+/g,removeDoubleSlashes=function(uri){return uri.replace(slashes,"$1/")},protocolRegEx=/[^\:\/]*:\/\/([^\/])*/,absUrlRegEx=/^(\/|data:)/,normalizeCSS=function(source,fromBase,toBase){fromBase=removeDoubleSlashes(fromBase),toBase=removeDoubleSlashes(toBase);var source;return source};return normalizeCSS.convertURIBase=convertURIBase,
normalizeCSS.absoluteURI=absoluteURI,normalizeCSS.relativeURI=relativeURI,normalizeCSS}),define("css",[],function(){if("undefined"==typeof window)return{load:function(n,r,load){load()}};var head=document.getElementsByTagName("head")[0],engine=window.navigator.userAgent.match(/Trident\/([^ ;]*)|AppleWebKit\/([^ ;]*)|Opera\/([^ ;]*)|rv\:([^ ;]*)(.*?)Gecko\/([^ ;]*)|MSIE\s([^ ;]*)|AndroidWebKit\/([^ ;]*)/)||0,useImportLoad=!1,useOnload=!0;engine[1]||engine[7]?useImportLoad=parseInt(engine[1])<6||parseInt(engine[7])<=9:engine[2]||engine[8]?useOnload=!1:engine[4]&&(useImportLoad=parseInt(engine[4])<18);var cssAPI={};cssAPI.pluginBuilder="./css-builder";var curStyle,curSheet,ieCurCallback,createStyle=function(){
curStyle=document.createElement("style"),head.appendChild(curStyle),curSheet=curStyle.styleSheet||curStyle.sheet},ieCnt=0,ieLoads=[],createIeLoad=function(url){ieCnt++,32==ieCnt&&(createStyle(),ieCnt=0),curSheet.addImport(url),curStyle.onload=function(){processIeLoad()}},processIeLoad=function(){ieCurCallback();var nextLoad=ieLoads.shift();return nextLoad?(ieCurCallback=nextLoad[1],void createIeLoad(nextLoad[0])):void(ieCurCallback=null)},importLoad=function(url,callback){if(curSheet&&curSheet.addImport||createStyle(),curSheet&&curSheet.addImport)ieCurCallback?ieLoads.push([url,callback]):(createIeLoad(url),ieCurCallback=callback);else{curStyle.textContent='@import "'+url+'";';
...
normalize(name)},cssAPI.load=function(cssId,req,load,config){(useImportLoad?importLoad:linkLoad)(req.toUrl(cssId+".css"),load)},cssAPI}),define("css!parccTei/pciCreator/ims/fractionModelInteraction/runtime/css/fractionModelInteraction",[],function(){}),define("fractionModelInteraction/runtime/fractionModelInteraction",["taoQtiItem/portableLib/jquery_2_1_1","qtiCustomInteractionContext","taoQtiItem/portableLib/OAT/util/event","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/OAT/scale.raphael","parccTei/pciCreator/ims/fractionModelInteraction/runtime/libs/pieChart","css!parccTei/pciCreator/ims/fractionModelInteraction/runtime/css/fractionModelInteraction"],function($,qtiCustomInteractionContext,event,_,scaleRaphael){
"use strict";function createCanvas($container,config){config=_.defaults(config,_defaults);var canvasSize=2*(parseInt(config.radius)+parseInt(config.padding)+parseInt(config.outlineThickness)),canvas=scaleRaphael($(".shape-container",$container)[0],canvasSize,canvasSize);return canvas}var _defaults={radius:100,padding:10,outlineThickness:1},_typeIdentifier="graphLineAndPointInteraction",fractionModelInteraction={typeIdentifier:_typeIdentifier,getInstance:function(dom,config,state){var response=config.boundTo;this.initialize(Object.getOwnPropertyNames(response).pop(),dom,config.properties),this.setSerializedState(state),"function"==typeof config.onready&&config.onready(this,this.getSelectedPartitions());
},getState:function(){return this.getSerializedState()},oncompleted:function(){this.destroy()},getResponse:function(){var selection=_.values(this.getSelectedPartitions()),numerator=_.filter(selection).length,denominator=selection.length;return{base:{string:numerator+"/"+denominator}}},destroy:function(){var $container=$(this.dom);$container.off().empty()},getTypeIdentifier:function(){return"fractionModelInteraction"},initialize:function(id,dom,config){event.addEventMgr(this),this.id=id,this.dom=dom,this.config=config||{};var numerator,denominator,selectedPartitions,_this=this,$container=$(dom);this.setFractionModel=function(num,den){numerator=parseInt(num),denominator=parseInt(den);
},this.getFractionModel=function(){return{numerator:numerator,denominator:denominator}},this.setState=function(stateData){var i;if(_.isString(stateData))""===stateData.trim()||"[]"===stateData.trim()?selectedPartitions=_.range(denominator).map(function(i){return i<numerator}):(selectedPartitions=JSON.parse(stateData),denominator=selectedPartitions.length);else{if(!_.isArray(stateData))throw"invalid format for selectedPartitions";if(stateData.length)selectedPartitions=stateData,denominator=selectedPartitions.length;else for(selectedPartitions=[],i=0;i<denominator;i++)selectedPartitions.push(!1)}},this.getSelectedPartitions=function(){return _.clone(selectedPartitions);
},this.setFractionModel(this.config.selectedPartitionsInit,this.config.partitionInit);var canvas=createCanvas($container,config);this.setState(config.selectedPartitions),canvas.pieChart(this.getSelectedPartitions(),this.config,$container),$container.on("click.fraction","button.more",function(){if(denominator<_this.config.partitionMax){denominator+=1;var newState=_this.getSelectedPartitions();newState.push(!1),_this.setState(newState),$container.trigger("statechange.fraction")}}).on("click.fraction","button.fewer",function(){if(denominator>_this.config.partitionMin){denominator-=1;var newState=_this.getSelectedPartitions();newState.pop(),_this.setState(newState),
$container.trigger("statechange.fraction")}}).on("click.fraction","button.reset",function(){_this.setFractionModel(_this.config.selectedPartitionsInit,_this.config.partitionInit),_this.setState(_this.config.selectedPartitions),$container.trigger("reset.fraction")}).on("statechange.fraction reset.fraction",function(event){canvas.clear(),canvas.pieChart(_this.getSelectedPartitions(),_this.config,$container),_this.trigger("changepartition",[_this.getSelectedPartitions()]),_this.trigger("responseChange",[_this.getResponse()])}).on("select_slice.pieChart unselect_slice.pieChart",function(e,selectedPartitions,totalSelected){numerator=totalSelected,_this.setState(selectedPartitions),
_this.trigger("selectedpartition",[selectedPartitions]),_this.trigger("responseChange",[_this.getResponse()])}),this.on("configchange",function(config){_this.config=config,_this.setFractionModel(_this.config.selectedPartitionsInit,_this.config.partitionInit),_this.setState(_this.config.selectedPartitions),canvas.clear(),canvas=createCanvas($container,_this.config),canvas.pieChart(_this.getSelectedPartitions(),_this.config,$container)})},setResponse:function(response){var i,parts,numerator,denominator,state=[],value="0";if(response&&response.base&&response.base.string){for(value=response.base.string,parts=value.split("/"),numerator=parseInt(parts[0]),denominator=parseInt(parts[1]||0),
i=0;i<denominator;i++)state.push(i<numerator);this.setState(state),$(this.dom).trigger("statechange.fraction")}},resetResponse:function(){this.setFractionModel(this.config.selectedPartitionsInit,this.config.partitionInit),this.setState(this.config.selectedPartitions),$(this.dom).trigger("statechange.fraction")},setSerializedState:function(state){state&&_.isArray(state.selection)&&(this.setState(state.selection),$(this.dom).trigger("statechange.fraction"))},getSerializedState:function(){return{selection:_.values(this.getSelectedPartitions())}}};qtiCustomInteractionContext.register(fractionModelInteraction)}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");
s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))}("/*add some style, need to scope (using the typeIdentifier as class prefix) every style declaration to prevent collision */\n\n.fractionModelInteraction {width:100%;}\n\n.fractionModelInteraction .likert-label {min-width:90px; color: #444;}\n.fractionModelInteraction .likert-label-min {text-align: right;}\n.fractionModelInteraction .likert-label-max {text-align: left;}\n\n.fractionModelInteraction .shape-title {margin-top: 10px; margin-right : auto; margin-left : auto; width : 50%; text-align : center;}\n.fractionModelInteraction .shape-panel {padding: 10px 10px 20px; background : #EFEFEF;}\n.fractionModelInteraction .shape-container {margin:auto;}\n.fractionModelInteraction .shape-controls {text-align: center;}\n.fractionModelInteraction .shape-controls button {margin: 0 5px; padding: 0 20px;}\n"),
define(["fractionModelInteraction/runtime/fractionModelInteraction"],function(IMSPCI){return IMSPCI});
//# sourceMappingURL=fractionModelInteraction.min.js.map