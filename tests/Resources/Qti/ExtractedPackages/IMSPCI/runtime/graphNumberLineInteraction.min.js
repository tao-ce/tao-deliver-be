...
clipping&&(nestedWrapper.style.left=parseInt((w-newWidth)/2)+"px",nestedWrapper.style.top=parseInt((h-newHeight)/2)+"px"),vmlDiv.style.overflow="visible"}clipping&&(newWidth=w,newHeight=h),wrapper.style.width=newWidth+"px",wrapper.style.height=newHeight+"px",paper.setSize(newWidth,newHeight),center&&(wrapper.style.position="absolute",wrapper.style.left=parseInt((w-newWidth)/2)+"px",wrapper.style.top=parseInt((h-newHeight)/2)+"px")},paper.scaleAll=function(amount){paper.changeSize(width*amount,height*amount)},paper.changeSize(width,height),paper.w=width,paper.h=height,paper};return scaleRaphael}),define("parccTei/portableLib/pointFactory",["taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/lodash"],function($,_){
"use strict";function pointFactory(paper,grid,options){options=_.defaults(options||{},_defaults);var _color=options.color,_r=parseInt(options.radius),_x=0,_y=1,_label="",_rGlow=parseInt(options.glowRadius)||3*_r,_events=options.on||{},_dragEnabled=!1,obj={children:paper.set(),uid:_.uniqueId(),setColor:function(color){_color=String(color)},setX:function(val){_x=parseInt(val)},getX:function(){return _x},getLabel:function(){return _label},getY:function(){return _y},setY:function(val){_y=parseInt(val)},setLabel:function(val){_label=val},setCartesianCoord:function(x,y){_x=grid.getOriginPosition().left+grid.getUnits().x*x,_y=grid.getOriginPosition().top-grid.getUnits().y*y;
},getCartesianCoord:function(){var cartesianX=(_x-grid.getOriginPosition().left)/grid.getUnits().x,cartesianY=-(_y-grid.getOriginPosition().top)/grid.getUnits().y;return{x:Number(cartesianX.toFixed(2)),y:Number(cartesianY.toFixed(2))}},setCoord:function(x,y){var coord=grid.snap(parseInt(x),parseInt(y));_x=coord[0],_y=coord[1]},setR:function(val){_r=parseInt(val)},setRGlow:function(val){_rGlow=parseInt(val)},render:function(){this.remove();var circle=paper.circle(_x,_y,_r);if(options.fill?circle.attr({fill:_color,stroke:"#000",cursor:"move"}):circle.attr({stroke:_color,"stroke-width":3,cursor:"move"}),this.children.push(circle),options.glow&&this.showGlow(),_label){
var text=paper.text(_x,_y,_label);text.attr({fill:options.labelColor||"#000",stroke:options.labelColor||"#000","stroke-width":options.labelWeight||0,"font-size":options.labelSize||10}),this.children.push(text)}},remove:function(){this.children.length>0&&this.children.remove().clear()},removeOnClic:function(){var self=this;this.children.click(function(){options.removable&&_dragEnabled===!1&&(self.remove(),$(paper.canvas).trigger("removed.point",self))})},drag:function(){var bb,self=this,moved=!1;_dragEnabled=!0,this.children.drag(function(dx,dy){moved=!0,bb=self.children.getBBox();var newX=self.oBB.x-bb.x+dx,newY=self.oBB.y-bb.y+dy;"x"===options.axis?newY=self.getY()-(bb.y+bb.width/2):"y"===options.axis&&(newX=self.getX()-(bb.x+bb.width/2));
var absoluteX=bb.x+bb.width/2,absoluteY=bb.y+bb.height/2;options.xMin&&absoluteX+newX<options.xMin?newX=options.xMin-absoluteX:options.xMax&&absoluteX+newX>options.xMax&&(newX=options.xMax-absoluteX),options.yMin&&absoluteY+newY<options.yMin?newY=options.yMin-absoluteY:options.yMax&&absoluteY+newY>options.yMax&&(newY=options.yMax-absoluteY),"function"==typeof _events.drag&&_events.drag.call(self,newX,newY),self.children.translate(newX,newY)},function(){moved=!1,"function"==typeof _events.dragStart&&_events.dragStart.call(self),self.oBB=self.children.getBBox()},function(){if(moved){var newX=bb.x+bb.width/2,newY=bb.y+bb.height/2;self.setCoord(newX,newY),self.children.translate(parseInt(self.getX()-newX),parseInt(self.getY()-newY)),
"function"==typeof _events.dragStop&&_events.dragStop.call(self,self.getX(),self.getY()),$(paper.canvas).trigger("moved.point",self)}else options.removable&&(self.remove(),$(paper.canvas).trigger("removed.point",self))})},unDrag:function(){this.children.undrag(),_dragEnabled=!1},showGlow:function(){var rgb=paper.raphael.color(_color),glow=paper.circle(_x,_y,_rGlow).attr({fill:"rgba("+rgb.r+","+rgb.g+","+rgb.b+", "+options.glowOpacity+" )",stroke:"none",cursor:"move"});this.children.length>1&&this.children.pop().remove(),this.children.push(glow),this.children.attr("cursor","move")},hideGlow:function(){this.children.length>1&&this.children.pop().remove(),this.children.attr("cursor","default");
},setOption:function(key,value){options[key]=value}};return options.cartesian?obj.setCartesianCoord(options.x,options.y):obj.setCoord(options.x,options.y),obj.setLabel(options.label),obj}var _defaults={x:0,y:0,color:"#f00",radius:10,glow:!0,glowRadius:0,glowOpacity:.3,fill:!0,removable:!0,cartesian:!1};return pointFactory}),define("parccTei/portableLib/axisFactory",["taoQtiItem/portableLib/lodash"],function(_){"use strict";function getArrowsPath(orientation,config){var tipPosition,arrowConfig={length:.7*config.unitSize,tip:.3*config.unitSize,orientation:orientation},axisSizePx=(config.max-config.min)*config.unitSize,path="";if("right"===arrowConfig.orientation)tipPosition=config.left+axisSizePx+arrowConfig.length,
path+="M"+(config.left+axisSizePx)+","+config.top,path+="L"+tipPosition+","+config.top,path+="M"+(tipPosition-arrowConfig.tip)+","+(config.top+config.divisionWidth),path+="L"+tipPosition+","+config.top,path+="M"+(tipPosition-arrowConfig.tip)+","+(config.top-config.divisionWidth),path+="L"+tipPosition+","+config.top;else{if("left"!==arrowConfig.orientation)throw"unknown orientation type to the arrow";tipPosition=config.left-arrowConfig.length,path+="M"+config.left+","+config.top,path+="L"+tipPosition+","+config.top,path+="M"+(tipPosition+arrowConfig.tip)+","+(config.top+config.divisionWidth),path+="L"+tipPosition+","+config.top,path+="M"+(tipPosition+arrowConfig.tip)+","+(config.top-config.divisionWidth),
path+="L"+tipPosition+","+config.top}return path}function _applyStyle(path,config){path.attr({stroke:config.color?config.color:_defaults.color,"stroke-width":config.thickness?config.thickness:_defaults.thickness,opacity:config.opacity?config.opacity:_defaults.opacity})}function addCssClass(raphaelObj,newClass){var pattern=new RegExp("\\b"+newClass+"\\b"),oldClass=raphaelObj.node.getAttribute("class")||"";raphaelObj.node.setAttribute("class",pattern.test(oldClass)?oldClass:oldClass+" "+newClass)}function getLabel(i,config){return config&&_.isArray(config.labels)&&void 0!==config.labels[i]&&config.labels[i]!==!1?config.labels[i]:i}function axisFactory(paper,config){
config=_.defaults(config||{},_defaults);var arrowRight,arrowLeft,set=paper.set(),steps=[],obj={getMin:function(){return config.min},getMax:function(){return config.max},getCanvas:function(){return paper},getOriginPosition:function(){return{left:config.left-config.unitSize*config.min,top:config.top}},snap:function(x0){var step=config.unitSize/config.unitSubDivision,x=paper.raphael.snapTo(steps,x0,step/2),y=config.top;return[x,y]},render:function(){this.clear();var path="M"+config.left+","+config.top,axisSizePx=(config.max-config.min)*config.unitSize,subDivisionSize=config.unitSize/config.unitSubDivision;path+="L"+(config.left+axisSizePx)+","+config.top;var position=config.left;
steps.push(position);for(var i=config.min;i<=config.max;i++){path+="M"+position+","+(config.top-config.divisionWidth),path+="L"+position+","+(config.top+config.divisionWidth);var label=paper.text(position,config.top-config.divisionWidth-config.fontSize/2-5,getLabel(i,config));if(label.attr({"font-size":config.fontSize}),addCssClass(label,"scene scene-text"),set.push(label),i<config.max){var subPosition=position+subDivisionSize;steps.push(subPosition);for(var j=config.unitSubDivision;j>1;j--)path+="M"+subPosition+","+(config.top-config.subDivisionWidth),path+="L"+subPosition+","+(config.top+config.subDivisionWidth),subPosition+=subDivisionSize,j!==config.unitSubDivision&&steps.push(subPosition);
}position+=config.unitSize,steps.push(position)}config.arrows&&(arrowRight=this.buildArrow("right"),arrowLeft=this.buildArrow("left"));var pathObj=paper.path(path);addCssClass(pathObj,"scene scene-grid"),_applyStyle(pathObj,config),set.push(pathObj)},getSet:function(){return set},isRendered:function(){return set.length},clear:function(){set.length>0&&(steps=[],set.remove().clear())},clickable:function(){var borderBox=set.getBBox(),clickableArea=paper.rect(borderBox.x,borderBox.y,borderBox.width,borderBox.height);return clickableArea.attr({fill:"rgba(0,0,0,0)",stroke:0}),set.push(clickableArea),clickableArea},getWidth:function(){return set.getBBox().width},getHeight:function(){
return set.getBBox().height},getUnits:function(){return{x:config.unitSize,y:0}},coordinateToPosition:function(x){return{left:this.getOriginPosition().left+this.getUnits().x*x,top:this.getOriginPosition().top}},buildArrow:function(orientation){var pathStr=getArrowsPath(orientation,config),arrow=paper.path(pathStr);return addCssClass(arrow,"scene scene-grid"),_applyStyle(arrow,config),set.push(arrow),arrow},getConfig:function(){return _.clone(config)},setConfig:function(key,value){config[key]=value},buildContainerBox:function(options){var _defaults={padding:20,absolute:!0,shadow:!1};options=_.defaults(options||{},_defaults);var box,padding=options.padding,bb=set.getBBox();
if(options.absolute)box=paper.rect(bb.x-padding,bb.y-padding,bb.width+2*padding,bb.height+2*padding);else{var axisSizePx=(config.max-config.min)*config.unitSize;box=paper.rect(config.left-padding,config.top-padding,axisSizePx+2*padding,config.divisionWidth+2*padding)}return box.attr({fill:"#fff"}),set.push(box),box.insertBefore(set),options&&options.shadow&&set.push(box.glow({width:5,fill:!0,offsetx:3,offsety:3,color:"#999"})),box},addCssClass:addCssClass};return obj.render(),obj}var _defaults={top:40,left:40,min:-5,max:5,labels:!1,thickness:2,color:"#000",divisionWidth:16,subDivisionWidth:8,unitSubDivision:2,unitSize:50,fontSize:18,arrows:!1,opacity:1};return axisFactory;
}),define("parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/libs/intervalFactory",["taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/lodash","parccTei/portableLib/pointFactory"],function($,_,pointFactory){function IntervalFactory(axis,config){function _applyStyle(path){path.attr({stroke:config.color,"stroke-width":config.thickness})}function getPosition(x,cartesian){var position;return position=cartesian?axis.coordinateToPosition(x):{top:axis.getOriginPosition().top,left:x}}function drawLine(position1,position2){var startPosition,endPosition;position1.left<position2.left?(startPosition=position1,endPosition=position2):(startPosition=position2,
endPosition=position1);var pathStr="M"+(startPosition.left+config.offset)+","+startPosition.top;pathStr+="L"+(endPosition.left-config.offset)+","+endPosition.top;var path=paper.path(pathStr);return _applyStyle(path),set.push(path),path}function drawArrow(orientation){var arrow=axis.buildArrow(orientation);return set.push(arrow),_applyStyle(arrow),arrow}function buildPoint(position,fill,drag,dragStop){var top=axis.getOriginPosition().top,pointConfig={axis:"x",glow:!0,fill:!!fill,color:config.color,glowOpacity:.1,removable:!1,on:{drag:drag||_.noop,dragStop:dragStop||_.noop}};pointConfig=_.defaults(pointConfig,{});var point=pointFactory(paper,axis,pointConfig);return point.setCartesianCoord(position,top,pointConfig),
point.render(),set.push(point.children),point}function buildFiniteInterval(min,max){function _drawLine(){line&&line.remove(),line=drawLine(start,end),set.push(line)}function enable(){active||(pointMin.showGlow(),pointMax.showGlow(),pointMin.drag(),pointMax.drag(),active=!0)}function disable(){active&&(pointMin.hideGlow(),pointMax.hideGlow(),pointMin.unDrag(),pointMax.unDrag(),active=!1)}function destroy(){set.remove().clear()}function getCoordinates(){return _.clone(coord)}var line,start=getPosition(min.coord,!0),end=getPosition(max.coord,!0),set=paper.set(),active=!1,coord={start:min.coord,end:max.coord},_interval={enable:enable,disable:disable,destroy:destroy,
getCoordinates:getCoordinates},pointMin=buildPoint(min.coord,!min.open,function(dx){start.left+=dx,_drawLine()},function(x){start.left=x,_drawLine(),pointMax.setOption("xMin",x+.5*axis.getUnits().x),coord.start=this.getCartesianCoord().x,$canvas.trigger("change.interval",[_interval])});pointMin.setOption("xMin",getPosition(axis.getMin(),!0).left),pointMin.setOption("xMax",getPosition(max.coord-.5,!0).left),set.push(pointMin.children);var pointMax=buildPoint(max.coord,!max.open,function(dx){end.left+=dx,_drawLine()},function(x){end.left=x,_drawLine(),pointMin.setOption("xMax",x-.5*axis.getUnits().x),coord.end=this.getCartesianCoord().x,$canvas.trigger("change.interval",[_interval]);
});return pointMax.setOption("xMin",getPosition(min.coord+.5,!0).left),pointMax.setOption("xMax",getPosition(axis.getMax(),!0).left),set.push(pointMax.children),_drawLine(),enable(),_interval}function buildInfiniteInterval(pt,orientation){function _drawLine(){line&&line.remove(),line=drawLine(pos,tip),set.push(line)}function enable(){active||(point.showGlow(),point.drag(),active=!0)}function disable(){active&&(point.hideGlow(),point.unDrag(),active=!1)}function destroy(){set.remove().clear()}function getCoordinates(){return{start:right?coord:null,end:right?null:coord}}var line,pos=getPosition(pt.coord,!0),right="right"===orientation,tip=getPosition(right?axis.getMax()+.5:axis.getMin()-.5,!0),coord=pt.coord,_interval={
enable:enable,disable:disable,destroy:destroy,getCoordinates:getCoordinates},set=paper.set(),active=!1,arrow=drawArrow(right?"right":"left"),point=buildPoint(pt.coord,!pt.open,function(dx){pos.left+=dx,_drawLine()},function(x){pos.left=x,_drawLine(),coord=this.getCartesianCoord().x,$canvas.trigger("change.interval",[_interval])});return set.push(arrow),set.push(point.children),_drawLine(),enable(),_interval}var config=_.defaults(config||{},_defaults),paper=axis.getCanvas(),set=paper.set(),$canvas=$(paper.canvas),plots={"closed-closed":function(min,max){return buildFiniteInterval({coord:min,open:!1},{coord:max,open:!1})},"closed-open":function(min,max){return buildFiniteInterval({
coord:min,open:!1},{coord:max,open:!0})},"open-closed":function(min,max){return buildFiniteInterval({coord:min,open:!0},{coord:max,open:!1})},"open-open":function(min,max){return buildFiniteInterval({coord:min,open:!0},{coord:max,open:!0})},"arrow-open":function(max){return buildInfiniteInterval({coord:max,open:!0},"left")},"arrow-closed":function(max){return buildInfiniteInterval({coord:max,open:!1},"left")},"closed-arrow":function(min){return buildInfiniteInterval({coord:min,open:!1},"right")},"open-arrow":function(min){return buildInfiniteInterval({coord:min,open:!0},"right")}};this.plot=function(intervalType,min,max){return plots[intervalType](min,max)},this.clear=function(){
set.length>0&&set.remove().clear()}}var _defaults={color:"#00f",thickness:5,offset:7};return IntervalFactory}),define("handlebars",[],function(){var Handlebars=function(){var __module4__=function(){"use strict";function SafeString(string){this.string=string}var __exports__;return SafeString.prototype.toString=function(){return""+this.string},__exports__=SafeString}(),__module3__=function(__dependency1__){"use strict";function escapeChar(chr){return escape[chr]||"&amp;"}function extend(obj,value){for(var key in value)Object.prototype.hasOwnProperty.call(value,key)&&(obj[key]=value[key])}function escapeExpression(string){return string instanceof SafeString?string.toString():string||0===string?(string=""+string,
possible.test(string)?string.replace(badChars,escapeChar):string):""}function isEmpty(value){return!value&&0!==value||!(!isArray(value)||0!==value.length)}var __exports__={},SafeString=__dependency1__,escape={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#x27;","`":"&#x60;"},badChars=/[&<>"'`]/g,possible=/[&<>"'`]/;__exports__.extend=extend;var toString=Object.prototype.toString;__exports__.toString=toString;var isFunction=function(value){return"function"==typeof value};isFunction(/x/)&&(isFunction=function(value){return"function"==typeof value&&"[object Function]"===toString.call(value)});var isFunction;__exports__.isFunction=isFunction;var isArray=Array.isArray||function(value){
return!(!value||"object"!=typeof value)&&"[object Array]"===toString.call(value)};return __exports__.isArray=isArray,__exports__.escapeExpression=escapeExpression,__exports__.isEmpty=isEmpty,__exports__}(__module4__),__module5__=function(){"use strict";function Exception(message,node){var line;node&&node.firstLine&&(line=node.firstLine,message+=" - "+line+":"+node.firstColumn);for(var tmp=Error.prototype.constructor.call(this,message),idx=0;idx<errorProps.length;idx++)this[errorProps[idx]]=tmp[errorProps[idx]];line&&(this.lineNumber=line,this.column=node.firstColumn)}var __exports__,errorProps=["description","fileName","lineNumber","message","name","number","stack"];
...
return text={version:"2.0.10",strip:function(content){if(content){content=content.replace(xmlRegExp,"");var matches=content.match(bodyRegExp);matches&&(content=matches[1])}else content="";return content},jsEscape:function(content){return content.replace(/(['\\])/g,"\\$1").replace(/[\f]/g,"\\f").replace(/[\b]/g,"\\b").replace(/[\n]/g,"\\n").replace(/[\t]/g,"\\t").replace(/[\r]/g,"\\r").replace(/[\u2028]/g,"\\u2028").replace(/[\u2029]/g,"\\u2029")},createXhr:masterConfig.createXhr||function(){var xhr,i,progId;if("undefined"!=typeof XMLHttpRequest)return new XMLHttpRequest;if("undefined"!=typeof ActiveXObject)for(i=0;i<3;i+=1){progId=progIds[i];try{xhr=new ActiveXObject(progId);
}catch(e){}if(xhr){progIds=[progId];break}}return xhr},parseName:function(name){var modName,ext,temp,strip=!1,index=name.indexOf("."),isRelative=0===name.indexOf("./")||0===name.indexOf("../");return index!==-1&&(!isRelative||index>1)?(modName=name.substring(0,index),ext=name.substring(index+1,name.length)):modName=name,temp=ext||modName,index=temp.indexOf("!"),index!==-1&&(strip="strip"===temp.substring(index+1),temp=temp.substring(0,index),ext?ext=temp:modName=temp),{moduleName:modName,ext:ext,strip:strip}},xdRegExp:/^((\w+)\:)?\/\/([^\/\\]+)/,useXhr:function(url,protocol,hostname,port){var uProtocol,uHostName,uPort,match=text.xdRegExp.exec(url);return!match||(uProtocol=match[2],
uHostName=match[3],uHostName=uHostName.split(":"),uPort=uHostName[1],uHostName=uHostName[0],!(uProtocol&&uProtocol!==protocol||uHostName&&uHostName.toLowerCase()!==hostname.toLowerCase()||(uPort||uHostName)&&uPort!==port))},finishLoad:function(name,strip,content,onLoad){content=strip?text.strip(content):content,masterConfig.isBuild&&(buildMap[name]=content),onLoad(content)},load:function(name,req,onLoad,config){if(config.isBuild&&!config.inlineText)return void onLoad();masterConfig.isBuild=config.isBuild;var parsed=text.parseName(name),nonStripName=parsed.moduleName+(parsed.ext?"."+parsed.ext:""),url=req.toUrl(nonStripName),useXhr=masterConfig.useXhr||text.useXhr;
...
function cacheBust(url){return url=url.replace(CACHE_BUST_FLAG,""),url+=url.indexOf("?")<0?"?":"&",url+CACHE_BUST_QUERY_PARAM+"="+Math.round(2147483647*Math.random())}var CACHE_BUST_QUERY_PARAM="bust",CACHE_BUST_FLAG="!bust",jsonParse="undefined"!=typeof JSON&&"function"==typeof JSON.parse?JSON.parse:function(val){return eval("("+val+")")},buildMap={};return{load:function(name,req,onLoad,config){!config.isBuild||config.inlineJSON!==!1&&name.indexOf(CACHE_BUST_QUERY_PARAM+"=")===-1?text.get(req.toUrl(name),function(data){config.isBuild?(buildMap[name]=data,onLoad(data)):onLoad(jsonParse(data))},onLoad.error,{accept:"application/json"}):onLoad(null)},normalize:function(name,normalize){
return name.indexOf(CACHE_BUST_FLAG)!==-1&&(name=cacheBust(name)),normalize(name)},write:function(pluginName,moduleName,write){if(moduleName in buildMap){var content=buildMap[moduleName];write('define("'+pluginName+"!"+moduleName+'", function(){ return '+content+";});\n")}}}}),define("json!i18ntr/messages.json",function(){return{serial:"ddb91769b3d553c7c7a33d7a1ebe3577",date:1513255518,translations:{"Manage test takers":"Manage test-takers","Record and manage test takers.":"Record and manage test-takers.","Test takers library":"Test-takers library","Edit subject":"Edit test-taker","Test taker saved":"Test-taker saved","Test takers":"Test-takers","Test takers may be assigned to sets according to the relevant assessment context (Human resources management, education, edumetric monitoring, etc.).":"Assign test-takers to groups before scheduling tests for delivery.\0",
"The Test takers module allows to record and manage  test takers.":"Create and manage test-takers with as many properties as needed, such as name, address, gender, age, etc.\0","Group test takers according to global features and classifications.":"Group test-takers according to global features and classifications."},version:"3.3.0-sprint66"}}),define("context",["module"],function(module){return module.config()}),define("core/format",["lodash"],function(_){"use strict";var pattern=/(%[sdj])/g;return function(message){var replacements=Array.prototype.slice.call(arguments,1);return _.reduce(message.match(pattern),function(acc,match,index){var replacement="";if(void 0!==replacements[index]){
switch(match){case"%d":replacement=Number(replacements[index]);break;case"%j":try{replacement=JSON.stringify(replacements[index]).replace(/"/g,"")}catch(e){}break;default:replacement=replacements[index]}message=message.replace(match,replacement)}return message},message)}}),define("i18n",["lodash","json!i18ntr/messages.json","context","core/format"],function(_,i18nTr,context,format){"use strict";var translations=i18nTr.translations,__=function(message){var localized=translations[message]?translations[message]:message;return arguments.length>1&&(arguments[0]=localized,localized=format.apply(null,arguments)),localized};return __}),define("tpl",["handlebars","i18n","lodash"],function(hb,__,_){
var buildMap={},extension=".tpl";return hb.registerHelper("__",function(key){return __(key)}),hb.registerHelper("join",function(arr,keyValueGlue,fragmentGlue,wrapper){var fragments=[];return keyValueGlue="string"==typeof keyValueGlue?keyValueGlue:void 0,fragmentGlue="string"==typeof fragmentGlue?fragmentGlue:" ",wrapper="string"==typeof wrapper?wrapper:'"',_.forIn(arr,function(value,key){var fragment="";null!==value||void 0!==value?"boolean"==typeof value?value=value?"true":"false":"object"==typeof value&&(value=_.values(value).join(" ")):value="",void 0!==keyValueGlue&&(fragment+=key+keyValueGlue),fragment+=wrapper+value+wrapper,fragments.push(fragment)}),fragments.join(fragmentGlue);
...
config.isBuild?(buildMap[name]=fs.readFileSync(req.toUrl(name+extension)).toString(),onload()):req(["text!"+name+extension],function(raw){onload(hb.compile(raw))})},write:function(pluginName,moduleName,write){if(moduleName in buildMap){var compiled=hb.precompile(buildMap[moduleName]);write("define('tpl!"+moduleName+"', ['handlebars'], function(hb){ \nreturn hb.template("+compiled.toString()+");\n});\n")}}}}),define("tpl!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/tpl/markup",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),
data=data||{};var stack1,helper,buffer="",functionType="function";return buffer+='<div class="graphNumberLineInteraction">\n    <div class="shape-panel">\n        <div class="shape-container"></div>\n        <div class="shape-controls clearfix">\n            <div class="intervals-available">\n                <div class="intervals-row">\n                    <button name="closed-closed" value="closed-closed" class="interval-available btn btn-info" type="button">\n                        ',(helper=helpers.closeClose)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.closeClose,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data
}):helper),(stack1||0===stack1)&&(buffer+=stack1),buffer+='\n                    </button>\n                    <button name="closed-open" value="closed-open" class="interval-available btn btn-info" type="button">\n                        ',(helper=helpers.closeOpen)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.closeOpen,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),(stack1||0===stack1)&&(buffer+=stack1),buffer+='\n                    </button>\n                    <button name="open-closed" value="open-closed" class="interval-available btn btn-info" type="button">\n                        ',(helper=helpers.openClose)?stack1=helper.call(depth0,{
hash:{},data:data}):(helper=depth0&&depth0.openClose,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),(stack1||0===stack1)&&(buffer+=stack1),buffer+='\n                    </button>\n                    <button name="open-open" value="open-open" class="interval-available btn btn-info" type="button">\n                        ',(helper=helpers.openOpen)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.openOpen,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),(stack1||0===stack1)&&(buffer+=stack1),buffer+='\n                    </button>\n                </div>\n                <div class="intervals-row">\n                    <button name="arrow-open" value="arrow-open" class="interval-available btn btn-info" type="button">\n                        ',
(helper=helpers.arrowOpen)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.arrowOpen,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),(stack1||0===stack1)&&(buffer+=stack1),buffer+='\n                    </button>\n                    <button name="arrow-closed" value="arrow-closed" class="interval-available btn btn-info" type="button">\n                        ',(helper=helpers.arrowClose)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.arrowClose,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),(stack1||0===stack1)&&(buffer+=stack1),buffer+='\n                    </button>\n                    <button name="open-arrow" value="open-arrow" class="interval-available btn btn-info" type="button">\n                        ',
(helper=helpers.openArrow)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.openArrow,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),(stack1||0===stack1)&&(buffer+=stack1),buffer+='\n                    </button>\n                    <button name="closed-arrow" value="closed-arrow" class="interval-available btn btn-info" type="button">\n                        ',(helper=helpers.closeArrow)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.closeArrow,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),(stack1||0===stack1)&&(buffer+=stack1),buffer+='\n                    </button>\n                </div>\n                <div class="intervals-overlay"></div>\n            </div>\n            <div class="intervals-selected clearfix"></div>\n            <div class="intervals-template">\n                <div class="interval" data-uid="">\n                    <!--<div class="interval-delete icon icon-bin"></div>-->\n                    <button class="btn btn-info" type="button"></button>\n                    <a class="deleter" title="delete the interval" href="#">delete</a>\n                </div>\n            </div>\n        </div>\n    </div>\n</div>\n';
})}),define("text!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/img/arrow-close.svg",[],function(){return'<svg\n   xmlns:osb="http://www.openswatchbook.org/uri/2009/osb"\n   xmlns:dc="http://purl.org/dc/elements/1.1/"\n   xmlns:cc="http://creativecommons.org/ns#"\n   xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"\n   xmlns:svg="http://www.w3.org/2000/svg"\n   xmlns="http://www.w3.org/2000/svg"\n   xmlns:sodipodi="http://sodipodi.sourceforge.net/DTD/sodipodi-0.dtd"\n   xmlns:inkscape="http://www.inkscape.org/namespaces/inkscape"\n   width="50"\n   height="20"\n   viewbox="0 0 376.80167 169.01511"\n   id="svg2"\n   version="1.1"\n   inkscape:version="0.48.4 r9939"\n   sodipodi:docname="drawing.svg">\n  <defs\n     id="defs4">\n    <linearGradient\n       id="linearGradient3788"\n       osb:paint="solid">\n      <stop\n         style="stop-color:#8c5959;stop-opacity:1;"\n         offset="0"\n         id="stop3790" />\n    </linearGradient>\n  </defs>\n  <sodipodi:namedview\n     id="base"\n     pagecolor="#ffffff"\n     bordercolor="#666666"\n     borderopacity="1.0"\n     inkscape:pageopacity="0.0"\n     inkscape:pageshadow="2"\n     inkscape:zoom="1.4"\n     inkscape:cx="-190.71724"\n     inkscape:cy="-175.99504"\n     inkscape:document-units="px"\n     inkscape:current-layer="layer1"\n     showgrid="false"\n     inkscape:window-width="1920"\n     inkscape:window-height="1176"\n     inkscape:window-x="1920"\n     inkscape:window-y="24"\n     inkscape:window-maximized="1"\n     fit-margin-top="20"\n     fit-margin-left="20"\n     fit-margin-bottom="20"\n     fit-margin-right="20" />\n  <metadata\n     id="metadata7">\n    <rdf:RDF>\n      <cc:Work\n         rdf:about="">\n        <dc:format>image/svg+xml</dc:format>\n        <dc:type\n           rdf:resource="http://purl.org/dc/dcmitype/StillImage" />\n        <dc:title></dc:title>\n      </cc:Work>\n    </rdf:RDF>\n  </metadata>\n  <g\n     inkscape:label="Layer 1"\n     inkscape:groupmode="layer"\n     id="layer1"\n     transform="translate(-80.08759,-72.01576)">\n    <path\n       sodipodi:type="arc"\n       style="fill:#e6eef4;stroke:#e6eef4;stroke-width:17.50611686999999961;stroke-linecap:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none;fill-opacity:1"\n       id="path2985"\n       sodipodi:cx="488.57144"\n       sodipodi:cy="516.64789"\n       sodipodi:rx="57.142857"\n       sodipodi:ry="63.510082"\n       d="m 545.7143,516.64789 a 57.142857,63.510082 0 1 1 -1.8e-4,-0.15749"\n       sodipodi:start="0"\n       sodipodi:end="6.2807055"\n       transform="matrix(0.91679254,0,0,0.82487918,-66.827453,-268.99553)"\n       sodipodi:open="true" />\n    <path\n       style="fill:none;stroke:#e6eef4;stroke-width:15;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       d="m 284.32629,158.01049 -176.7387,0 52.51665,55.52038"\n       id="path2987"\n       inkscape:connector-curvature="0" />\n    <path\n       style="fill:none;stroke:#e6eef4;stroke-width:15;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       d="m 107.70201,155.89867 50.322,-56.38291"\n       id="path2989"\n       inkscape:connector-curvature="0" />\n    <path\n       style="fill:none;stroke:#e6eef4;stroke-width:15;stroke-linecap:round;stroke-linejoin:miter;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       d="m 245.78046,158.05276 c 81.38782,0 80.2805,0 80.2805,0"\n       id="path3759"\n       inkscape:connector-curvature="0" />\n  </g>\n</svg>\n';
}),define("text!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/img/arrow-open.svg",[],function(){return'<svg\n   xmlns:osb="http://www.openswatchbook.org/uri/2009/osb"\n   xmlns:dc="http://purl.org/dc/elements/1.1/"\n   xmlns:cc="http://creativecommons.org/ns#"\n   xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"\n   xmlns:svg="http://www.w3.org/2000/svg"\n   xmlns="http://www.w3.org/2000/svg"\n   xmlns:sodipodi="http://sodipodi.sourceforge.net/DTD/sodipodi-0.dtd"\n   xmlns:inkscape="http://www.inkscape.org/namespaces/inkscape"\n   width="50"\n   height="20"\n   viewbox="0 0 376.80167 169.01511"\n   id="svg2"\n   version="1.1"\n   inkscape:version="0.48.4 r9939"\n   sodipodi:docname="drawing.svg">\n  <defs\n     id="defs4">\n    <linearGradient\n       id="linearGradient3788"\n       osb:paint="solid">\n      <stop\n         style="stop-color:#8c5959;stop-opacity:1;"\n         offset="0"\n         id="stop3790" />\n    </linearGradient>\n  </defs>\n  <sodipodi:namedview\n     id="base"\n     pagecolor="#ffffff"\n     bordercolor="#666666"\n     borderopacity="1.0"\n     inkscape:pageopacity="0.0"\n     inkscape:pageshadow="2"\n     inkscape:zoom="0.7"\n     inkscape:cx="-103.43461"\n     inkscape:cy="383.58253"\n     inkscape:document-units="px"\n     inkscape:current-layer="layer1"\n     showgrid="false"\n     inkscape:window-width="1920"\n     inkscape:window-height="1176"\n     inkscape:window-x="1920"\n     inkscape:window-y="24"\n     inkscape:window-maximized="1"\n     fit-margin-top="20"\n     fit-margin-left="20"\n     fit-margin-bottom="20"\n     fit-margin-right="20" />\n  <metadata\n     id="metadata7">\n    <rdf:RDF>\n      <cc:Work\n         rdf:about="">\n        <dc:format>image/svg+xml</dc:format>\n        <dc:type\n           rdf:resource="http://purl.org/dc/dcmitype/StillImage" />\n        <dc:title></dc:title>\n      </cc:Work>\n    </rdf:RDF>\n  </metadata>\n  <g\n     inkscape:label="Layer 1"\n     inkscape:groupmode="layer"\n     id="layer1"\n     transform="translate(-80.08759,-72.01576)">\n    <path\n       sodipodi:type="arc"\n       style="fill:none;stroke:#e6eef4;stroke-width:17.50611687;stroke-linecap:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       id="path2985"\n       sodipodi:cx="488.57144"\n       sodipodi:cy="516.64789"\n       sodipodi:rx="57.142857"\n       sodipodi:ry="63.510082"\n       d="m 545.7143,516.64789 a 57.142857,63.510082 0 1 1 -1.8e-4,-0.15749"\n       sodipodi:start="0"\n       sodipodi:end="6.2807055"\n       transform="matrix(0.91679254,0,0,0.82487918,-66.827453,-268.99553)"\n       sodipodi:open="true" />\n    <path\n       style="fill:none;stroke:#e6eef4;stroke-width:15;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       d="m 284.32629,158.01049 -176.7387,0 52.51665,55.52038"\n       id="path2987"\n       inkscape:connector-curvature="0" />\n    <path\n       style="fill:none;stroke:#e6eef4;stroke-width:15;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       d="m 107.70201,155.89867 50.322,-56.38291"\n       id="path2989"\n       inkscape:connector-curvature="0" />\n    <path\n       style="fill:none;stroke:#e6eef4;stroke-width:15;stroke-linecap:round;stroke-linejoin:miter;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       d="m 245.78046,158.05276 c 81.38782,0 80.2805,0 80.2805,0"\n       id="path3759"\n       inkscape:connector-curvature="0" />\n  </g>\n</svg>\n';
}),define("text!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/img/close-arrow.svg",[],function(){return'<svg\n   xmlns:osb="http://www.openswatchbook.org/uri/2009/osb"\n   xmlns:dc="http://purl.org/dc/elements/1.1/"\n   xmlns:cc="http://creativecommons.org/ns#"\n   xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"\n   xmlns:svg="http://www.w3.org/2000/svg"\n   xmlns="http://www.w3.org/2000/svg"\n   xmlns:sodipodi="http://sodipodi.sourceforge.net/DTD/sodipodi-0.dtd"\n   xmlns:inkscape="http://www.inkscape.org/namespaces/inkscape"\n   width="50"\n   height="20"\n   viewbox="0 0 376.80167 169.01511"\n   id="svg2"\n   version="1.1"\n   inkscape:version="0.48.4 r9939"\n   sodipodi:docname="arrow-close.svg">\n  <defs\n     id="defs4">\n    <linearGradient\n       id="linearGradient3788"\n       osb:paint="solid">\n      <stop\n         style="stop-color:#8c5959;stop-opacity:1;"\n         offset="0"\n         id="stop3790" />\n    </linearGradient>\n  </defs>\n  <sodipodi:namedview\n     id="base"\n     pagecolor="#ffffff"\n     bordercolor="#666666"\n     borderopacity="1.0"\n     inkscape:pageopacity="0.0"\n     inkscape:pageshadow="2"\n     inkscape:zoom="1.4"\n     inkscape:cx="248.80218"\n     inkscape:cy="-147.42361"\n     inkscape:document-units="px"\n     inkscape:current-layer="layer1"\n     showgrid="false"\n     inkscape:window-width="1920"\n     inkscape:window-height="1176"\n     inkscape:window-x="1920"\n     inkscape:window-y="24"\n     inkscape:window-maximized="1"\n     fit-margin-top="20"\n     fit-margin-left="20"\n     fit-margin-bottom="20"\n     fit-margin-right="20" />\n  <metadata\n     id="metadata7">\n    <rdf:RDF>\n      <cc:Work\n         rdf:about="">\n        <dc:format>image/svg+xml</dc:format>\n        <dc:type\n           rdf:resource="http://purl.org/dc/dcmitype/StillImage" />\n        <dc:title></dc:title>\n      </cc:Work>\n    </rdf:RDF>\n  </metadata>\n  <g\n     inkscape:label="Layer 1"\n     inkscape:groupmode="layer"\n     id="layer1"\n     transform="translate(-80.08759,-72.01576)">\n    <path\n       sodipodi:type="arc"\n       style="fill:#e6eef4;fill-opacity:1;stroke:#e6eef4;stroke-width:17.50611687;stroke-linecap:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       id="path2985"\n       sodipodi:cx="488.57144"\n       sodipodi:cy="516.64789"\n       sodipodi:rx="57.142857"\n       sodipodi:ry="63.510082"\n       d="m 545.7143,516.64789 a 57.142857,63.510082 0 1 1 -1.8e-4,-0.15749"\n       sodipodi:start="0"\n       sodipodi:end="6.2807055"\n       transform="matrix(-0.91679254,0,0,0.82487918,608.00624,-268.99553)"\n       sodipodi:open="true" />\n    <path\n       style="fill:none;stroke:#e6eef4;stroke-width:15;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       d="m 256.8525,158.01049 176.7387,0 -52.51665,55.52038"\n       id="path2987"\n       inkscape:connector-curvature="0" />\n    <path\n       style="fill:none;stroke:#e6eef4;stroke-width:15;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       d="m 433.47678,155.89867 -50.322,-56.38291"\n       id="path2989"\n       inkscape:connector-curvature="0" />\n    <path\n       style="fill:none;stroke:#e6eef4;stroke-width:15;stroke-linecap:round;stroke-linejoin:miter;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       d="m 295.39833,158.05276 c -81.38782,0 -80.2805,0 -80.2805,0"\n       id="path3759"\n       inkscape:connector-curvature="0" />\n  </g>\n</svg>\n';
}),define("text!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/img/close-close.svg",[],function(){return'<svg\n   xmlns:osb="http://www.openswatchbook.org/uri/2009/osb"\n   xmlns:dc="http://purl.org/dc/elements/1.1/"\n   xmlns:cc="http://creativecommons.org/ns#"\n   xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"\n   xmlns:svg="http://www.w3.org/2000/svg"\n   xmlns="http://www.w3.org/2000/svg"\n   xmlns:sodipodi="http://sodipodi.sourceforge.net/DTD/sodipodi-0.dtd"\n   xmlns:inkscape="http://www.inkscape.org/namespaces/inkscape"\n   width="50"\n   height="20"\n   viewbox="0 0 390.85715 160"\n   id="svg2"\n   version="1.1"\n   inkscape:version="0.48.4 r9939"\n   sodipodi:docname="close-open.svg">\n  <defs\n     id="defs4">\n    <linearGradient\n       id="linearGradient3788"\n       osb:paint="solid">\n      <stop\n         style="stop-color:#8c5959;stop-opacity:1;"\n         offset="0"\n         id="stop3790" />\n    </linearGradient>\n  </defs>\n  <sodipodi:namedview\n     id="base"\n     pagecolor="#ffffff"\n     bordercolor="#666666"\n     borderopacity="1.0"\n     inkscape:pageopacity="0.0"\n     inkscape:pageshadow="2"\n     inkscape:zoom="1.4"\n     inkscape:cx="248.80218"\n     inkscape:cy="-151.42361"\n     inkscape:document-units="px"\n     inkscape:current-layer="layer1"\n     showgrid="false"\n     inkscape:window-width="1920"\n     inkscape:window-height="1176"\n     inkscape:window-x="1920"\n     inkscape:window-y="24"\n     inkscape:window-maximized="1"\n     fit-margin-top="20"\n     fit-margin-left="20"\n     fit-margin-bottom="20"\n     fit-margin-right="20" />\n  <metadata\n     id="metadata7">\n    <rdf:RDF>\n      <cc:Work\n         rdf:about="">\n        <dc:format>image/svg+xml</dc:format>\n        <dc:type\n           rdf:resource="http://purl.org/dc/dcmitype/StillImage" />\n        <dc:title></dc:title>\n      </cc:Work>\n    </rdf:RDF>\n  </metadata>\n  <g\n     inkscape:label="Layer 1"\n     inkscape:groupmode="layer"\n     id="layer1"\n     transform="translate(-80.087587,-77.030866)">\n    <path\n       sodipodi:type="arc"\n       style="fill:#e6eef4;stroke:#e6eef4;stroke-width:17.50611686999999961;stroke-linecap:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none;fill-opacity:1"\n       id="path2985"\n       sodipodi:cx="488.57144"\n       sodipodi:cy="516.64789"\n       sodipodi:rx="57.142857"\n       sodipodi:ry="63.510082"\n       d="m 545.7143,516.64789 c 0,35.07565 -25.58373,63.51008 -57.14286,63.51008 -31.55913,0 -57.14285,-28.43443 -57.14285,-63.51008 0,-35.07565 25.58372,-63.51008 57.14285,-63.51008 31.50381,0 57.06456,28.33853 57.14268,63.35259"\n       sodipodi:start="0"\n       sodipodi:end="6.2807055"\n       transform="matrix(-0.91679254,0,0,0.82487918,608.00624,-269.14122)"\n       sodipodi:open="true" />\n    <path\n       style="fill:none;stroke:#e6eef4;stroke-width:16.15803146;stroke-linecap:round;stroke-linejoin:miter;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       d="m 329.83454,158.05276 c -113.55503,0 -112.01007,0 -112.01007,0"\n       id="path3759"\n       inkscape:connector-curvature="0" />\n    <path\n       sodipodi:type="arc"\n       style="fill:#e6eef4;stroke:#e6eef4;stroke-width:17.50611686999999961;stroke-linecap:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none;fill-opacity:1"\n       id="path2985-5"\n       sodipodi:cx="488.57144"\n       sodipodi:cy="516.64789"\n       sodipodi:rx="57.142857"\n       sodipodi:ry="63.510082"\n       d="m 545.7143,516.64789 c 0,35.07565 -25.58373,63.51008 -57.14286,63.51008 -31.55913,0 -57.14285,-28.43443 -57.14285,-63.51008 0,-35.07565 25.58372,-63.51008 57.14285,-63.51008 31.50381,0 57.06456,28.33853 57.14268,63.35259"\n       sodipodi:start="0"\n       sodipodi:end="6.2807055"\n       transform="matrix(-0.91679254,0,0,0.82487918,838.86339,-269.14122)"\n       sodipodi:open="true" />\n  </g>\n</svg>\n';
}),define("text!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/img/close-open.svg",[],function(){return'<svg\n   xmlns:osb="http://www.openswatchbook.org/uri/2009/osb"\n   xmlns:dc="http://purl.org/dc/elements/1.1/"\n   xmlns:cc="http://creativecommons.org/ns#"\n   xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"\n   xmlns:svg="http://www.w3.org/2000/svg"\n   xmlns="http://www.w3.org/2000/svg"\n   xmlns:sodipodi="http://sodipodi.sourceforge.net/DTD/sodipodi-0.dtd"\n   xmlns:inkscape="http://www.inkscape.org/namespaces/inkscape"\n   width="50"\n   height="20"\n   viewbox="0 0 390.85715 160"\n   id="svg2"\n   version="1.1"\n   inkscape:version="0.48.4 r9939"\n   sodipodi:docname="open-open.svg">\n  <defs\n     id="defs4">\n    <linearGradient\n       id="linearGradient3788"\n       osb:paint="solid">\n      <stop\n         style="stop-color:#8c5959;stop-opacity:1;"\n         offset="0"\n         id="stop3790" />\n    </linearGradient>\n  </defs>\n  <sodipodi:namedview\n     id="base"\n     pagecolor="#ffffff"\n     bordercolor="#666666"\n     borderopacity="1.0"\n     inkscape:pageopacity="0.0"\n     inkscape:pageshadow="2"\n     inkscape:zoom="1.4"\n     inkscape:cx="248.80218"\n     inkscape:cy="-151.42361"\n     inkscape:document-units="px"\n     inkscape:current-layer="layer1"\n     showgrid="false"\n     inkscape:window-width="1920"\n     inkscape:window-height="1176"\n     inkscape:window-x="1920"\n     inkscape:window-y="24"\n     inkscape:window-maximized="1"\n     fit-margin-top="20"\n     fit-margin-left="20"\n     fit-margin-bottom="20"\n     fit-margin-right="20" />\n  <metadata\n     id="metadata7">\n    <rdf:RDF>\n      <cc:Work\n         rdf:about="">\n        <dc:format>image/svg+xml</dc:format>\n        <dc:type\n           rdf:resource="http://purl.org/dc/dcmitype/StillImage" />\n        <dc:title></dc:title>\n      </cc:Work>\n    </rdf:RDF>\n  </metadata>\n  <g\n     inkscape:label="Layer 1"\n     inkscape:groupmode="layer"\n     id="layer1"\n     transform="translate(-80.087587,-77.030866)">\n    <path\n       sodipodi:type="arc"\n       style="fill:#e6eef4;stroke:#e6eef4;stroke-width:17.50611686999999961;stroke-linecap:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none;fill-opacity:1"\n       id="path2985"\n       sodipodi:cx="488.57144"\n       sodipodi:cy="516.64789"\n       sodipodi:rx="57.142857"\n       sodipodi:ry="63.510082"\n       d="m 545.7143,516.64789 c 0,35.07565 -25.58373,63.51008 -57.14286,63.51008 -31.55913,0 -57.14285,-28.43443 -57.14285,-63.51008 0,-35.07565 25.58372,-63.51008 57.14285,-63.51008 31.50381,0 57.06456,28.33853 57.14268,63.35259"\n       sodipodi:start="0"\n       sodipodi:end="6.2807055"\n       transform="matrix(-0.91679254,0,0,0.82487918,608.00624,-269.14122)"\n       sodipodi:open="true" />\n    <path\n       style="fill:none;stroke:#e6eef4;stroke-width:16.15803146;stroke-linecap:round;stroke-linejoin:miter;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       d="m 329.83454,158.05276 c -113.55503,0 -112.01007,0 -112.01007,0"\n       id="path3759"\n       inkscape:connector-curvature="0" />\n    <path\n       sodipodi:type="arc"\n       style="fill:none;stroke:#e6eef4;stroke-width:17.50611687;stroke-linecap:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       id="path2985-5"\n       sodipodi:cx="488.57144"\n       sodipodi:cy="516.64789"\n       sodipodi:rx="57.142857"\n       sodipodi:ry="63.510082"\n       d="m 545.7143,516.64789 c 0,35.07565 -25.58373,63.51008 -57.14286,63.51008 -31.55913,0 -57.14285,-28.43443 -57.14285,-63.51008 0,-35.07565 25.58372,-63.51008 57.14285,-63.51008 31.50381,0 57.06456,28.33853 57.14268,63.35259"\n       sodipodi:start="0"\n       sodipodi:end="6.2807055"\n       transform="matrix(-0.91679254,0,0,0.82487918,838.86339,-269.14122)"\n       sodipodi:open="true" />\n  </g>\n</svg>\n';
}),define("text!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/img/open-arrow.svg",[],function(){return'<svg\n   xmlns:osb="http://www.openswatchbook.org/uri/2009/osb"\n   xmlns:dc="http://purl.org/dc/elements/1.1/"\n   xmlns:cc="http://creativecommons.org/ns#"\n   xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"\n   xmlns:svg="http://www.w3.org/2000/svg"\n   xmlns="http://www.w3.org/2000/svg"\n   xmlns:sodipodi="http://sodipodi.sourceforge.net/DTD/sodipodi-0.dtd"\n   xmlns:inkscape="http://www.inkscape.org/namespaces/inkscape"\n   width="50"\n   height="20"\n   viewbox="0 0 390.85715 160"\n   id="svg2"\n   version="1.1"\n   inkscape:version="0.48.4 r9939"\n   sodipodi:docname="close-arrow.svg">\n  <defs\n     id="defs4">\n    <linearGradient\n       id="linearGradient3788"\n       osb:paint="solid">\n      <stop\n         style="stop-color:#8c5959;stop-opacity:1;"\n         offset="0"\n         id="stop3790" />\n    </linearGradient>\n  </defs>\n  <sodipodi:namedview\n     id="base"\n     pagecolor="#ffffff"\n     bordercolor="#666666"\n     borderopacity="1.0"\n     inkscape:pageopacity="0.0"\n     inkscape:pageshadow="2"\n     inkscape:zoom="1.4"\n     inkscape:cx="248.80218"\n     inkscape:cy="-147.42361"\n     inkscape:document-units="px"\n     inkscape:current-layer="layer1"\n     showgrid="false"\n     inkscape:window-width="1920"\n     inkscape:window-height="1176"\n     inkscape:window-x="1920"\n     inkscape:window-y="24"\n     inkscape:window-maximized="1"\n     fit-margin-top="20"\n     fit-margin-left="20"\n     fit-margin-bottom="20"\n     fit-margin-right="20" />\n  <metadata\n     id="metadata7">\n    <rdf:RDF>\n      <cc:Work\n         rdf:about="">\n        <dc:format>image/svg+xml</dc:format>\n        <dc:type\n           rdf:resource="http://purl.org/dc/dcmitype/StillImage" />\n        <dc:title></dc:title>\n      </cc:Work>\n    </rdf:RDF>\n  </metadata>\n  <g\n     inkscape:label="Layer 1"\n     inkscape:groupmode="layer"\n     id="layer1"\n     transform="translate(-80.08759,-72.01576)">\n    <path\n       sodipodi:type="arc"\n       style="fill:none;fill-opacity:1;stroke:#e6eef4;stroke-width:17.50611686999999961;stroke-linecap:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       id="path2985"\n       sodipodi:cx="488.57144"\n       sodipodi:cy="516.64789"\n       sodipodi:rx="57.142857"\n       sodipodi:ry="63.510082"\n       d="m 545.7143,516.64789 a 57.142857,63.510082 0 1 1 -1.8e-4,-0.15749"\n       sodipodi:start="0"\n       sodipodi:end="6.2807055"\n       transform="matrix(-0.91679254,0,0,0.82487918,608.00624,-268.99553)"\n       sodipodi:open="true" />\n    <path\n       style="fill:none;stroke:#e6eef4;stroke-width:15;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       d="m 256.8525,158.01049 176.7387,0 -52.51665,55.52038"\n       id="path2987"\n       inkscape:connector-curvature="0" />\n    <path\n       style="fill:none;stroke:#e6eef4;stroke-width:15;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       d="m 433.47678,155.89867 -50.322,-56.38291"\n       id="path2989"\n       inkscape:connector-curvature="0" />\n    <path\n       style="fill:none;stroke:#e6eef4;stroke-width:15;stroke-linecap:round;stroke-linejoin:miter;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       d="m 295.39833,158.05276 c -81.38782,0 -80.2805,0 -80.2805,0"\n       id="path3759"\n       inkscape:connector-curvature="0" />\n  </g>\n</svg>\n';
}),define("text!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/img/open-close.svg",[],function(){return'<svg\n   xmlns:osb="http://www.openswatchbook.org/uri/2009/osb"\n   xmlns:dc="http://purl.org/dc/elements/1.1/"\n   xmlns:cc="http://creativecommons.org/ns#"\n   xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"\n   xmlns:svg="http://www.w3.org/2000/svg"\n   xmlns="http://www.w3.org/2000/svg"\n   xmlns:sodipodi="http://sodipodi.sourceforge.net/DTD/sodipodi-0.dtd"\n   xmlns:inkscape="http://www.inkscape.org/namespaces/inkscape"\n   width="50"\n   height="20"\n   viewbox="0 0 390.85715 160"\n   id="svg2"\n   version="1.1"\n   inkscape:version="0.48.4 r9939"\n   sodipodi:docname="close-close.svg">\n  <defs\n     id="defs4">\n    <linearGradient\n       id="linearGradient3788"\n       osb:paint="solid">\n      <stop\n         style="stop-color:#8c5959;stop-opacity:1;"\n         offset="0"\n         id="stop3790" />\n    </linearGradient>\n  </defs>\n  <sodipodi:namedview\n     id="base"\n     pagecolor="#ffffff"\n     bordercolor="#666666"\n     borderopacity="1.0"\n     inkscape:pageopacity="0.0"\n     inkscape:pageshadow="2"\n     inkscape:zoom="1.4"\n     inkscape:cx="248.80218"\n     inkscape:cy="-151.42361"\n     inkscape:document-units="px"\n     inkscape:current-layer="layer1"\n     showgrid="false"\n     inkscape:window-width="1920"\n     inkscape:window-height="1176"\n     inkscape:window-x="1920"\n     inkscape:window-y="24"\n     inkscape:window-maximized="1"\n     fit-margin-top="20"\n     fit-margin-left="20"\n     fit-margin-bottom="20"\n     fit-margin-right="20" />\n  <metadata\n     id="metadata7">\n    <rdf:RDF>\n      <cc:Work\n         rdf:about="">\n        <dc:format>image/svg+xml</dc:format>\n        <dc:type\n           rdf:resource="http://purl.org/dc/dcmitype/StillImage" />\n        <dc:title></dc:title>\n      </cc:Work>\n    </rdf:RDF>\n  </metadata>\n  <g\n     inkscape:label="Layer 1"\n     inkscape:groupmode="layer"\n     id="layer1"\n     transform="translate(-80.087587,-77.030866)">\n    <path\n       sodipodi:type="arc"\n       style="fill:none;stroke:#e6eef4;stroke-width:17.50611686999999961;stroke-linecap:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none;fill-opacity:1"\n       id="path2985"\n       sodipodi:cx="488.57144"\n       sodipodi:cy="516.64789"\n       sodipodi:rx="57.142857"\n       sodipodi:ry="63.510082"\n       d="m 545.7143,516.64789 c 0,35.07565 -25.58373,63.51008 -57.14286,63.51008 -31.55913,0 -57.14285,-28.43443 -57.14285,-63.51008 0,-35.07565 25.58372,-63.51008 57.14285,-63.51008 31.50381,0 57.06456,28.33853 57.14268,63.35259"\n       sodipodi:start="0"\n       sodipodi:end="6.2807055"\n       transform="matrix(-0.91679254,0,0,0.82487918,608.00624,-269.14122)"\n       sodipodi:open="true" />\n    <path\n       style="fill:none;stroke:#e6eef4;stroke-width:16.15803146;stroke-linecap:round;stroke-linejoin:miter;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       d="m 329.83454,158.05276 c -113.55503,0 -112.01007,0 -112.01007,0"\n       id="path3759"\n       inkscape:connector-curvature="0" />\n    <path\n       sodipodi:type="arc"\n       style="fill:#e6eef4;stroke:#e6eef4;stroke-width:17.50611686999999961;stroke-linecap:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none;fill-opacity:1"\n       id="path2985-5"\n       sodipodi:cx="488.57144"\n       sodipodi:cy="516.64789"\n       sodipodi:rx="57.142857"\n       sodipodi:ry="63.510082"\n       d="m 545.7143,516.64789 c 0,35.07565 -25.58373,63.51008 -57.14286,63.51008 -31.55913,0 -57.14285,-28.43443 -57.14285,-63.51008 0,-35.07565 25.58372,-63.51008 57.14285,-63.51008 31.50381,0 57.06456,28.33853 57.14268,63.35259"\n       sodipodi:start="0"\n       sodipodi:end="6.2807055"\n       transform="matrix(-0.91679254,0,0,0.82487918,838.86339,-269.14122)"\n       sodipodi:open="true" />\n  </g>\n</svg>\n';
}),define("text!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/img/open-open.svg",[],function(){return'<svg\n   xmlns:osb="http://www.openswatchbook.org/uri/2009/osb"\n   xmlns:dc="http://purl.org/dc/elements/1.1/"\n   xmlns:cc="http://creativecommons.org/ns#"\n   xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"\n   xmlns:svg="http://www.w3.org/2000/svg"\n   xmlns="http://www.w3.org/2000/svg"\n   xmlns:sodipodi="http://sodipodi.sourceforge.net/DTD/sodipodi-0.dtd"\n   xmlns:inkscape="http://www.inkscape.org/namespaces/inkscape"\n   width="50"\n   height="20"\n   viewbox="0 0 390.85715 160"\n   id="svg2"\n   version="1.1"\n   inkscape:version="0.48.4 r9939"\n   sodipodi:docname="open-arrow.svg">\n  <defs\n     id="defs4">\n    <linearGradient\n       id="linearGradient3788"\n       osb:paint="solid">\n      <stop\n         style="stop-color:#8c5959;stop-opacity:1;"\n         offset="0"\n         id="stop3790" />\n    </linearGradient>\n  </defs>\n  <sodipodi:namedview\n     id="base"\n     pagecolor="#ffffff"\n     bordercolor="#666666"\n     borderopacity="1.0"\n     inkscape:pageopacity="0.0"\n     inkscape:pageshadow="2"\n     inkscape:zoom="1.4"\n     inkscape:cx="248.80218"\n     inkscape:cy="-151.42361"\n     inkscape:document-units="px"\n     inkscape:current-layer="layer1"\n     showgrid="false"\n     inkscape:window-width="1920"\n     inkscape:window-height="1176"\n     inkscape:window-x="1920"\n     inkscape:window-y="24"\n     inkscape:window-maximized="1"\n     fit-margin-top="20"\n     fit-margin-left="20"\n     fit-margin-bottom="20"\n     fit-margin-right="20" />\n  <metadata\n     id="metadata7">\n    <rdf:RDF>\n      <cc:Work\n         rdf:about="">\n        <dc:format>image/svg+xml</dc:format>\n        <dc:type\n           rdf:resource="http://purl.org/dc/dcmitype/StillImage" />\n        <dc:title></dc:title>\n      </cc:Work>\n    </rdf:RDF>\n  </metadata>\n  <g\n     inkscape:label="Layer 1"\n     inkscape:groupmode="layer"\n     id="layer1"\n     transform="translate(-80.087587,-77.030866)">\n    <path\n       sodipodi:type="arc"\n       style="fill:none;stroke:#e6eef4;stroke-width:17.50611687;stroke-linecap:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       id="path2985"\n       sodipodi:cx="488.57144"\n       sodipodi:cy="516.64789"\n       sodipodi:rx="57.142857"\n       sodipodi:ry="63.510082"\n       d="m 545.7143,516.64789 c 0,35.07565 -25.58373,63.51008 -57.14286,63.51008 -31.55913,0 -57.14285,-28.43443 -57.14285,-63.51008 0,-35.07565 25.58372,-63.51008 57.14285,-63.51008 31.50381,0 57.06456,28.33853 57.14268,63.35259"\n       sodipodi:start="0"\n       sodipodi:end="6.2807055"\n       transform="matrix(-0.91679254,0,0,0.82487918,608.00624,-269.14122)"\n       sodipodi:open="true" />\n    <path\n       style="fill:none;stroke:#e6eef4;stroke-width:16.15803146;stroke-linecap:round;stroke-linejoin:miter;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       d="m 329.83454,158.05276 c -113.55503,0 -112.01007,0 -112.01007,0"\n       id="path3759"\n       inkscape:connector-curvature="0" />\n    <path\n       sodipodi:type="arc"\n       style="fill:none;stroke:#e6eef4;stroke-width:17.50611687;stroke-linecap:round;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:none"\n       id="path2985-5"\n       sodipodi:cx="488.57144"\n       sodipodi:cy="516.64789"\n       sodipodi:rx="57.142857"\n       sodipodi:ry="63.510082"\n       d="m 545.7143,516.64789 c 0,35.07565 -25.58373,63.51008 -57.14286,63.51008 -31.55913,0 -57.14285,-28.43443 -57.14285,-63.51008 0,-35.07565 25.58372,-63.51008 57.14285,-63.51008 31.50381,0 57.06456,28.33853 57.14268,63.35259"\n       sodipodi:start="0"\n       sodipodi:end="6.2807055"\n       transform="matrix(-0.91679254,0,0,0.82487918,838.86339,-269.14122)"\n       sodipodi:open="true" />\n  </g>\n</svg>\n';
...
return".css"==name.substr(name.length-4,4)&&(name=name.substr(0,name.length-4)),normalize(name)},cssAPI.load=function(cssId,req,load,config){(useImportLoad?importLoad:linkLoad)(req.toUrl(cssId+".css"),load)},cssAPI}),define("css!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/css/graphNumberLineInteraction",[],function(){}),define("graphNumberLineInteraction/runtime/graphNumberLineInteraction",["taoQtiItem/portableLib/jquery_2_1_1","qtiCustomInteractionContext","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/OAT/util/event","taoQtiItem/portableLib/OAT/scale.raphael","parccTei/portableLib/pointFactory","parccTei/portableLib/axisFactory","parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/libs/intervalFactory","tpl!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/tpl/markup","text!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/img/arrow-close.svg","text!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/img/arrow-open.svg","text!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/img/close-arrow.svg","text!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/img/close-close.svg","text!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/img/close-open.svg","text!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/img/open-arrow.svg","text!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/img/open-close.svg","text!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/img/open-open.svg","css!parccTei/pciCreator/ims/graphNumberLineInteraction/runtime/css/graphNumberLineInteraction"],function($,qtiCustomInteractionContext,_,event,scaleRaphael,pointFactory,axisFactory,IntervalFactory,markupTpl,arrowCloseSvg,arrowOpenSvg,closeArrowSvg,closeCloseSvg,closeOpenSvg,openArrowSvg,openCloseSvg,openOpenSvg){
"use strict";function createCanvas($container,config){var padding=2,width=2*padding+config.unitSize*(2+config.max-config.min),paper=scaleRaphael($(".shape-container",$container)[0],width,120);return paper}function buildAxisConfig(rawConfig){var _default={top:60,left:50,unitSize:50,min:-5,max:5,unitSubDivision:2,arrows:!0,plot:{color:"#266d9c",thickness:5},point:{color:"#266d9c",radius:10}};return _.merge(_default,{min:void 0===rawConfig.min?void 0:parseInt(rawConfig.min),max:void 0===rawConfig.max?void 0:parseInt(rawConfig.max),unitSubDivision:void 0===rawConfig.unitSubDivision?void 0:parseInt(rawConfig.unitSubDivision),plot:{color:rawConfig.graphColor,thickness:void 0===rawConfig.graphWidth?void 0:parseInt(rawConfig.graphWidth)
},point:{color:rawConfig.graphColor,radius:10}})}function getAuthorizedIntervals(){return["closed-closed","closed-open","open-closed","open-open","arrow-open","arrow-closed","open-arrow","closed-arrow"]}var graphNumberLineInteraction,_typeIdentifier="graphNumberLineInteraction";graphNumberLineInteraction={typeIdentifier:_typeIdentifier,getInstance:function(dom,config,state){var response=config.boundTo;this.initialize(Object.getOwnPropertyNames(response).pop(),dom,config.properties),this.setSerializedState(state),"function"==typeof config.onready&&config.onready(this,this.getState())},getState:function(){return this.getSerializedState()},oncompleted:function(){
this.destroy()},getResponse:function(){var types=[],values=[],rawResponse=this.getRawResponse();return _.isArray(rawResponse)&&_.size(rawResponse)?(_.each(rawResponse,function(interval){types.push(interval.type),values.push([interval.start,interval.end])}),{record:[{name:"lineTypes",base:{list:{string:types}}},{name:"values",base:{list:{pair:values}}}]}):{base:null}},destroy:function(){var $container=$(this.dom);$container.off().empty()},getTypeIdentifier:function(){return _typeIdentifier},initialize:function(id,dom,config){function initAxis($container,axisConfig){paper=createCanvas($container,axisConfig),axis=new axisFactory(paper,axisConfig),intervalFactory=new IntervalFactory(axis,axisConfig.plot);
}function activate(uid){_.forIn(intervals,function(interval,id){id===uid?(interval.$control.find(".btn").removeClass("btn-info").addClass("btn-success"),interval.obj.enable()):(interval.$control.find(".btn").removeClass("btn-success").addClass("btn-info"),interval.obj.disable())})}function reset(){_.each(intervals,function(interval){interval.obj.destroy(),interval.$control.remove(),$intervalsOverlay.hide()}),intervals={}}function setAvailableIntervals(availableIntervals){$intervalsAvailable.find(".interval-available").each(function(){var $this=$(this);_.indexOf(availableIntervals,$this.attr("value"))>=0?$this.addClass("activated"):$this.removeClass("activated");
}),reset()}function setAxis(axisConfig){_this.axisConfig=buildAxisConfig(_.merge(_this.config,_this.axisConfig,axisConfig)),initAxis($container,_this.axisConfig),reset()}function createInterval(intervalType,start,end){var $button,uid,$svg,$tpl,interval;if(_.size(intervals)<selectionMax)return uid=_.uniqueId("interval_"),$button=$intervalsAvailable.find("button[value="+intervalType+"]"),$svg=$button.find("svg").clone(),$tpl=$intervalTemplate.clone(),$tpl.find(".btn").append($svg),$tpl.attr("data-uid",uid),$intervalsSelected.append($tpl),interval=intervalFactory.plot(intervalType,start,end),intervals[uid]={type:intervalType,obj:interval,$control:$tpl},activate(uid),
_.defer(function(){_this.trigger("responseChange",[_this.getRawResponse()])}),_.size(intervals)===selectionMax&&$intervalsOverlay.show(),interval}event.addEventMgr(this),this.id=id,this.dom=dom,this.config=config||{};var paper,axis,intervalFactory,$container=$(dom),_this=this;$container.append(markupTpl({closeClose:closeCloseSvg,closeOpen:closeOpenSvg,openClose:openCloseSvg,openOpen:openOpenSvg,arrowOpen:arrowOpenSvg,arrowClose:arrowCloseSvg,openArrow:openArrowSvg,closeArrow:closeArrowSvg})),this.reset=reset,this.axisConfig=buildAxisConfig(this.config),initAxis($container,this.axisConfig);var $intervalsAvailable=$container.find(".intervals-available"),$intervalsOverlay=$container.find(".intervals-overlay"),$intervalsSelected=$container.find(".intervals-selected"),$intervalTemplate=$container.find(".intervals-template .interval"),selectionMax=2,intervals={},availableIntervals=this.config.intervals?this.config.intervals.split(","):getAuthorizedIntervals();
setAvailableIntervals(availableIntervals),$container.on("click",".intervals-available .btn-info",function(){createInterval($(this).val(),0,1)}).on("click",".intervals-selected .btn-info",function(){var $parent=$(this).parent(".interval"),uid=$parent.data("uid");activate(uid)}).on("click",".intervals-selected .deleter",function(){var $deleter=$(this),$parent=$deleter.parent(".interval"),uid=$parent.data("uid"),interval=intervals[uid];interval.obj.destroy(),$parent.remove(),intervals=_.omit(intervals,uid),_.size(intervals)<=selectionMax&&$intervalsOverlay.hide(),_this.trigger("responseChange",[_this.getRawResponse()])}).on("change.interval",function(){_this.trigger("responseChange",[_this.getRawResponse()]);
}),this.on("intervalschange",setAvailableIntervals),this.on("axischange",setAxis),this.getRawResponse=function(){var response=[];return _.each(intervals,function(interval){var coords=interval.obj.getCoordinates();response.push({type:interval.type,start:coords.start,end:coords.end})}),response},this.setRawResponse=function(intervals){_.isArray(intervals)&&_.each(intervals,function(interval){var start=interval.start,end=interval.end;null===start&&(start=end),createInterval(interval.type,start,end)})}},setResponse:function(response){var i,point,lineTypes,values,rawResponse=[];if(response&&_.isArray(response.record)&&response.record[0]&&response.record[1]&&"lineTypes"===response.record[0].name&&response.record[0].base&&response.record[0].base.list&&_.isArray(response.record[0].base.list.string)&&"values"===response.record[1].name&&response.record[1].base&&response.record[1].base.list&&_.isArray(response.record[1].base.list.pair)&&response.record[0].base.list.length===response.record[1].base.list.length){
for(lineTypes=response.record[0].base.list.string,values=response.record[1].base.list.pair,i=0;i<lineTypes.length;i++)point=values[i],rawResponse.push({type:lineTypes[i],start:point[0],end:point[1]});this.setRawResponse(rawResponse)}},resetResponse:function(){this.reset()},setSerializedState:function(state){this.setResponse(state)},getSerializedState:function(){return this.getResponse()}},qtiCustomInteractionContext.register(graphNumberLineInteraction)}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))}("/*add some style, need to scope (using the typeIdentifier as class prefix) every style declaration to prevent collision */\n\n.graphNumberLineInteraction {\n    width:100%;\n    padding:30px 30px 50px;\n    margin:20px 0;\n    padding:20px;\n    border : 6px solid #f5f3f2;\n}\n\n.graphNumberLineInteraction .shape-controls .clearfix {\n    overflow: auto;\n}\n\n.graphNumberLineInteraction .shape-controls .intervals-available {float:left; position : relative; border : 6px solid #f5f3f2; padding:4px; margin-right : 10px; margin-bottom:10px;}\n.graphNumberLineInteraction .shape-controls .intervals-available .intervals-overlay {position:absolute; width:100%; height : 100%; top:0; left:0; background: #ddd; opacity : 0.6; display:none;}\n.graphNumberLineInteraction .shape-controls .intervals-available .interval-available {display:none;}\n.graphNumberLineInteraction .shape-controls .intervals-available .interval-available.activated {display:inline-block;}\n\n.graphNumberLineInteraction .shape-controls .intervals-selected {float:left;}\n.graphNumberLineInteraction .shape-controls .intervals-selected .interval {float:left; position : relative; width:92px; border : 6px solid #f5f3f2; padding:4px; margin-right:10px;}\n.graphNumberLineInteraction .shape-controls .intervals-selected .interval .deleter {cursor: pointer; }\n.graphNumberLineInteraction .shape-controls .intervals-selected .interval .deleter:before {font-size:1.25rem; margin-right: 3px;}\n\n.graphNumberLineInteraction .shape-controls .intervals-selected .interval .btn {margin-bottom:10px;}\n\n.graphNumberLineInteraction .shape-controls button {padding: 0 6px 8px; height:36px; width:72px; margin-right:2px;}\n\n.graphNumberLineInteraction .shape-controls .intervals-template {display:none;}\n\n.graphNumberLineInteraction .shape-controls .intervals-row {margin-bottom: 2px;}\n\n/*alternative deleter*/\n/*.graphNumberLineInteraction .shape-controls .intervals-selected .interval .interval-delete {cursor: pointer; position:absolute; top:0; right:0; font-size:2rem;}\n.graphNumberLineInteraction .shape-controls .intervals-selected .interval .deleter {display:none;}*/\n"),
define(["graphNumberLineInteraction/runtime/graphNumberLineInteraction"],function(IMSPCI){return IMSPCI});
//# sourceMappingURL=graphNumberLineInteraction.min.js.map