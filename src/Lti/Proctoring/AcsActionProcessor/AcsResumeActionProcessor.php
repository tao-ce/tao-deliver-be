...
class AcsResumeActionProcessor implements AcsActionProcessorInterface
{
    ...
    {
        return $acsControl->getAction() === AcsControlInterface::ACTION_RESUME;
    }
    ...
    public function process(
        ...
        if ($deliveryExecution->getFinishedAt()) {
            $deliveryExecution->reopen();
        }

        if (!$testSession->isRunning()) {
            $this->testSessionInitiator->startQtiSession($deliveryExecution);
            $this->timerService->beginServerTimer($deliveryExecution, $testSession);
        }

        $deliveryExecution->setStatus(DeliveryExecution::STATUS_INTERACTING);
