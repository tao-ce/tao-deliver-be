<?php

// SPDX-FileCopyrightText: 2012-2026 Open Assessment Technologies S.A.
// Copyright (C) 2022-2024 (original work) Open Assessment Technologies SA;
//
// SPDX-License-Identifier: AGPL-3.0-only OR LicenseRef-TAO-Commercial-License

declare(strict_types=1);

namespace App\TestRunner\ItemEnricher;

use App\Generator\Asset\SignedUrlGeneratorInterface;
use App\Registry\SignedUrlGeneratorRegistry;
use Psr\Log\LoggerInterface;

abstract readonly class ItemDataUrlAbstractEnricher
{
    public function __construct(
        protected SignedUrlGeneratorRegistry $signedUrlGeneratorRegistry,
        protected LoggerInterface $auditDeliveryExecutionLogger,
    ) {
    }

    protected function getSignedUrlGenerator(string $generatorName): SignedUrlGeneratorInterface
    {
        return $this->signedUrlGeneratorRegistry->getGenerator($generatorName);
    }
}
